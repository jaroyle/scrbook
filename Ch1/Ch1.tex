...
usage, are fundamental to understanding landscape connectivity, and
density dependence, and spatial interactions among individuals are
fundamental population processes.  At the same time, the inherent
spatial aspect of {\it sampling} populations strongly affects apprent
biases in how we observe population structure.  Books have been
written on spatial processes in animal populations
\citep{tilman_kareiva:1997,hanski:1999} and books have been written on
how to sample these populations using capture-recapture methods
...
capture-recapture methods that accomodate space have been developed. 
These methods, which we refer to as spatial capture-recapture methods,
..............
...
model -- processes of density, spatial organization of individuals, movement
...
capture-recapture models. The practical importance of SCR models is
that they allow
...
use of individual {\it encounter history} data, by which we mean
sequences
(usually)
of 0's and 1's denoting if an individual was encountered 
during sampling over a certain time period. For example, the
...
%models
for inference about population density, space usage and resource
...
accomodate unmarked individuals (Chapt. \ref{chapt.scr-unmarked}), use of
telemetry information (Chapt. \ref{chapt.rsf}), and developing
explicit models of landscape connectivity based on ecological
or least-cost distance
(Chapt. \ref{chapt.ecoldist}), and many other new topics that have
only recently appeared 
 in the literature.  Our intent is to provide a comprehensive
resource for ecologists interested in understanding and applying SCR
models to solve common problems faced in the study of population
dynamics. To do so, we make use of hierarchical models, which allow
extraordinary flexibility in accommodating virtually any type of
...
extend these models to unique situations.  We hope that these advanced
topics will not discourage those new to these methods, but instead out
intent is to 
allow readers to advance their
own understanding and become less reliant on restrictive tools and
software. 
...
\item[(2)] Spatial processes in capture-recapture. 
Density, space usage, resource selection,
ecological distance or landscape resistance,
...
\hl{this is a weak closing paragraph}
...
ago.  
These methods have revolutionized the study of animal populations by
...
\citep[{\it Gulo gulo}][]{magoun_etal:2011,royle_etal:2011jwm}, 
...
``devices''. Spatial encounter histories
are commonly obtained by conducting manual
...
birds \citet{kery_etal:2010}, and many other species.
Similar data structure arises from the use of standard territory or spot
mapping of birds \cite{bibby_etal:1992} or area sampling in which 
space is searched  by observers to physically capture individuals. 
This is common in surveys that involve reptiles and
amphibians, e.g., we might walk transects picking up 
box turtles \citep{hall_etal:1999}, or desert tortoises \citep{zylstra_etal:2010},
...
\citep[``MMDM''][]{wilson_anderson:1985a} seems to be the standard approach even
today, presumably justified by Dice's suggestion to use 1/2 the home
range diameter, with the mean over individuals of the maximum distance
moved being an estimator of home range diameter. Alternatively, some studies have used the full
MMDM (e.g. \citet{parmenter_etal:2003}),
because the trap array might not provide a full coverage of the home 
range (home ranges near the edge should be truncated) and
so 1/2 MMDM should be biased smaller than the home range radius. 
And, sometimes home range size is
...
% extra value!
Use of MMDM summaries to estimate home range radius is 
usually combined
...
$M_h$).  Almost all of these early methods were motivated by studies of
...
\citep{kendall_etal:1997, chandler_royle:2011} where individuals flip a coin with
...
seems practically impossible to accomplish. In other words, ``in the
study area'' has no precise meaning itself and is impossible to
characterize in almost all capture-recapture studies.  Deciding what
is ``in the study area'' is effectively the same as choosing an
arbitrary buffer which defines who is in the study area who who isn't.
That said, the temporary emigration analogy is a good heuristic for
understanding SCR models and has a precise technical relevance to
certain models.

Another interesting idea is that of using some summary of
...
adjustment to the estimated $N$ obtained by estimating the proportion
of time individuals are ``on the area formally covered by the grid''
...
\section{Classical Capture-Recapture}
...
\hl{some of this should go at the end of this section}

More generally, ordinary capture-recapture methods are distinctly
...
leads immediately to 3 main deficiencies: (1) Ordinary CR models do
...
variation.  %or something like that last sentence
 %so that all 3 issues match.




\subsection{Capture-Recapture for Modeling Detection Probability}
...
of ``model $M_h$.''  We fitted this model (see
...
make use of this information. Model $M_h$ thus merely accounts for what
we observe in our data (some bears were more frequently captured than
others) rather than explicitly accounting for the processes that
generated the data.
...
kind of buffer?  The fact that $N$ doesn't scale with $A$, as part of
...
The main point is that  N doesn't scale with A in the model, and so we
are left taking arbitrary guesses at area, and matching it up with
...
an irregular and sparse array of traps. Does it make sense that the
same model that is completely sensible for a population fo goldfish in
a bowl, should also be the right model for this population of bears?
No -- that shit is fucking insane!

There are a number of specific problems that ordinary
capture-recapture models do not provide even ad hoc solutions for. 
trap level covariates ......
spatial variation in stuff ......
...
Speficially, let ${\bf s}_{i}; i=1,2,\ldots,N$ be the locations of all
...
biased by the observation mechanism.  It is natural to think about the
...
points which we will denote by ${\cal S}$. In the point process
literature this is also called the {\it observation window} (REF
XXXX).  The region ${\cal S}$ serves as a prior distribution for ${\bf
...
%% I find the next two sentences a little confusing
  But the existence of an explicit state-space ${\cal S}$ is kind of
  beside the point -- ${\cal S}$ is really not always terribly
  important itself. Instead, as soon as you give the latent variables
  ${\bf s}$ a place to live, and this is recognized explicitly in the
  model upon which inference is based, then you achieve spatial
...
The state-space of the latent point process is the critical element of
SCR models but there are many more aspects relevant to the formulation
of SCR models for specific situations.  We address some of these in
...
on whether replication of the sampling is done.  The classical
distance sampling model with no replication (i.e., $t=1$) is a basic
model which confounds the two processes.
...
observation models.  One type of a device is what we think of as the
classical ``camera trap'' and which \citet{efford:2011} refers to as a
``proximity detector''. We can take pictures of or detect any number
of individuals and an individual can be caught in any number of traps,
and an arbitrary number of times. Iid Bernoulli model is convenient
but if you think the re-encounters are valuable then you can have a
frequency model.  Bear hair snares are slightly different because you
cannot differentiate re-encounters.  The standard observation model
that applies for ``single-catch'' \citep{efford_etal:2004} traps
posits that individuals are encountered in at most one trap per sample
occasion and traps only hold one individual.  Unfortunately we're
really screwed in the single-catch situation.  A ``multi-catch'' is
like a mist-net or other things - individual is captured and
restrained but traps hold > 1 individual. In this case, the
observation model is a multinomial. There are many variations on all
of these models and new models.

SCR models have a detection probability model which looks like a
distance sampling model -- p is a function of distance.
This implies models for resource selection or space usage (REF
XXX).....
...
(Chapt. \ref{chapt.searchencounter}) or the distribution of
...
clustering (REF XXXX).  Thus, SCR models are capture-recapture models
...
We will see that SCR models are a conceptual and technical
intermediates between the class of models referred to as model $M_h$,
and so-called individual covariate models.  We develop technical tools
for likelihood (Chapt. \ref{chapt.mle}) and Bayesian analysis
(Chapt. \ref{chapt.mcmc}).  The middle part of the book expands set of
models that we can deal with to include alternative observation models
related to the type of encounter device
(Chapt. \ref{chapt.poisson-mn}), models for encounter probability
...
for model fit and selection (Chapt. \ref{chapt.gof}).  [should include
the design chapter right here].  Finally in the last third of the book
