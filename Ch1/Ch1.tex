...
space -- 
have been developed. 
These spatial capture-recapture (SCR) methods
resolve a host of technical problems that arise in applying
capture-recapture methods to animal populations. 
...
existed for decades this is a completely new dimension of those
models.  The practical importance of SCR models is
that they allow
ecological scientists to study elements of ecological theory using
...
Thus, SCR models enable ecologists
...
\citep[``MMDM''][]{wilson_anderson:1985a} seems to be the standard
...
\citep{kendall_etal:1997, chandler_royle:2011} where individuals flip
...
compute the area are given in the {\bf R} package \mbox{scrbook} which
accompanies the book).  Hence, the estimated density here is
approximately $0.178$ bears/km$^2$ for an estimated population size
obtained using model $M_0$.  We could assert that the problem has been
...
The main point is that $N$ from a CR model is functionally unrelated
...
were before we conducted this analysis.
