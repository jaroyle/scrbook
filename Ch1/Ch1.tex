...
lions \citep{sollmann_etal:inprep} and coyotes %add scientific names
...
individuals \citep{taberlet_bouvent:1992, woods_etal:1999,
...
used in studies of martens, fishers \citep{thompson_etal:inpress},
...
analysis of such data as we discussed in Chapt. \ref{chapt.searchencounter}.
...
(Chapts. \ref{chapt.scr0,chapt.mcmc}).
...
The state-space of the latent point process is the critical and unifying element of
...
things in later chapters but, for now, we provide a concise summary of
the following things:

\subsection{ Biological focus}
density -- movement -- density + movement
...
capture-recapture models (as noted in sec. xXXXXX above). 
Indeed, there are probably now $>100$ papers applying SCR models in
the last 5 years that focus exclusively on density esetimation. 

At the same time, there are models of CR data that focus exclusively
on {\it movement} modeling.
\citep{ovaskainen:2004, ovaskainen_etal:2008}


Conversely now there are a very small number of papers that focus on
{\it boht} movement and density \citep{royle_young:2008,
  royle_etal:2011mee} or population dynamics and density
\citep{gardner_etal:2010}. 
...
in what the ecological focus is. 
 ...
 I do believe the models you (and related) are working on should be relevant for capture-recapture problems. In our applications that use explicit movement models , we have used extremely simple models out of convenience (and sparse data) and also a degree of mathematical ignorance on my part.  That said, if you look at the Royle and Young (2008) paper or the Royle et al. 2011 (MEE) paper, you can see it would be easy to replace that simple "iid movements" model with a random walk or Ar(1) type of a model (in the former case, this would be changing one line of BUGS model description I think).
 
 I think there are two important issues that need to be considered, in general, in attempting to link these two modeling approaches (i.e., that of movement models and that of capture-recapture):
 (1) most capture-recapture data e.g., by camera trapping or whatever, produces only a few observations of each individual (between 1-5 would be typical). So there is not too much information about complex movement models.
 (2) Typically people have an interest in density of individuals and therefore you need models that can be extrapolated from the sample to the unobserved part of the population.  My sense in looking at some of the movement modeling papers is that they are focused on "what is this individual doing in relation to the space it has available" and there is no formal attempt to extrapolate a sample to the population.
 
 That said, there are clearly some cases where more elaborate movement models should come into play. If one has some telemetry data in addition to SCR then there is additional info on fine-scale movements that should be useful. Another example, my colleague Beth Gardner has a project of modeling fish movements in a stream and it makes sense there should be some kind of structured up-stream movement (spawning) that could be a diffusion type model or similar. 
