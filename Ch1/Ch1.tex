...
\subsection{The encounter process}

The point process and the state-space are sufficient for describing
the ecological process of interest, %\emph{i.e.} the ``state model'',
which in the context of SCR is a model describing the number of
indviduals in the population and how they are distributed in space. In some
cases, we may want to elaborate on this state model by including
explicit models for movementThey do
not, however, fully describe how the data arise. Thus a complete SCR
model requires an additional component that describes how the data
arise
