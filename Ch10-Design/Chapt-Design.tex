...
Key thing to note about this as a criterion:
 (1) it depends on $\bar{p}$
 (2) It depends on $\hat{\bar{p}}$
 (3) We will analyze these things shortly -- it will be the case that
$\hat{\bar{p}}$ depends on $\hat{\alpha}$ and its variance on
 the variance of $\hat{\alpha}$.

 We suggest, therefore, that designs for
capture-recapture studies should seek to minimize
Eq. \ref{design.eq.theQ}. Alternatively, it makes sense to pick
designs that minimize $Var(\hat{\bar{p}})$ alone, or even variance of 
$\hat{\alpha}$. Here are a list of 4 criteria that we think are
sensible. For conveneicne we label them $Q_{1}$ - $Q_{4}$:
...
\item[(3)] $Q_{3} = 1-\bar{p}$. Designs which minimize this criterion
  ...
  should maxinmize $n$.
\item[(4)] $Q_{3} = 1-\bar{p}$. Designs which minimize this criterion
  are those which maximize the average capture probability. These
  should maxinmize $n$.
\end{itemize}
...
In order to work directly with the expressions above, we will have to
do some analysis of $\mbox{Var}(\hat{\bar{p}})$ which we take up now.
...
Here, we work with the Poisson observation model, and we do that
because the technical argument that follows is somewhat easier for
that case. In particular, we first express the integral in Eq.
...
the $\mbox{Var}(\hat{\bar{p}}({\bf X}))$ -- this is just a calculation involving sum's
