...
state-space. 
% XXX RS: When it says 'compromise between', I'd expect a second thing, not just the variance of N.
% Reading on, it sounds like that 'compromise' part could maybe be removed from this sentence because it's explained in the following.
...
density, 
% XXX RS: Maybe say again that by 'efficient' here you mean with low variance; otherwise the 'other variance-based' seems out of context
or perhaps some other variance-based criterion?

As usual, we regard the population of $N$ individual ``activity
centers'' as the outcome of a point process
% XXX RS: Sounds like the point process is distributed independently...
...
of traps ${\bf X}$ we measure a response variable
...
the frequency of encounter in trap $j$. 
...
based on the variance of the Huggins-Alho estimator of $N$.  
...
\item[(1)] $Q_{1} = \mbox{Trace}({\bf V}_{\hat{\bm \alpha}})$ where ${\bf
  ...
  are those which are good for estimating the model parameters. 
\item[(2)] $Q_{2}$ is the variance expression in
  Eq. \ref{design.eq.theQ}. We should prefer designs that minimize
  this variance.
...
\subsection{Calculation $\bar{p}$ and variances}
...
We now take up the problem of figuring out the form of
$\mbox{Var}(\hat{\bar{p}})$.
We work with the Poisson observation model, and we do that
...
\ref{design.eq.pbar} by a summation over a fine mesh of points so
that:
\[
...
The big picture is this: For a given design ${\bf X}$, we can compute
 $\mbox{Var}(\hat{\bar{p}}({\bf X}))$ -- this is just a calculation involving sum's
...
out the asymptotic form of this matrix.  For our purposes,  we think
it might suffice\footnote{Warning: But we don't know. No warranty is implied.} to a approximate the matrix, using the analogous result from a Poisson or Binomial GLM assuming
that $N$ is known,
since we have conveneint formulas for those.
 In particular, if we knew the
...
outlined above and compute the 
 {\it expected} information obtained from a
particular realization of $N$ individuals, and invert that result. In
particular, the total information for all $N$ individuals is
\[
{\cal I}(N) =  {\bf M}_{1}' {\bf D}_{1} {\bf M}_{1} + \ldots +
{\bf M}_{N}{\bf D}_{N} {\bf M}_{N}
\]
If {\it individual activity centers are
  independent}, we can compute the expected information 
 over {\it all} elements of the state-space, which is just $N$ times
 the average information of a  single individual:
\[
\mathbb{E}( {\cal I}(N) ) = N  \sum_{{\bf s}}  M({\bf s})'D({\bf s}) M({\bf s}).
\]

To evaluate a design criterion for a single design ${\bf X}$, we need
to compute this quantityk, invert it to get the variance of alpha hat,
plug it into xxxx to get the variance of p.bar.hat, use the parametes
to compute p.bar, and then Bob's your uncle. 
