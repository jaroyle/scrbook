...
problems. We discuss some of these here. 
...
the attribute being observed is not, for the specific sample that is chosen. However, in the SCR modeling
...
encounter histories.  It makes sense in this context that we should
...
population, and for calculation of density. The ``no holes''
...
As a consequence, trap spacing should be on the same order as the
...
\% and 97.5 \% quantiles, relative bias of mean (RB) and 95\% Bayesian
credible interval (BCI)
...
on 
bias and precision of estimates of $N$,  
we expanded 
the simulation study of 
...
around the traps was constant in units of $\sigma$. 
 For each trap spacing
...
and
the number of spatial recaptures 
...
requires sampling $J$ sites, then the 3 strategies are:
\begin{itemize}
...
model, where the optimality is with respect to $Q({\bf X})$.

What types of functions make reasonable objective functions, $Q({\bf X})$?
 We will
...
for a minute).
So we have a response variable that has a normal linear model of the form:
\[
...
The inference objective here is to estimate the parameters ${\bm \alpha}$.
The variance-covariance matrix of $\hat{\bm \alpha}$ is, supressing
the dependence on ${\bf X}$ for notational convenience, 
\[
...
For a population of $N$ individuals, if we
...
know ${\bf s}$ then we might consider minimizing  the expected
(spatially averaged) variance:
\[
...
function of this  integrated information matrix. E.g., find the design
that maximizes the
diagonals, or the determinant. 
This can be done for any number of design points ${\bf x}_{1},\ldots,
{\bf x}_{J}$ using standard exchange algorithms
\citep[see][Chapt. 3]{muller:2007} and we discuss this below
in Sec. \ref{design.sec.exchange}.
%which always improve the criterion but will not necessarily yield {\it
%  the} optimal design. 
However, our SCR models are not normal
linear models but, rather, more like Poisson or binomial GLMs. We see
in the next section that we can adapt these ideas for such models. 


\subsection{Formalization of the Design Problem for SCR Studies}


Following our development of the normal linear model above, 
...
counts, and the linear predictor has this form:
\begin{equation}
...
Huggins-Alho estimator of $N$.  We 
find that these expressions depend on
individual sample inclusion probabilities (if ${\bf s}$ is close to
traps, the individual has a high probability of being encountered and
{\it vice versa}), and hence  
the
specific trap locations,  and parameters of the model
These variance expressions provide a natural design criterion. On the
other hand, we find that the calculus 
is a bit tedious at the present time.
...
directly.  This merits further investigation.  
As an alternative to both of those approachs, 
we instead adopt an
approach where we devise 
a variance criteria based on
the conditional estimator of $N$ having the form
\[
  \tilde{N}  =  \frac{n}{\hat{\bar{p}}}
\]
where $\hat{\bar{p}}$ is the MLE of the probability that an individual
...
MLE of the parameters of the encounter probability model, $\hat{\bm
  \alpha}$. In SCR models, an individual with activity center ${\bf
...
also the design -- the trap locations ${\bf x}_{j}$. To get the $\hat$
on these expressions, we need to plug-in the MLE of the parameters. 
 The {\it
...
\frac{n^{2}}{\hat{\bar{p}}^{2}} \mbox{Var}(\hat{\bar{p}}) + \frac{1}{\hat{\bar{p}}^{2}} \mbox{Var}(n)
...
Eq. \ref{design.eq.theQ}.
For operational purposes we replace $n^{2}$ with its expected value
and substitute $\mbox{Var}(n) = N\bar{p}(1-\bar{p})$, do some
rearranging and this produces the following:
\begin{equation}
 \mbox{Var}(\tilde{N}(\bm \alpha) ) =
N^{2} \mbox{Var}(\hat{\bar{p}}) +  \frac{N(1-\hat{\bar{p}})}{\hat{\bar{p}}}( \mbox{Var}(\hat{\bar{p}}) + 1)
\label{design.eq.theQ2}.
\end{equation}
Based on this development, we identify 3 potential design criteria:
\begin{itemize}
\item[(1)] $Q_{1}$ is the variance expression in Eq. \ref{design.eq.theQ2}.
\item[(2)] $Q_{2} = \mbox{Var}(\hat{\bar{p}})$ 
\item[(3)] $Q_{3} = 1-\bar{p}$. Designs which minimize this criterion
  are those which maximize the average capture probability. These
  should maxinmize $n$.
\end{itemize}
To make use of this expression in a particular design problem, we need
...
{\bf \HUGE some of this in an appendix, appendix 2 perhaps? }

\subsection{Calculation of the variance of $\hat{\bar{p}}$}

In order  to work directly with the expressions above, we will
have to do some analysis of $\mbox{Var}(\hat{\bar{p}})$ which we take up now.
...
key point because, then, we can use
a delta approximation to  express $\mbox{Var}(\hat{\bar{p}})$ in terms of the variance of
the MLE $\hat{\bm \alpha}$.  

Here, we work 
with the Poisson observation model, and we do that because 
...
\right\}
\]
To compute the variance of the MLE of $\bar{p}$, we note that the variance
...
\mbox{Var}( \hat{\bar{p}}({\bf X}) )  = \sum_{{\bf s}}
 \mbox{Var} \left(
 \exp(-     \sum_{j}   \exp(\hat{\alpha}_{0} + \hat{\alpha}_{1}  d({\bf x}_{j}, {\bf s})^2))
   \right)
\]
At this point we apply the Delta approximation to produce
\[
\mbox{Var}( \hat{\bar{p}}({\bf X}) )  = \sum_{{\bf s}}
\left( \exp( - \sum_{j} exp(\hat{\alpha}_{0} + \hat{\alpha}_{1}  d({\bf x}_{j}, {\bf  s})^2) )
\right)
...
random starting designs.  The putative optimal designs
(henceforth ``best'') are shown
in Fig.
\ref{design.fig.designs}.
  For J=9, $\sigma =2$, the
...
For the 11 point designs we have this:
\begin{verbatim} 
...
> #points(a3.1$Xlst[[1]],pch=20,col="blue")
> dbar(a1.2$Xlst[[1]])
[1] 1.872972
> dbar(a2.2$Xlst[[1]])
[1] 2.207012
...
The results are shown in Figure .... These show the best 11 point
designs in the left panel for the var alpha-hat criterion and the
variance N hat. It turns out that designs that maximize pbar are
essentially the same as those which min var Nhat so we don't show the
designs that max pbar here but they are very similar.
The 21 point designs are in the right panel.  
Comments:
these are uniform designs for the most part. But, note, the trap
spacing varies {\it even though} $\sigma$ is the same. In one case the
average spacing is 2.0 and 2.4 for the var(alph ahat) and var(N hat)
designs and, in the other case, it is 1.4 and 1.7 for the 2 designs
with 21 points. These are about 1-1.8 times sigma.
This is somewhat paradoxical that they don't match up, we need to
understand that. 
