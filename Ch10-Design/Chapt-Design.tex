...
couple of specific case studies that apply to the Bernoulli sampling
...
radius of a typical home range (e.g., \citealp{dillon_kelly:2007}).
...
\sigma$. {\bf XXXXX ANDY:SIMULATION DETAILS XXXXX}. For each trap spacing
...
Huggins-Alho type of problem. Follow along their analysis for
developing variance estimators, we would find that these depend on
...
is 
a bit tedious at the present time.
...
directly.  This merits further investigation.   We adopt a 3rd
approach here and devise  
...
captured in {\it any} trap and therefore, under the Bernoulli (passive
detector) model,
\[
...
on a delta approximation to $\mbox{Var}(n/\bar{p})$:
\begin{equation}
 \mbox{Var}(\tilde{N}(\bm \alpha) ) =
\frac{n^{2}}{\bar{p}^{2}} \mbox{Var}(\bar{p}) + \frac{1}{\bar{p}^{2}} \mbox{Var}(n)
...
\begin{equation}
 \mbox{Var}(\tilde{N}(\bm \alpha) ) =
N^{2} \mbox{Var}(\bar{p}) +  \frac{N(1-\bar{p})}{\bar{p}}( \mbox{Var}(\bar{p}) + 1)
...
However, in order to work directly with this expersssion we will
