...
where 
 ${\bf M}$
is the $J \times 2$ design matrix where the 2nd column contains the
squared 
pairwise distances between each individual $i$ and trap $j$, and thus
it depends on both ${\bf X}$ and ${\bf s}$.
A simpler model is the normal linear model, of the form: 
\[
...
\[
 \mbox{Var}( {\bm \alpha}) = ({\bm M}({\bf s})'{\bm M}({\bf s}))^{-1}
\]
Note that the
design points ${\bf x}_{j}$ appear explicitly (in the 2nd column of
{\bf M}).  In considering design
for estimation in such models it is natural to choose design points,
corresponding to values of ${\bf x}$, such that the variance of
$\hat{\bm \alpha}$ is minimized. 
In particular, for a population of $N$ individuals, 
 if we know {\it all} $N$ values of ${\bf s}$ 
we could now
...
\]
Or we can maximize the expected information which is probably more
appealing from an analytic point of view because the maximum
information is the sum of each point ...... it is the ``integrated
information''.......
in the same way as the integrated likelihood.........
This can be done for any number of design points ${\bf x}_{1},\ldots,
{\bf x}_{J}$ using imperfect exchange algorithms XXXX REF XXXXXX
