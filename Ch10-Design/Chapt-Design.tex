...
traps (or sampling devices) within the array.  For traditional capture-recapture, conceptual and
heuristic design considerations have been addressed by a number of
authors (e.g., \citealp{nichols_karanth:2002}, Chapt. 11), but little
...
grid size (relative to animal home range area) on capture-recapture density estimates using a simulation
...
simulation in terms of their accuracy and precision. In the past few years, however, a growing number of simulation studies addressing questions of study design in the context of spatial capture-recapture have come out (e.g., \citet{marques_etal:2011, sollmann_etal:2012, efford_fewster:2012, efford:2011ecol}), the results of which we will discuss throughout this chapter. 
...
simulation of specific design scenarios \citep{sollmann_etal:2012} based on trade-offs between
...
% RS: Added citations to intro above; adding more specific recommendations by Murray below
...
spatial units and we sample those units  randomly (or by some other method, such as
generalized random tesselation stratified (GRTS) sampling \citep{stevens_olsen:2004})
and measure some attribute. The resulting inference
...
the sample (i.e., an individual exposed to more traps is more likely to be included into our sample than an individual exposed to few traps) and so we must also learn about these unequal probabilities
...
With these general concepts of spatial sampling and the sampling of individuals in mind, we can now turn our attention to more specific aspects of study design in SCR surveys, namely the spatial arrangement of detectors. 

%XXX RS: I feel like there should be some concluding sentence to this section. I put in a suggestion but it's somewhat vague. I just think the way it is now this section ends too abruptlyXXXX
...
most unique individuals, but probably few spatial recaptures.  We will formalize this trade-off later, when
...
population, and for calculation of density. The ``no holes'' assumption stems from the fact that, conceptually, traditional capture-recapture models represent a classical ball-and-urn experiment, in which the study area -- however difficult to define -- is the urn and the population of animals in that area is the population of balls in that urn. Capturing an individual is equivalent to drawing a ball from the urn and then putting it back in. Clearly, each ball in the urn has some probability of being drawn; otherwise it is not part of the population of balls in the urn. In other words, if an animal lives in a ``holes'' -- a part of the study area with no traps -- it has no probability of being detected (``drawn'') and therefore is, by definition, not part of the population we sample. 
...
survey populations of both jaguars ({\it Panthera onca}) and the much smaller
ocelots
...
jaguars.  
The ``no holes'' assumption entails some strong restrictions with respect to study design. Although we need not cover an area systematically with traps, there has to be some consistent coverage of the entire area of interest. Often, this is achieved by dividing the study area into grid cells, the size of which approximates an average home range (or possibly the smallest home range recorded for the study species in the study area or a similar area; e.g. \citet{wallace_etal:2003}), and then place (at least) one trap within each cell. 
% Within cell placement can be random, or in a way that optimizes detection of the target species (e.g., along roads and trails for large cats; \citep{karanth_nichols:2002}), or based on logistic considerations (ease of access).  
In many field situations, especially when dealing with large mammals and accordingly large study areas, achieving this consistent coverage can be extremely challanging or even impossible. Depending on local environmental conditions, parts of the study area can be virtually inaccessible to humans, because of dense vegetation cover, or unsuitable for setting up detectors, because of flooding. Even when accessible, setting up traps in difficult habitat conditions can consume unproportional amounts of time, manpower and other resources. 

Where approaches such as MMDM are used in combination with
...
Though closely related, the requirements in terms of spatial study design for SCR models differ distinctly from those for traditional CR. For one, holes in the study area are of no concern in SCR studies.  As a
...
which is not accommodated in those models. This alone allows for completely new and much more flexible study designs in SCR studies, as compared to traditional CR, such as linear designs, ``hollow grids'' (detectors trace the outline of a square), or small clusters of grid spread out over larger landscapes  \citep{effort_etal:2005, effort_etal:2009euring, efford_fewster:2012}. 
...
\subsection{Summary thoughts on spatial trap arrangement in SCR models}
...
\citep{maffei_noss:2008} -- covering at least 4 times the average home range is recommended. Further, we need consistent coverage of the entire study area, as all individuals in the population of interest must have some probability $>0$ to be captured. 

In contrast, SCR models work well in study areas on the scale of an individual's home range (as long as sufficient data is collected) \citep{sollmann_etal:2012, efford_etal:2011ecol, marques_etal:2011}, and they provide unbiased estimates for sampling designs that do not expose all individuals in the sampled population to detectors, i.e., that have ``holes'' \citep{efford_fewster:2012}. 
These results, however, should not encourage researchers to design non-invasive
trap arrays based on minimum area requirements and with a minimum number of detectors. Study
...
amounts of data, both individuals and recaptures, do not only improve precision of parameter estimates
\citep{sollmann_etal:2012, efford_etal:2004}, they
also allow including potentially important covariates (such as gender
or time effects in the black bear example -- see Chapt. \ref{chapt.covariates}) into SCR models to obtain
density estimates that reflect the actual state of the studied
population.

Beyond the traditional grid-based sampling design, the flexibility of SCR models allows for different patial detector arrangements such as linear (see \citet{efford_etall:2011} for an example), or dispersed clusters of detectors (see also below). How well these different designs perform, comparatively, remains to be explored. 
...
\citet{karanth_nichols:2002} describe 3 variations of moving traps to
...
requires sampling $J$ sites, then the 3 ``designs'' are:
\begin{itemize}
...
interest. They found that in a spatial framework this design performed well, although there
...
spatial recaptures {\it within} clusters, meaning that spacing of
detectors within clusters has to be appropriate to the movements of
the species under study.
A clustered type of design is also suggested by \citet{efford_etal:2009ecol} for
acoustic detectors (see Chapt. \ref{poisson-mn.sec.acoustic}, with small groups of such detectors 
(e.g., $2 \times 2$) being distributed in a probabilistic fashion
...
on hair snares distributed over a 2625 km$^2$ study area. She simulated
detection data of bears for 3 trap arrangements including a regular
...
moved mid-way through the study to fill the gap (a sequential or 
``rotating'' design).  She found that the precision and accuracy of estimates of
$N$ generally decreased when changing from a uniform to
a clustered to a rotating design, although the loss of
efficiency was relatively small when using the clustered design. The result seems to support
that cluster designs can be effective with relatively little loss of efficiency.

Further 
research on optimal detector configurations, espacially for large sclae studies,
...
needed.  We believe the model-based spatial design approach that we
introduce below is one possible way to do that.
...
We just introduced or motivated the construction of some design
criteria based on a normal linear model. However, we emphasize that
asymptotic formulae for
$\mbox{Var}( {\bm \alpha})$ can be cooked up for any type of GLM. As
an example (we use this below), 
for the Poisson GLM, the asymptotic
variance-covariance matrix of $\hat{\bm \alpha}$, considering a single
individual having location ${\bf s}$, is\footnote{
This is basic GLM theory that derives from the fact that
  the Poisson is a member of the natural exponential family of
  distributions, e.g., see
 \citet{mccullagh_nelder:1989} or
  \citet{agresti:2002}.}
\begin{equation}
...
This is a 
 ...
 are balled-up in the regression design matrix ${\bf M}$,
and the matrix ${\bf D}$ which is a diagonal
matrix having elements $\mbox{Var}( y_{j}|{\bf s}) = \exp({\bf
  m}'{\bm \alpha})$ for $y_{j}$ the frequency of encounter in trap
$j$. We can compute the integrated information under the Poisson model
with known $N$ using this modified formulation. 
These ideas are meant to motivate technical concepts related to
model-based design, where we know $N$, and therefore have a convenient
variance or information expression to work with. However, in all real
capture-recapture applications we won't know $N$, and so we need to
address that issue, which we do in the next section. 
...
flexibility and unambiguous interpretation. 
...
then  optimizing the criterion over all possible designs. 
