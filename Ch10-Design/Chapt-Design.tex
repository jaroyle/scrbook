...
in order to understand how factors 
%XXX RS: Would be good to add what kind of factors
influence the design problem. However, often field studies face
logistic difficulties, especially when dealing with wide-ranging, rare
and cryptic species like large carnivores. The need to sample large
areas with limited resources often forces researchers to compromise
between a study design that is optimized for statistical inference and
a study design that is logistically viable or efficient. In such
...
design where traps are spread out uniformly over the study site.
% XXXX RC: Would be good to conclude this paragraph by indicating the
% implications of such design choices
% XXX RS: We pick up on that later on (the no-hole thing); I don't know if we should remove this paragraph?
% XXX Andy sez: I commented it out. It is a lot of words, per unit
% idea i think. There's something here, but its not done.
\end{comment}
...
of specific design scenarios %\citep{sollmann_etal:2012}
...
terms of its ability to generate useful estimates, % of things,
...
your PhD thesis and, hopefully, your career. % XXXX Well said!
...
inference objective.  % XXXX RC: Andy, you should cite your grad work
                      % on spatial design. Andy sez: I did below in
                      % the technical stuff.
...
%XXX RS: I think this would be much clearer if we had an example. Something like (if I udnerstand correctly): 
% For example, the attribute we are interested in could be the degree of canopy closure, and we could measure this variable
% at a random number of location.
% andy sez: I tried to do that.
...
%% random ones.....
%% XXX RS: Don't know if the '.' after 'locations' should just be a comma or if there's something missing between the two parts.
%% Andy sez: you r right.
...
% linger for decades.
%% Andy sez: yea, will be funny to see this exact quote: "We could be
%% a bit sloppy in our definition of the study area and not be too
%% worried about that (Royle, Chandler, Sollmann and Gardner 2013).
...
As a consequence, trap spacing is recommended to %should
...
%% XXX RS: In our sims parameters became unidentifiable with a lack of spatial recaptures. I think maybe you know that sigma has to be small, but not how small... don't know.
...
% XXX RS: I'd leave it out; we discuss the results a little below.
...
estimable % XXXX RC: posterior=prior? Here is what Wikipedia says:
...
% it."
%% Andy sez: Screw those assholes!
% XXX RS: Estimable better? Actually, Ken Pollock pointed that out to me before...
in 88\% of the
...
where a trapping grid that is small relative to animal movement% XXXX RBC says: this isn't so clear. Do you
...
% XXX RS: Is this better? I think we discuss before that you should use a grid 4x the home range, as a rule of thumb, so I'd hope that what we have here should be understandable. But if it's that unclear we can spell it out again, that a small grid doesnt capture the full extent of movement and thus the buffer around the traps will be too small and density too high. 
...
% XXXXX RS: As per suggestion by RBC I commented the following out here (it's at the end of the chapter)
...
the variance of estimating $N$ for a prescribed
state-space. 
...
exposed to capture.  By seeking to minimize the variance of an
...
% aren't done....
% XXX RS: How about just one sentence somewhere in this paragraph saying something along the line of 'We have to take a guess at S, which we usually define a posteriori, as part of our analysis, but considering that we usually may have some idea of how far animals move, it should not be too problematic to make an educated guess as to S.'
% Andy sez: lets not belabor this point -- we have 20 pages talking
% about choosing S in the rest of the book ;)
...
strength of an
acoustic signal,
% XXX RS: What's the response variable here? In the context my mind went straight to distance... but I'm guessing that's not it...
% ok, you say no context... but again, I think some even vague example would really help, also for alpha
 that has a normal
distribution.
% (without identifying specific context, just buy into this
%for a minute).  
...
the frequency of encounter in trap $j$ and where ${\bf m}'$ is a row
of ${\bf M}({\bf s})$. 
% XXX RS: What's the lower case m in the formula above?
% this is  a good question. I think it must be a row of M but I wrote
% this > 5 years ago so i forget ;)
...
% XXX RS: Could we add a reminder sentence what the H-A estimator is/does?
...
\end{equation}
% XXX RS: Isn't this new thing conditional on N, which we also don't know? 
% Ok, you mention that later; it just seems odd that that's preferrable considering that above it says that we have to take into account that we don't know N.
% Andy sez: this is a good point, and one that causes the statistician
% to pull his hair out (or lose it at a faster rate). We don't know N
% --  but the sampling distribution
% of its estimator depends on N. So you need a random stab in the dark
% at what N is to do the design. So basically we're just running in
% circles ;)
...
criterion can be evaluated for any design. i.e., what arethe values of
...
operates on a discrete representation of ${\cal S}$ (an aribtrary
...
the 4 criteria we described above. To refresh your memory $Q_{1}$ is
the trace of the variance-covariance matrix of $\hat{\bm \alpha}$,
 $Q_{2}$ is the variance of $\tilde{N}$,
$Q_{3}$ is $1-\bar{p}$ (so the design that minimizes this
criterion obtains the highest possible $\bar{p}$), and $Q_{4}$ is 
the variance of $\hat{\bar{p}}$.
...
{\bf STUFF BELOW HERE IS NOT RIGHT}
\begin{verbatim}
> 
> dbar(a1.1$Xlst[[1]])
[1] 2.586636
...
distance of $1.87$ for $Q_{1}$ and 2.21 for $Q_{2}$ but,
qualitatively, the structure of the designs is similar to $J=11$. 
...
\caption{Best designs for each of 4 design criteria, produced using
  ...
  designs, and the right panel shows the best $J=21$ point designs.
}
...
periods
 ...
 demographic closure.
% XXX RS: I am confused - isn't Pollock's design for a fully dynamic model? Otherwise, it sounds like the multi-session model.  
%(Chapt. \ref{chapt.multi-session})
% Yea: it could be that too. Doesn't have to be fully dynamic. "Random
% temporary emigration" is the standard application of Pollock's
% robust design (see Kendall papers on that).
...
The nice thing about simulation studies is that we can simulate data for
...
% XXX RS: Andy, see if you agree; I moved the next 4 paragraphs up since this stuff is a summary of the earlier part of the chapter; then the design based stuff, then the telemetry outlook.
% andy sez: I'm good with this.
When designing a capture-recapture study for a single species, trap
spacing and the size of the array can (and should) be tailored to the
spatial behavior of that species to ensure adequate data
collection. However, some trapping devices like camera traps may
