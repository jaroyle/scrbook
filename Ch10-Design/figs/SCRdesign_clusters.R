...
SCRdesign<-function(S=S,C=C,clusters=NULL,ntraps=9,ndesigns=10,nn=19,sigma=2,crit=3){
...
if(is.null(clusters)){
## needs to be modified to distance among clusters.....
...
## I think this is something related to distance between clusters but not _actual_ distance.
Cd<-round(e2dist(C,C),8)  # distance among candidates
Cd<- clusters%*%Cd%*%t(clusters)
NN2<-NN<-matrix(0,nrow=nrow(Cd),ncol=ncol(Cd))
for(i in 1:nrow(Cd)){
xx<-Cd[i,]
NN[i,] <- (xx>0 & xx<= sort(xx)[nn])
...
if(is.null(clusters)){
	...
	# next line figures out all sites contained in the clusters
	# note here some sites can be in > 1 cluster which is ok
...
for(i in 1:ntraps){
# have to remove X.current [can be a cluster of sites]
chk<- NN[X.current[i],]
chk[X.current]<-0
...
## need to resolve this issue here 3/25/13
   #Xtest<-X
   #Xtest[i,]<- C[x.consider[j],]  
# this is complicated for a whole cluster..... you have to remove all the sites in the cluster 
# and add in all the sites from the candidate cluster
   #xxxx<-Qfn(Xtest,S)
   #qtest[j]<- xxxx[crit]
Xtest.current<-X.current  # this is just a list of clusters thats all.
Xtest.current[i]<- x.consider[j]
which.test<- apply(clusters[Xtest.current,],2,sum)>0
...
Q<-min(qtest)
kp<- qtest==min(qtest)
X.current[i]<-x.consider[kp][1]  # if one had a smaller criterion then swap it here

#### new to deal with clusters
if(is.null(clusters)){
	X<-C[X.current,]
}
else{
...
### end new
### replaces this line:   X<-C[X.current,]
...
a1<-SCRdesign(S,C,clusters=clusters,ntraps=11,ndesigns=10,nn=15,sigma=1,crit=6)
