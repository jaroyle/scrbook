...
spatial-capture situations, but here we focus on the binomial model in
which we suppose that $J$ traps at locations ${\bf x}_{j}$ are
operated for $K$ periods (e.g., nights).  The observations are the
individual- and trap specific counts $y_{ij}$ which are binomial
counts with sample size $K$ and probabilities $p_{ij}$. A standard model
(Borchers and Efford 2008) is the Gaussian model in which 
\[
log(p_{ij})= \theta_{0} - \theta_{1} ||{\bf x}_{j} - {\bf s}_{i}||^{2}
\]
