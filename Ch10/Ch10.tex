...
We develop an example here using our made-up 16 pixel raster above which we regard
for the pursepos of this example as an extremely coarse landscape covariate, e.g., at
say a 10 x 10 km resolution. This 4 x 4 raster therefore defines a landscape of 
40 x 40 km and we suppose that at the center point of each raster is a camera trap
and we are studying a population of $N$ animals such as tigers at some extraordinarily
high density. We suppose that each raster is characterized by a single covariate which
is increasing from XYZ to XYZ so that effectively there is an increasing gradient from the 
NW to the SE. We define 
\[
 cost(x) = exp( \beta z )
 \]
where $\beta = 1$ which we regard as known. Of course this is a model and it is completely
...
\subsection{Construction of the likelihood}

The likelihood is really just this.....
\[
 big formula
\]
which we optimize using the nlm() optimizer in {\bf R}. The code for simulating a data
set and fitting the model is given in the following set of commands.


\subsection{Simulation studies}

We generated N=100 individuals with this set of parameter values XYZ and we fit the true
model (ecological distance) and the wrong model (Euclidean distance) to see what effect, if any,
this would have. A sample realization of this is shown in Fig. 
\ref{ecoldist.fig.raster100}.
We simulated a few levels of N and detection probability 
low p: alpha0<- -2     sigma<- .5    K<-10
...
Secondly then we did a 3rd set where we estimated the parameter $\beta$ to see if we could estimate
it effectively. 
...
Interest in SCR models has grown rapidly in the last several years since their formalization
using likelihood (Borchers and Efford 2008) and Bayesian (Royle and Young 2008) methods. 
This is because of the universal ubiquity of auxiliary spatial information in all
capture-recapture stucdies and thus the unifersal applicability of SCR models. 

HEre we have developed for the first time a formal framework for estimating
the relative cost of animals moving around the landscape from capture-recapture data.
The cost is manifest in encounter rate data from which we formally fit encounter probability
models using a modification of the SCR likelihood.
...
problem for fish, abmphibians or animals that favor riparian habitat. 
