...
%  royle_etal:2009ecol}.
A central feature of spatial capture-recapture models is the
expression of encounter probability
...
has been based on encounter
probability models in which distance between individual activity
center $\bf s_i$ and trap location $x_j$ is defined by a function of
simple Euclidean
distance.  While these models will often be sufficient for practical
...
non-stationary home ranges of individuals. As an example, Fig. \ref{fig.homeranges} shows home ranges of 6 individuals
...
XXX MAYBE SAY SOMETHING EXPLICIT ABOUT DIAGONAL MOVEMENTS... SILLY BUT IT TOOK ME A COUPLE OF MINUTES TO GET WHERE THE SQRT(2) COMES FROM; WHAT MIGHT HELP, TOO, IS JUST STATING THAT THE EUCLEDIAN DISTANCE BETWEEN PIXELS IN THIS EXAMPLE IS 1 UNIT XXX
...
spatial capture-recapture model XXX I GUESS THE CHAPTER WON'T HAVE AN APPENDIX(Appendix 1)XXX. The {\bf R} package
...
To compute the cost distance we prescribe a set of points, or XXX we can compute it XXX between
...
direct hypothesis test of a parameter, or to tabulate a bunch of AIC values. It turns out, for this class
...
distance metric. XXX As a consequence, home ranges contours are no longer circular, as in SCR models based on Euclidean distance. XXX For example, using one of the covariates we use in
...
\ref{tab.results1} XXX 2 ?XXX below.  For the ``patchy'' landscape we see extreme
...
the boundary XXXX(see Appendix 3) IS THERE AN APPENDIX?XXX.
...
define any potential state-space restrictions XXX DOES THAT MEAN PIXELS WITHIN S HAVE A '1' AND OTHERS HAVE A '0'? XXXX. In the analysis of this
...
results do not differ too much in this single instance.XXX ANY SUGGESTION WHY?XX
