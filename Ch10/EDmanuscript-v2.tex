...
\title{Spatial Capture-Recapture Models for Jointly Estimating
...
\begin{flushleft}
{\em \bf Abstract}
Assessing the impacts of habitat fragmentation and habitat loss
on population density and landscape connectivity are high priorities
in applied ecological research, yet no methods exist for
simultaneously estimating density and connectivity.
...
landscape connectivity. Connectivity is determined by both population
density and landscape resistence, i.e. the degree to which landscape
structure impedes movement, and numerous studies have focused on the
implications of increased landscape resistence resulting from
fragmentation \citep{ricketts:2001,adriaensen_etal:2003}. However, few
methods exist for estimating resistence
parameters, and instead, ecologists often use expert opinion or
\textit{ad hoc} methods of specifying resistence values
\citep{zeller_etal:2012}. In addition, no
methods are available for simulaneously estimating landscape
resistence parameters and population density. Such a method is needed
to evaluate theory predicting interactive effects between population density
and connectivity on population viability.
...
distance sampling and capture-recapture models, which require spatial
...
their use is already becoming widespread \citep{efford_etal:2009ecol,
  ...
  gopalaswamy_etal:2012, foster_harmsen:2012}. Furthermore,
essentially all capture-recapture studies produce auxiliary spatial
information and therefore SCR models
are directly relevant to standard data collected from such studies.
\begin{comment}
The basic idea is to express capture %encounter
...
%, and they provide a framework for modeling of trap-specific covariates.
...
In spite of the utility of SCR models for estimating population
density, these  methods are still in their
...
distance.  While these simple encounter probability models will often
...
The basic idea of SCR models is to express capture %encounter
probability %of individuals
as a function of the distance between an individual's center of
activity, say ${\bf s}$, and a trap location, say ${\bf x}$.
%In these models ${\bf s}_{i}$ is regarded as a latent variable and
