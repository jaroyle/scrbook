...
distance, or the {\it least-cost path}, between any two locations which
...
development
of models for cell-specific cost.
...
\begin{comment}
In many studies of landscape connectivity, when data on dispersal
and gene flow does not exist, practicioners have used other data,
including simple occupancy, or resource selection functions,
to parameterize corridor design \citep{zeller_etal:2012} models.
%Use of our approach for corridor design requires only the assumption that
%animal movement is similarly impacted by the landscape during home
%range movements and dispersal movements, whereas all the other
%approaches also have to guess at the relationship between occupancy or
%habitat-use (non-movement based metrics) to cost.
\end{comment}
...
and DNA sampling, species which
