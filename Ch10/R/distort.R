...
cost <- exp(3*raster(matrix(elev.z, len)))
tr <- transition(cost, function(x) mean(1/x), directions=16)
...
pr <- seq(0, 1, by=0.1)
contour(sp, sp, t(p), levels=pr, axes=FALSE, frame=TRUE)
plot(raster(elev.z), axes=FALSE)
contour(sp, sp, p2, levels=pr, axes=FALSE, frame=TRUE)
par(op)
dev.off()
