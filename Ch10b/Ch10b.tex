...
%relatively simple
SCR models to describe the observation process. %They included different models of the
%observation process,
...
behavioral responses %and other things
that can affect detection
probability.  %That said, the models remain pretty basic in the sense
Although these various observation models are immensely useful, they
...
underlying space utilization process. \hl{I added this next sentence
  because otherwise the previous sentence is too much of a cliff
  hanger} As we will explain, this latent
variable is the number of times that an individual occurs in a
particular pixel of the raster representing the landscape.
...
telemetry data. \hl{ANDY, is it necessary to have the minus sign
  before alpha? I added the next lines for clarity.} $\alpha_2$ is the effect of the landscape
  covariate $z({\bf x})$ on the relative probability of use. Thus,
  if $\alpha_2$ is positive, the relative probability of use decreases as the
  covariate increases. In practice, we don't get to observe $n_{ij}$ for all
...
the multinomial cell probabilities above \hl{there is a lot of info
  packed into this one parenthetical sentence. Perhaps you could walk people through it a
  little more slowly}).  Therefore, for the
...
equivalent model of space usage (sec. XXX in Chapt. \ref{chapt.scr0})
...
z({\bf x}_{j}))$, which ensures that the utilization distribution is a
...
simulated a covariate that represents %typical patterns of
...
Eq. \ref{rsf.eq.rsf}. \hl{Is there an unbiased estimator of the
  activity center as defined here?}
...
independent should only entail a minor %an incremental
loss of efficiency
...
{\bf y}_{scr})$
\hl{This is the first time $alpha_0$ has been discussed in this
  chapter. Should it be included in previous equations?}
...
estimation, not space usage.% are regarded
%more as methods for estimating density.
However, it is intuitive that %Although models of
space usage should affect encounter probability and thus be highly
relevant to density estimation in SCR applications, but
%seem to underly the simplistic models for encounter probability,
this
...
practice. \hl{ANDY, they actually argued that this requirement is not
  true--I need to go back and reread their paper.}%y do not propose ways of adjusting estimates of that quantity for
%spatial sampling bias in the individuals that are telemetered.
In
