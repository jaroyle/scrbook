...
individual uses a particular pixel of the landscape.
...
use is the outcome of individuals moving around their home range but,
i.e., where an individual is at any point in time is the result of
some movement process. But, to understand space usage, it is not
...
\begin{figure}
\centering
\includegraphics[width=3.5in,height=3.5in]{figs/habitat.png}
...
related to the amount of movement, or number of selection decisions
...
values of $E(n_{ij})$.  This is an equivalent model to the multinomial
...
Chapt. \ref{chapt.poisson-mn}).  We see that the intensity parameter
$a_{0}$ is not relevant for understanding patterns of space usage. In
particular, it cancels from the numerator and denominator of the
multinomial cell probabilities, analogous to what happens under
...
\begin{equation}
y_{ij}| {\bf s}_{i} \sim \mbox{Bin}(K, p(D_{ij}; {\bm \alpha}))
...
\[
 cloglog(p_{ij}) = -2  -\frac{1}{2\sigma^{2}} D_{ij}^{2} + 1 \times z({\bf x}_{j})
\]
...
deviation 2.
These settings yielded an average of $n=61$ individuals captured for
the $N=100$ case and $n=XXX$ for the $N=200$ case. The later case
...
\begin{verbatim}
N=100, 300 iters each, mean SCR only N: 99.418
...
N=200, 500 iters. Mean SCR only N = 199.712
n=2
SCR only 198.85  14.24   0.99   0.10   2.00   0.091
SCR/RSF  199.37  12.80   0.99   0.09   2.00   0.078
sbar     197.87  13.94   0.96   0.10   1.99   0.080
RSF only   NA      NA    1.04   0.33   1.99   0.169

n=4
SCR only 200.06  15.34   1.00   0.09   2.00   0.092
SCR/RSF  200.25  14.36   1.00   0.08   2.01   0.073
sbar     198.14  14.31   0.94   0.10   1.98   0.075
RSF only   NA     NA     1.02   0.21   2.01   0.122

n=8
SCR only 200.85  14.06   1.00   0.09   2.00   0.087
SCR/RSF  200.29  13.98   1.00   0.08   2.00   0.061
sbar     196.46  14.59   0.90   0.13   1.97   0.069
RSF only   NA     NA     0.99   0.16   2.00   0.084

n=12 
SCR only 198.76  14.47   0.99   0.10   2.00   0.091
SCR/RSF  198.72  14.14   1.00   0.08   2.00   0.054
sbar     193.83  15.14   0.87   0.15   1.97   0.063
RSF only    NA     NA    1.01   0.13   2.00   0.069

n=16
SCR only 200.04  14.09   0.99   0.10   2.01   0.088
SCR/RSF  200.25  13.40   1.00   0.07   2.00   0.047
sbar     194.38  14.26   0.85   0.17   1.96   0.059
RSF only   NA     NA     1.00   0.11   2.00   0.055


To check misspecification with isotropic h/r model

 
n=2

SCR only 199.11  14.28   0.99   0.09   2.00   0.08986493
SCR/RSF  199.11  13.80   0.99   0.09   2.00   0.07908368
SCR0     161.48  39.98    --     --    1.84   0.1801644
...
SCR0     160.93  40.44   1.00   0.00   1.84   0.190
...
across all 5 simulations for N=100 was 99.418, an empirical bias of
$0.6\%$. For N=200, the estimated $N$ across all 5 simulations (5
levels of ntel)  was
199.712, an empirical bias of about $0.15\%$, within the MC error of
the true value of $N=200$.
The results suggest a small persistent bias of $< 1\%$
in the MLE of $N$ in general, and as much as 2-4\% when ${\bf s}$ is
estimated by the average observed location. The results are consistent
even as we double the expected sample size by doubling $N$ from 100 to
200. 
  In general we expect a
...
The key benefit of our model, therefore, is its ability to
...
$E[n] = XXXX$ under the $N=100$ scenario.  The upshot is we have only
incremental relative improvements in RMSE of N but relatively more
improvement in RMSE for estimating $\sigma$. .... but persistent bias,
which is of opposing direction.... i.e., the SCR estimator is biased
positively when we have really sparse data which makes
sense...... Interestingly this bias opposes slightly negative bias for
the estimator based on estimating ${\bf s}$ and so that wrong
estimator actually does better. This is a complete fluke and we should
not get too excited by that. 
\begin{verbatim}
...
RSF only   NA     NA     1.02   0.30   1.99   0.163
n=4
...
\citep{efford:2004,borchers_efford:2008, royle:2008,
  efford_etal:2009ecol,royle_etal:2009ecol, gardner_etal:2010,
  gardner_etal:2011, kery_etal:2010,
  sollmann_etal:2011,mollet_etal:2012,gopalaswmany_etal:2012}. These,
and indeed, most,
...
models. First, we have shown how to integrate classical RSF data from
...
importance of data from those studies.  Thirdly, it is also now clear
...
alone. While this is of less practical relevance, it does suggest that
one might be able to estimate density with very sparse data and few
actual recaptures of individuals, by having good estimates of $\sigma$
from auxiliary data. This is an idea exploited by
\citet{chandler_royle:2012}.

Bullet point findings: Adding telemetry data:
Improvement in precision for N, but only
incrementally. Huge improvement in estimating sigma. SCR data alone
provide decent estimates of RSFs, at least simple ones considered in
our simulation. Further exploration of this point
is an area worthy of some research.
SCR0 is extremely biased under a model of resourc eselection that is
