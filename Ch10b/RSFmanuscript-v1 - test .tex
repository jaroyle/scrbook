...
Let $n(x)$ be the use frequency of pixel ${\bf x}$ for some individual
in the population.
...
\begin{equation}
 \pi({\bf x},{\bf s})  = \frac{ \exp( -\alpha_{1} D({\bf s},{\bf x})^{2} +\alpha_{2} z({\bf x}) )}
...
Note that if use frequencies are summarized over individuals for
each pixel, i.e.,
