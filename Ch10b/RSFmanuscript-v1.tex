...
data obtained from telemetry used to record movements or dispersal
events.  
On the other hand, spatial capture-recapture (SCR) models
...
proposed.  In this paper we develop a statistical framework for
integrating SCR data, such as from camera traps, mist nets, or
conventional catch-traps, with RSF data from telemetry of
individuals. We provide framework for estimation based on marginal
...
estimate parameters of resrouce selection functions and, as such, SCR
methods can be used as the sole-source for studying space-usage in
place of conventional animal telemetry. Finally, we find that SCR
models using standard symmetric and stationary encounter probability
models produce extremely biased estimates of density when animal space
usage is, in fact, related to a landscape covariate. Therefore, it is
critically important that space usage be taken into consideration, if
possible, in studies focused on estimating density in
