...
\hl{[should we use ``capture'' or ``encounter'' probability throughout?]}
capture probability is expressed as a function of
...
ill-defined area sampled and heterogeneity in capture %encounter
...
more complex models are needed to relate the encounter %detection
...
which are binomial with sample size $K$ and encounter probabilities
...
\st{This is equivalently stated in the
literature in terms of individual having  selected x but
the latter term implies some kind of conscious intent, and so we keep
to the term use as much as possible.} \hl{I kind of agree with
you, but since we use ``resource selection'' throughout, then I don't
think we can avoid the term selection.} As a biological matter,
...
\hl{I changed pixel id from j to k because j was used for traps}
...
Let $n_{ik}$ be the use frequency of pixel $k$ ($k=1,\ldots,nG$) by individual $i$ --
i.e., the number of times individual $i$ used pixel $k$
