...
framework for modeling of trap-specific covariates.  Furthermore,
essentially all real capture-recapture studies produce auxiliary
spatial information and therefore SCR models are directly relevant to
standard data collected from such studies.

Despite the inreasing popularity of SCR models, every single
application of them so far has been based on encounter probability
...
by sparse data or lack of auxiliary information about animal space
usage, the development of more complex models of the detection process
as it relates to space usage of individuals is needed.  In this paper,
we extend the encounter probability models used in SCR models to
...
between SCR and RSF studies are that, in SCR studies, encounter of
...
spatial covariate $z({\bf x})$ affecting encounter probability.  By
integrating RSF data obtained from telemetry into SCR models to model
space usage explicitly, this implies a model of asymmetric, irregular
...
The critical assumption that motivates our work here is that the Euclidean
...
\begin{comment} in manuscript we need intro here on SCR
