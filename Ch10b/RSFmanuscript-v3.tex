...
accumulate in pixels. % by some omnipotent accounting mechanism. 
We will
define ``use of ${\bf x}$'' to be the event that an individual animal
appeared in some pixel ${\bf x}$ at any instant in time.
This is equivalently stated in the
literature in terms of individual having {\it selected} ${\bf  x}$.
 As a biological matter,
use is the outcome of individuals moving around their home range \citep{aarts_etal:2008,johnson_etal:2008,forrester_etal:2009,hooten_etal:2010},
...
${\bf s}$ denote the centroid of an individual's home range and let
...
We note that this form (Eq. \ref{rsf.eq.rsf}) 
arises explicitly as a limiting form of
the Gaussian process movement model of
\citep{johnson_etal:2008}. Conceptually, it recognizes that apparent
...
\citep{johnson_etal:2008, forrester_etal:2009}.
...
This occurs when (and, in this case, because) first order selection is
{\it independent} of second order selection, which we recognize may
...
(the term related to $z({\bf x})$) \citep{forrester_etal:2009}.
...
We have ${\bf m}_{i}$, the $nG \times 1$ vector of pixel counts for individual $i$,
...
\citep{chandler_royle:2012, sollmann_etal:inprep} but, so far, not
likelihood methods.

Bayesian analysis might have an advantage in situations where
landscape is characterized by 
a very fine covariate raster, or even continuous covariates, 
 because the individual activity centers can be
...
affect on integration, raster size is itself an important practical concern..
raster size  -- as coarse as a quarter of the size of a typical home
range hardly causes error in SCR models -- it is a matter of numerical
error in doing the integration.
However when we have spatial covariates its possible that selection is
occuring at a much finer resolution ..... what to do.....
