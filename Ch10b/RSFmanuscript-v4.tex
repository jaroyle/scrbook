...
{\it all} coordinates.
%% Right: we need to say something like: We assume pixels are small
%% relative to an individual home range center, and equate pixel
%% center coordinates with potential trap locations.... then there is
%% a random sampling element along the lines of: if an individual uses
%% pixel x then whether or not it gets detected is a Bernoulli trial.
%%% but note, traps don't have to be pixels at all
...
multi-colored maps of things.  A map of density under the SCR+p(z)+RSF+D(z)
...
The SCR data is based on $n=33$ individuals, and the telemetry data
...
% fair point -- but this is generic RSF conceptual and motivation
% stuff -- not anything to do with SCR or combining SCR with RSF
...
garshelis_pelton:1981,
kendall:1983, graber_white:1983, 
...
such covariates. 
%This does raise
% one specific issue which is: why not just develop the model in
% continuous space?  I guess I don't see a point in doing that because
% then we would have to talk about the "special case" of a discrete
% covariate in order to solve any real problem, and so we might as well
% just begin with the discrete case.
