...
\begin{equation}
  \log(\mu(\mathbf{s}, {\bm \beta})) = \beta_0 + \sum_{v=1}^V \beta_v C_v(\mathbf{s})%, \quad  \mathbf{s} \in \cal{S}
...
$N$ is entirely determined by the encounter rate parameters---the
sampling error. That is,
...
\caption{\bugs~code for fitting inhomogeneous point process model in
  ...
  omitting $\beta_0$ and modeling the data augmentation parameter as
...
available in \scrbook---see \verb#help(ch11secr-jags)#.
...
\beta_1\text{PROTECT}(\mathbf{s}))$. We hypothesized that $\beta_10$
...
function. The second map is the realized density surface. This is the,
conditional on $N$, probability distribution for the number of
