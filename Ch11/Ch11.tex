...
This model states that $N$, the number of realized points in $\mathcal{S}$ is
...
population density. Other important charactheristics
of the homogenous Poisson point
process are that the points neither repel or attract
...
as is illustrated by this simple \R~code:
\begin{verbatim}
Area <- 1                          # Area of unit square
muP <- 4                           # intensity
nP <- rpois(1, muP*Area)           # nP is random
...
One distinction between the Poisson and binomial models is that if we divide the
state-space into $K$ disjunct regions, the number of points in each
region $n(B_k): k=1,\dots,K$; are
independent and identically distributed (i.i.d.) under the Poisson
model but not under the binomial model. That is, some dependence
exists under the binomial model such that the number of points in one
region is somewhat informative about the number of points in another
region.
...
point falling in each of the 25 disjunct regions is $\pi(B_k) = 1/25$ and
the expected counts are $\mathbb{E}(n(B_k)) = N\pi(B_k)$.
In the figure $N=50$, and consequently we would expect 2 points per pixel, which
happens to be the empirical mean in this instance. However, these counts are not
...
In SCR models that assume a homogeneous Poisson point process, the intensity
parameter $\mu$ is population density, and it can be estimated directly
using maximum likelihood \citep{borchers_efford:2008}. Once $\mu$ is
estimated, as estimator of the expected value of $N$ is $\hat{N}=\hat{\mu}|\mathcal{S}|$. under the Poisson assumption and by estimating $\mu$ we can
determine the expected number of activity centers in any region.
...
in the state-space is $\mathbff{E}[N] = \mu|\mathcal{S}|$ for a
