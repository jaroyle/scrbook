...
The intensity parameter can be used to compute the expected number of points
in any region $B$ of the state-space $\cal{S}$. Specifically,
$\mathbb{E}[n(B)] = A(B)\mu$ where $A(B)$ is the area of region $B$.
This just says that
the expected number of points is simply the area of $B$
...
binomial point process is that $N$ is a random variable in former but not in
...
but not under the binomial model.
%In the Poisson case
...
%$B_k$.
Fig.~\ref{ch9.fig.homo} illustrates this point.
