...
independent realizations from a binomial distribution since $\sum_k %{k=1}^K
...
%  with because the multinomial distribution describes the
%probability that an activity center occurs in one of the $G$ pixels.
%${\bf s} \sim
%\mbox{Unif}(\cal{S})$ to ${\bf s} \sim \mbox{Categorical}(1, \mathbf{\pi})$. In the
...
In the context of spatial capture-recapture models, if one assumes an
...
%N/A(\mathcal{S}) =  \psi M / A(\mathcal{S})$
...
$\beta_0 = N/|\mathcal{S}|$. %, which is to say that the intercept is
...
make predictions from our fitted model, it is necessary to retain
$\beta_0$ in the model.
