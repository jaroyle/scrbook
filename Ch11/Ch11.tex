...
In SCR models, the intensity
parameter $\mu$ is population density, and $N$ is
population size, or strictly-speaking, the number of activity centers in
$\mathcal{S}$. Having now gone over the Poisson and binomial point
processes, one might legitimately ask if it is better to regard $N$ as
random or fixed.
$\mu$ or , and it can be estimated directly
