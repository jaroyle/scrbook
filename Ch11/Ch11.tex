...
%of points in some infinitesimally small area. In other words, it
...
from Poisson and binomial point processes in the unit square ($\mathcal{S} = [0,1]\times[0,1]$):
...
distribution. %Such a change will result in very little loss
%of information as long as the pixel resolution is fine enough, which
%can be assessed by altering the resolution as demonstrated in
...
% discrete space. So although Eq.~\ref{state-space.eq.EN} looks nice and
% makes us feel great about using calculus, we will tend to favor
% the summation in practice.
...
%, which is consistent with the fact that the intensity parameter is a
% constant.
...
\begin{equation}
[\mathbf{s}_i | \bm{\beta}] = \frac{\mu(\mathbf{s}_i, \bm{\beta})}{\int_{\mathbf{s} \in \mathcal{S}} \mu(\mathbf{s}, \bm{\beta})\, \mathrm{d}s}
...
all pixels,
\begin{equation}
[\mathbf{s}_i | \bm{\beta}] = \frac{\mu(\mathbf{s}_i, \bm{\beta})}{\sum_{\mathbf{s} \in \mathcal{S}} \mu(\mathbf{s}, \bm{\beta})}%\, \mathrm{d}s}
...
be implemented using maximum likelihood or MCMC; however, even though such
...
\begin{equation}
\psi = \frac{\int_{\mathbf{s} \in \mathcal{S}} \mu(\mathbf{s},
  \bm{\beta}) \, \mathrm{d}\mathrm{s}}{M}
...
If we refer to the distribution $[\mathbf{s} | \bm{\beta}]$ as
...
elevation, and $\beta_0=5$ and
$\beta_1=2$. Simulating data from an inhomogeneous point process is
not as simple as simulation data from a homogeneous point process
...
the {\tt R2Cuba} package to compute the two-dimensional integral
\citep{hahn_etal:2011}. An alternative would be to evaluate the
...
which are very close to the data generating values. We could plug
these estimate into our linear model at each point in the state-space to
...
parameters governing the underlying point process. However, in
SCR models, the activity centers cannot be directly observed---yet,
...
X <- cbind(rep(xsp, each=len),
           rep(xsp, times=len)) # trap locations
ntraps <- nrow(X)
K <- 5                          # occasions
y <- array(NA, c(N, ntraps, K)) # capture data

nz <- 80 # data augmentation
M <- nz+nrow(y)
...
augmented it to create the new data object \texttt{yz}, we are ready to
...
tool for fitting SCR models using MCMC---it is primarily presented so
...
trace plots of the Markov chains and the posterior distributions are
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{Ch11/figs/fm1p}
...
prepare the data for \secr. The results are presented in
Table~\ref{state-space.tab.simIPP}, and we see that the point
...
\begin{table}%[b]
\centering
\caption{Comparison of parameter estimates from SCR model with
  inhomogeneous point process. Estimates are posterior means from MCMC
analysis and maximum likelihood estimates from analysis using
\texttt{secr}. Note that the MCMC analysis assumed a binomial prior
for $N$ whereas a Poisson prior was assumed for the \texttt{secr} analysis.}
\begin{tabular}{lrrrrr}
\hline
Parameter 	& Method 	& Estimate 	& SD/SE 	& Lower 	& Upper \\
\hline
 $\sigma=5$ 	& MCMC 	&  5.232 	&  0.310 	&  4.681 	&  5.858 \\
  	        & ML 	&  5.237 	&  0.303 	&  4.676 	&  5.866 \\
 $\lambda_0=1$ 	& MCMC 	&  0.802 	&  0.119 	&  0.595 	&  1.049 \\
  	        & ML 	&  0.781 	&  0.115 	&  0.586 	&  1.041 \\
% $\beta_1=1$ 	& MCMC 	&  0.985 	&  0.209 	&  0.575 	&  1.378 \\
%  	        & ML 	&  1.037 	&  0.274 	&  0.501 	&  1.574 \\
 $N=41$ 	& MCMC 	& 47.615 	&  8.041 	& 35.000 	& 66.000 \\
  	        & ML 	& 47.778 	&  7.744 	& 37.074 	& 68.830 \\
 $\mathbb{E}[N]=39.9$ 	& MCMC 	& 47.551 	& 10.992 	& 29.837 	& 71.332 \\
  	        & ML 	& 47.778 	& 10.380 	& 31.363 	& 72.784 \\
\hline
\end{tabular}
...
sigma ~ dunif(0, 10)
lam0 ~ dunif(0, 5)
beta0 ~ dnorm(0, 0.01) #log(D) # D=density defined below
beta1 ~ dnorm(0, 0.01)
for(j in 1:nPix) {
