...
g(x, {\bold \theta})$, and interest lies in estimating the parameters
$\bold \beta$ and $\bold \theta$.
...
to the rastser data as a ``habitat mask''. \R~code to
...
roads or trails. Forty-five detections of 16 jaguars were made over a 95
...
jaguar density. \R~code to fit the model is available in
\scrbook. Parameter estimates are shown in Table\ref{ch9.tab.jagposts}. Our results
...
this chapter and in Chapter\ref{chapt.ecoldist}. Indeed, anecdotal
evidence suggests that female jaguars in Argentina are less apt to
leave the national park than are males, so we could possibly improve
upon our analysis by defining the encounter process in terms of
ecological distance instead of simple Euclidean distance. As an aside,
it could prove even more important to account for the fact that
jaguars prefer to move along roads and trails rather than through
dense forest. This is another factor affecting how the animals percieve
distance, and including it in the model should yield a more robust
density estimate.
