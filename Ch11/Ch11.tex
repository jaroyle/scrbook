...
To be concise we will subsequently drop the vector of cofficients from our
...
\lambda_{ij}$. If we refer to the distribution for the
inhomogeneous point process as ``IPP'', we can now write a
simple description of a SCR model with a Poisson encounter process and
...
point process model and an inhomogeneous point process model.
...
centers. To estimate the intensity surface $\mu(x, \beta)$ underlying these points, we
need to derive the likelihood for our data under this model. Given the
probability density function $f(x)$ (Eq.~\ref{eq.pdf.ipp}) and assuming that the points are
