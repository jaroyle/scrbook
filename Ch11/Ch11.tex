...
\begin{equation}
\mathbb{E}[N] = \int_{\mathbf{s} \in \mathcal{S}} \mu(\mathbf{s}, {\bm \beta}) d\mathbf{s}
...
parameter estimates among varying levels of resolution, as we
...
reviewed in Chapt.~\ref{chapt.mle}. Thus, here we focus on the binomial
...
\begin{equation}
\psi = \frac{\int_{\mathbf{s} \in \mathcal{S}} \mu(\mathbf{s}, \bm{\beta})}{M}
\label{state-space.eq.psimu}
\end{equation}
As before, the data augmentation limit $M$ must be high enough so that
it is greater than the numberator shown above, i.e. the expected value
...
In SCR models, the points (e.g. activity centers) are not directly
...
later embed in our MCMC algorithm used to fit SCR models with IPP.
...
elevation and we prescribe the values $\beta_0=5$ and
$\beta_1=2$. Simulating such data can be
accomplished using rejection sampling, which involves
