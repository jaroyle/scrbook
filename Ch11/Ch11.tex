...
Suppose we knew the locations of $N=100$ animals' activity
...
model. If we assume $N$ is Poisson distributed and the points are
mutually independent of one another, we can fit the
inhomogeneous Poisson point process model whose likelihood is simply
the product of $N$ densities given by
Eq.~\ref{state-space.eq.pdf.hetero}. The log-likelihood is thus:
\[
\ell({\bm \beta} | \{{\bf s}_i\}) = \sum_{i=1}^N
\log(\mu(\mathbf{s}_i, \bm{\beta})) - \int_{\mathbf{s} \in
  \mathcal{S}} \log(\mu(\mathbf{s}, \bm{\beta} d\mathrm{s}))
\]
...
elevation. Simulating data under an inhomogeneous
point process model is often accomplished using indirect methods such
as rejection sampling. Rejection sampling proceeds by
