...
Poisson observation model, which could be appropriate in camera
...
\label{ch9.panel1}
\end{panel}
The vector \verb+probs[]+ is the prior probability defined
...
\begin{table}[h!]
\centering
\caption{Comparision of \secr~and \jags~results. Point estimates from
...
formatted using the {\tt raster} package. The linear model for the
...
fm <- scrDED(y, traplocs=X, den.formula=~elev, dist.formula=~elev,
             rasters=elev.raster)
\end{verbatim}
To assess the possiblity of estimating both $\beta$ and $\theta$, we
conducated a small simulation study, generating 500 datasets from the
...
jaguars very rarely forge the waterway.
