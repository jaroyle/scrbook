...
centers (${\bf s}$) within the state space ($\cal{S}$), which is
...
explicitly and so cannot be used to make general predictions about
habitat-specific density. A more direct approach would be to replace
the homogeneous model with an inhomogeneous model in the point process
intensity, i.e. density, varies spatially.
...
the binomial model that we use in our SCR models. An important
descriptor of any homogeneous point process model is the
intensity parameter $\mu(x)$ which describes the expected number
% start with \mu(x)???? Or, wait for inhomogeneous case?
of points in an infinitesimally small area. In the homogeneous case,
$\mu(x)$ is a single value, i.e. a scalar that does not vary
spatially. Specifically, the intensity parameter is just the inverse
of the area of the state-space: $\mu(x) = 1 / A(\mathcal{S})$.
Given the intensity parameter, the expected number of points
in any region $B$ of the state-space $\cal{S}$ is given by
...
independent and identically distributed, (i.i.d.). This is one
...
$N$. The binomial model conditions on $N$, \textit{i.e.}, $N$ is
fixed; whereas under the Poisson model $N$ is random. Here is some
simple \R~code to illustrate this point.
\begin{verbatim}
mu <- 4                            # intensity
...
\end{verbatim}
This code generates realizations from Poisson and binomial point
...
is ${\bf n(B)} \sim \mbox{Multin}(N, {\bm{\pi}} = \{p_1, p_2, \dots,
...
what follows, we will often abbreviate the intensity function as $\mu(x)$ by
dropping the vector of coefficients. Since an intensity must be strictly
...
traps on the map shown in Fig.~\ref{ch9.fig.hetero}. We will then
