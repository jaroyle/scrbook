...
point processes is not only helpful for understanding their
properties, it is also of practical importance when fitting SCR models
because spatial covariates are almost always represented in as
rasters. In such cases, we can change the the definition of the prior for
activity centers from the probability that a point occurs at some
point in space to the probability that it occurs in some pixel
