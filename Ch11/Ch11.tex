...
$\bm \beta$ and obtain the posterior distribution of
$\bm \beta$ using Bayesian methods, or we can find the maximum likelihood
...
not as simple as simulation data from an homogeneous point process
...
the (scaled) probability density function $[\mathbf{s},
\bm{\beta}]$. The following \R~commands demonstrate the use of
...
\begin{samepage}
  \begin{small}
\begin{verbatim}
# spatial covariate (with mean 0). Let's call it elevation.
elev.fn1 <- function(s) x[1]+x[2]-1
# intensity function
...
while(counter <= N) {
  ...
  s.cand <- c(x.c,y.c) # Candidate point
  pr <- mu(s.cand, beta0, beta1) / EN
  if(runif(1) < pr/Q) { # Keep the candidate if TRUE
    s[counter,] <- s.cand
    counter <- counter+1
    }
  }
...
\caption{An example of a spatial covariate, say elevation, and a
  ...
  \mbox{ELEV}(\mathbf{s}))$ where $\beta_0=5$ and $\beta_1=2$.}
\end{figure}

The 217 simulated points are shown in
Fig~\ref{state-space.fig.hetero}. High elevations
are represented by light green and low elevations by dark green. The
density of points in apparently higher in the dark green regions
...
obtained estimates of $\hat{\beta}_0=5.08$ and $\hat{\beta}_1=1.92$,
which are very close to the data generating values of 5 and 2. We could plug
...
SCR models, the activity centers cannot be directly observed; however,
spatial recaptures provide us with the information needed to
