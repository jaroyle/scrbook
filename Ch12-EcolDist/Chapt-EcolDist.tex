...
(which can be run using the {\bf R} script \mbox{tt SCRed} -- see the
help file for that):
\begin{verbatim}
> library(raster)
> library(gdistance)
...
modifies this object depending on the projecting of the coordinate
...
is given by:
\[
 \log(\mbox{cost}({\bf x},{\bf x}'))=  \alpha_2 \frac{C({\bf x}) + C({\bf x}')}{2}
\]
...
\begin{table}
\caption{
Summary output of fitting euclidean and lcp distance
  models..................
...
least-cost path (truth) & 70.119& -1.780 &  2.471    &    4.459 & 0.046  \\
\end{tabular}
