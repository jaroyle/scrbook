...
% XXXX It would be great to get comments from Nichols. He told me he
% was looking forward to reading this chapter XXXX

\section{Introduction}

% XXXX First paragraph is great, although it could possibly be split
% in two b/c it has a lot of info in it XXXX
...
However, a thorough
understanding of population dynamics %also
...
% XXXX Bill Link has papers on this. As does Hal Caswell XXXX
Inference about density-dependent population
regulation, which has fascinated theoretical ecologists for well over
a century,
% XXXX Cite early papers on pop growth, and some new ones by people
% like Dennis and DeValpine
...
rates is just as important % XXXX as what? XXXX
for applied ecologists and conservation
biologists, because a mechanistic understanding of population decline
requires it. Furthermore, if we know how environmental
...
% XXXX I suggest making the third paragraph the second. i.e. Briefly
% state what the traditional, non-spatial approaches are, and then
% explain the advantages of spatial models. XXXX
...
% XXXX Suggesting cutting back on the words: "we", "us", and "our". XXXX
...
wildcat study that was presented in chapter \hl{XXX insert ref XXX} was
...
% XXXX Change section title to Brief Overview... or else add more
% material XXXX
...
% XXXX I don't think the balance sheet analogy helps too much. But I
% like beginning with the BIDE model. It would be nice to refer back
% to it throughout the chapter. XXXX
In this case, we can think of
% XXXX "we can think of" makes this sound rather abstract. I would present
% BIDE as reality and state that if we really want a mechanistic
% understanding of population dynamics, we need to understand these 4
% fundamental processes XXXX
population size as a function
of credits (i.e., births and immigrants) and debits (i.e., deaths and
emigrants).  We can then set up the population at time $t+1$ as a
% XXXX Again, "we can then set up the population" sounds as though
% BIDE is just some abstract model, rather than a fundamental reality.  XXXX
...
equation model is %then termed
known as
the ``BIDE model''.  We can easily derive
...
% XXXX Yes, but note that so much info is lost when BIDE is collapsed
% to r, the intrinstic rate of increase. You might also note that
% lambda = exp(r) is often called the finite rate of increase, and
% Nichols and others have done a bunch of work estimating lambda from
% CR data, especially using the Pradell (reverse-time) model.

In closed population models, we focus on estimating $N(t)$,
% XXXX I would say that in closed populatoin models, we estimate N,
% not N(t), because we assume N does not change over time. XXX
...
``debits'' % XXXX suggest removing credit/debit analogy XXXX
that drive the population changes.
If we take the basic parameters in the BIDE model and reconceptualize them,
% XXXX Suggest saying that B and D can be estimated directly using the
% JS model in the absence of movement. Or, D can be estimated directly
% using the CJS model in the absence of movment. Then say that
% movement complicates things and makes it harder to distinguish death
% from permanent emigration and birth from immigration. But then cite
% nichols_pollock:1990 who discussed how to estimate both B and I
% using the robust design XXXX
we can relate these to the commonly used parameters in JS and CJS models, described in more detail below.
For example, survival ($\phi(t)$) is defined as the probability of an individual surviving from time $t$ to $t+1$, and often we call this `\textit{apparent} survival' because deaths and emigration cannot be separated. Mortality, the probability of dying from time $t$ to $t+1$, is $1-\phi(t)$.
...
% XXXX Great section, but I think you should seperate the two subjects
% you discuss. I would begin with your second point, that movement can
% make it difficult to distinguish
% survival from emigration. This point fits in well with the previous
% paragraph. Then you could discuss the more advanced topic that
% movement can also affect survival. This is a huge topic and we
% should cite some of the key papers, many of which are refereneced in
% the multistate CR papers. There is also the book "Dispersal" by
% Clobert, Danchin, Dhondt, and Nichols. We should cite that too. XXXX

%We commonly consider that
Density may influence demographic parameters such as % XXXX Perhaps save this for later XXXX
...
\hl{NONE OF THESE ARE IN BIB FILE: %\citep{
kendalletal:1997, pradeletal:1997, hinesetal:2003, claveletal:2008}.  Because we estimate movement
...
the SCR model.
% XXXX Would be good to disucss effects of temporary emigration too,
% not just transients XXXX
...
% XXXX How does pop sturcture affect pop size? You should add lots of
% citations to back this stuff up. XXXX
We also know that how animals associate themselves % XXXX distribute themselves?
in space
...
For example, if habitat is limited, %and we have
%a territorial
% species, then
...
%We also understand that there are impacts on the population structure from animals
%dispersing.
Dispersal can also affect population structure.
In many
animal populations, dispersal is linked with reproduction and
population regulation.
% XXXX Add citations (including some of Ovaskainen's stuff), and be clear how dispersal is linked to population
% regulation. These are huge topics that are hard to skim over. XXXX
Thus, movement including spatial
arrangement of activity centers % XXXX Is arrangment of activity centers movement?
...
% XXXX Suggest sticking to the theme that traditional non-spatial
% models have problems that can be remided by SCR models XXXX
...
% XXXX Again, suggest avoiding "we" when you can just say: "the JS
% model does not condition... " XXXX
...
% XXXX Suggest begining with the objective of the Jolly-Seber model,
% and point out some key references. Then you could present the
% simplest (or original) formulation before segueing into the
% alternative parameterizatoins. At this point, I don't think the
% reader has a grasp of the basic model. XXXX
There are a number of ways that researchers have %used to
formulated the JS model and while
all %of these models
are slightly different, %parameterizations of the underlying population processes,
%they are all essentially equivalent.  T
...
Link-Barker, Pradel-recruitment, Burham JS, and the Pradel-l models.
% XXXX Add citations XXXX
...
under the JS model.
% XXXX Why bring up the robust design before the basic JS model has
% been described? XXXX
...
%Typically,
The standard JS model %sampling framework would
...
population chapters are %equivalent to
called {\it secondary} sampling occasions. %periods).

% XXXX Can you increase the font size? XXXX
\begin{figure}
\centering
\includegraphics[height=2in,width=2.5in]{Ch12-Open/figs/RobustDesign.pdf}
...
conditional on $z(i,t)$,
% XXXX Remind people what z is first XXXX
as:
 \[
  y_{ikt}|z(i,t) \sim
...
% XXXX Next two citations are missing XXXX
...
to be implemented directly in the \textbf{JAGS} software \cite {royle:book}.
...
models. % XXXX OK, but you brought up DA earlier XXXX


\subsection{Data Augmentation for the Jolly-Seber Model}
% XXXX Make sure that you say that this is the formulation advanced by
% royle_dorazio:2008, the book XXXX
...
during the study) %at least 1 time period)
...
model. % XXXX remind people what gamma is XXXX

Data augmentation formally reparameterizes the model, replacing $N$,
the number of individuals ever alive with the parameter $\psi$
% XXXX I think it would be more correct to say that N is replaced with
% M, and we assume N~Bin(M, psi), which is the same as a discrete
% uniform prior on N when the prior for psi is Unif(0,1).
which
...
related to the population size.  This is easily resolved
% XXXX Not sure that "resolved" is right word here XXXX
by deriving
...
variables $r(i,t)$
% XXXX since r is usually growth rate, something like A(i,t) might
% better, as in RD book XXXX
...
% XXXX Remind people what step() does. XXXX
...
% XXXX Suggest adding comments to this to explain what the key parameters
% are. Also, some of the notation seems to be different that what is
% in the text. XXXX
\begin{panel}[htp]
\centering
...
30 m apart on the perimeter of a 600-m x 100-m rectangle \hl{(see
...
of ``multi-season'' % XXXX or "multi-session"? XXXX
model where abundance in each year, $N_{t}$, was estimated
separately. This %of course is a first step in thinking about
...
% now, we can use the JS model to
...
we do this by using the \mbox{\tt unique}() function % command
on the row names
...
% XXXX Need to say something like: "here comes some R code".
% And actually, since this is rather long, I think it might be better
% to put it in the R package and just reference it here. XXXX
{\small
\begin{verbatim}
library("secr")
library(scrbook)
...
% XXXX Can you elaborate? Why isn't it needed in SCR models? XXXX
...
determine that area in an ad hoc manner.
% XXXX do we italicize ad hoc?  XXXX
Not making use of the
spatial information in the data makes %arrangement of the trap array renders
the estimation of density %to
...
impact of movement.
% XXXX Suggest deleting this paragraph. It contradicts the earlier
% statements that density can't be estimated using standard JS, and
% people do use multi-state models to get at movement


\section{Spatial Jolly-Seber Models}
% XXXX This section does not seem to have anything about modeling the
% activity center over time. Shouldn't s(i,t) be a function of
% s(i,t-1)? I think we need to add something about this or at least
% justify the "independence" assumption that appears in your BUGS
% model below. I guess in my mind, one of the major reasons for doing
% the spatial analysis is to model movement among years XXXX
...
% XXXX Sometimes Bernoulli, sometimes Bern
\[
  y_{ijkt}|z(i,t) \sim
...
not alive ($z(i,t)=0$), then the observations must be fixed zeros with
probability 1.  We can of course %let this encounter model not only be
                                %Bernoulli, but
consider other encounter models such as the
%also
Poisson or multinomial models described %as we saw
...
and then %the
model %describing
the transition of individual states from
$t$ to $t+1$ as: % is:
\[
 z(i,t+1) \sim \mbox{Bern}( \phi_t z(i,t)  + \gamma_t (1-z(i,t)) ).
\]
...
we did not make use of the spatial location for each net the ovenbirdswere captured in.
...
% XXXX Suggest putting BUGS code in panel and adding comments  XXXXX
...
for(i in 1:M){
  ...
  for(t in 1:5){
  S[i,1,t] ~ dunif(xlim[1],xlim[2]) # XXXX This needs to be justified XXXX
...
% XXXX Suggest avoiding sci numbers by scaling D to indivduals per
% 100km2 or something XXXX
\begin{table}
\centering
\caption{
...
R2    &  NA &   1.5e+01 & NA \\
R3    &   NA &   1.9e+01 & NA \\
R4    &   NA &   8.3e+00 & NA \\  % XXXX How can this be a fraction?
...
% XXXX Font is too small.  x-axis label should be changed because
% "hat" usually refers to a MLE XXXX
\begin{figure}
\centering
\includegraphics[height=2in,width=2.5in]{Ch12-Open/figs/Nhat5ovenbird.pdf}
...
% XXXX Finish with some conclusions about the spatial model. What did
% it get us in terms of the analysis of the ovenbird data. Did it
% resolve all the problems listed in the intro? Could we seperate
% survival from emigration? XXXX
...
\]
% XXXX Should p have an i subscript ? XXXX
...
\[
 z_{i f_i} = 1
\]
% XXXXX Need to clearly define f_i
...
% XXXX Suggest beginning with the case where permanent emigration
% isn't possible, so phi=survival
...
\begin{figure}
\centering
\includegraphics[height=2in,width=4.43in]{Ch12-Open/figs/American_Shad_Raabe.jpg}
...
% XXXX Is every fish that enters the river caught? XXXX
...
% XXXX This sounds like p should be 1 XXXX
...
% XXXX citations? It's hard for me to imagine that they are dying due
% to the energetic costs of swimming upstream. What about fishing? Are
% people catching them?
Additionally, the CJS model is only estimating apparent survival
and some fish may have left the stream temporally or permanently.
% XXXX Where would they go? Are there tributaries that aren't being monitored?
...
we do $p$.
As we move into the multi-state model, % XXXX What are the states? XXXX
...
% XXXX Is there a multi-state JS too? XXXX
...
% XXXX Are you referring explicitly to state=location here?
...
where the ``home area'' ${\bf s}$ becomes the ``activity center" for
each individual. % XXXX Perhaps "area of activity" instaed of "home area"
...
% XXXX I think we should seperate
\begin{panel}[htp]
\centering
\rule[0.1in]{\textwidth}{.03in}
%\begin{minipage}{2.5in}
...
% XXXX At some point, we should note that it is hard to fit
% multi-state CR models when some of the states are hidden. Sarah C
% has papers on this I think? i.e. when animals are in some region not
% exposed to sampling, it makes everything difficult, and it is a good
% reason for using SCR
...
% XXXX Don't shad die after spawing?
...
% XXXX Can you say something about how to handle mortality at capture?
% A common issue in standard CJS models
...
based under the Binomial encounter model.
% XXXX Explain why you used the Poisson model XXX
The
...
% XXXX Add some examples of species that show site fidelity. For
% example, many migratory passerines, like the ovenbird, return to the
% same location, or nearly so.
...
% XXXX This stuff is very cool, and it would be great to show an
% example of an Ovaskainen model, or at least some BUGS code.

% XXXX It would also be good to cite some recent movement papers by
% McClintock and Johnson

%This is just the beginning of modeling
%the spatial arrangement of individuals within a study, we can also
%look at  % XXXX This is an old field I would say
%Other point process models
%such as Strauss and clustering processes.
...
of fish, % What do you mean by "overall movement of fish"?
then we could allow the variation in locations to be a function of flow. This means that we assume the
...
function of habitat variables. %incorporate habitat information directly into
%the models of movement or correlation of movement.
...
% XXXX I think this book is the place to do this.
...
fragmentation, etc.. % XXXX Trailing end
