...
parameterization of the CJS models in which the observation model is
...
be 1 when $t$ is the first primary period of capture.  We can denote this $z(i, first_i)$ where $first_i$ indicates the primary occasion in which individual $i$ is first captured.  This ensures that each individual is alive upon entering the model.   Modeling time-effects either
...
\]
Thus, because we start with $z(i, first_i) = 1$, the individual survives with probability
$\phi$ to time $first_i + 1$ and so forth.  Once an individual leaves the
