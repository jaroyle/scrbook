...
\hl{I'm trying to expand this first paragraph to broaden the scope of
  the chapter.}
...
spatial and temporal variation in population density. In this chapter, we develop a framework for
inference about the processes governing spatial and temporal dynamics,
namely survival, fecundity, and movement (migration, dispersal, etc...).
%In many cases
%. data from spatial capture recapture studies for
...
modeling time series data \citep{nichols_etal:2002}.  Modeling vital
...
\hl{rbc: I think it would be helpful to define and discuss the different
parameters that affect population size. Even something as basic as the
BIDE model, N(t) = N(t-1) + B + I - D - E, would help set the stage
for the next sections where we talk about how to estimate these
parameters. }
