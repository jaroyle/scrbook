...
Pollock (1982) created the robust design in order to allow for heterogeniety in capture probability
under the JS model.  The basic idea is that there are primary occassions (e.g., years, seasons) and we 
allow the population to be "open" between the primary occasions.  This means that individuals can enter
and leave the population (i.e., births, deaths, immigration, emigration) between the primary occassions.  
...
occassion (we call these ``secondary occassions"), then variation in detection probabilty can be modeled and thus
...
Based on the robust design, we can easily create a non-spatial JS model in {\bf JAGS}.  
...
XXXXMaybe make model into a panel?
{\small
\begin{verbatim}
cat("
model {
