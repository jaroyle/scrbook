...
XXXXMaybe make model into a panel?XXXXXX

Implementation of the non-spatial JS model in the \jags. 
{\small
...
In Chapt. 5, we dealt with this dataset as a type of ``multi-season"
...
Chapt. \ref{  we carried out data augmentation for each year separately;
...
hold<- unique(c(unlist(dimnames(Y[[1]])[1]), unlist(dimnames(Y[[2]])[1]), 
       unlist(dimnames(Y[[3]])[1]), unlist(dimnames(Y[[4]])[1]), unlist(dimnames(Y[[5]])[1])))
...
Stmp[pmatch(unlist(dimnames(Y[[i]])[1]), hold),1:2]<-
        spiderplot(tmp2[pmatch(unlist(dimnames(Y[[i]])[1]), hold),1:nrep],as.matrix(X[[i]]))$avg.s   #$
Sst[,,i]<-Stmp
Yarr[,,i]<-tmp2
}
...
print(summary(out2NSJS), digits = 2)
...
N[1]     26.518 3.184  0.01300        0.02415
N[2]     30.207 2.713  0.01108        0.01932
N[3]     33.070 2.955  0.01206        0.02203
N[4]     29.483 3.270  0.01335        0.02955
N[5]     21.743 2.750  0.01123        0.01855
gamma[1]  0.500 0.288  0.00118        0.00125
gamma[2]  0.086 0.027  0.00011        0.00021
gamma[3]  0.109 0.030  0.00012        0.00022
gamma[4]  0.129 0.035  0.00014        0.00028
gamma[5]  0.074 0.030  0.00012        0.00024
p.mean    0.137 0.012  0.00005        0.00011
phi       0.484 0.055  0.00023        0.00044
psi       0.136 0.029  0.00012        0.00019
...
Also, we can potentially model the relationship between density and the 
...
of $z(i,t)$ to $z(i, t+1)$ which is the same as in the non-spatial JS model.  To do so, we describe the Bernoulli observation model,
...
XXX do I need to remind the reader of more about the close SCR models? XXX
...
were captured in.   However, there 44 mist nets that each bird could be capture in during
any of the sampling occasions.  We already organized the data above so that our 3-D encounter
...
\centering
\caption{
Posterior mean of model parameters for the spatial JS model and the multi-season model fitted to the 
ovenbird data set.
}
\begin{tabular}{crr}
\hline \hline 
&           JS   &   Multi-seaon \\  \hline
D[1]    &   9.6e-05 &  9.3e-05   \\
D[2]     &  1.0e-04 &  1.0e-04  \\
D[3]   &    1.1e-04 &  1.2e-04  \\
D[4]   &    1.1e-04 &  8.9e-05  \\
...
R5    &     8.3e+00 & NA \\ 
phi   &     5.3e-01 & NA \\ 
psi   &     1.7e-01 & NA \\ \hline
\end{tabular}
\label{open.tab.JSmulti}
\end{table}

Our results for density, alpha0, and alpha1 are rather similar to those found in the multi-season analysis from Chapt. \ref{chapt.poisson-mn}.
...
There are some slight variations in the parameter estimates, such as the density in the multi-season model is lower
...
In the JS formulation of the model, we also estimate the
 recruitment for each year, and we can look at our derived values for recruitment (R2, R3, R4, and R5).  
...
it should be easy to see that we could allow survival to vary by time, as we did with recruitment.
 These results for survival seem pretty
...
XXX compare to non-spatial results  detection, N.  comment again on how N is not associated with an area
in the non-spatial JS, and thus it's not clear how we compare the two.
...
\hl{rbc comment: I would first describe the non-spatial versions of
  the model first. Describe each part and then explain the weaknesses
  associated with ignoring space.}
...
a North Carolina river using an American shad \textit{Alosa sapidissima}
...
\[
 \sigma_t = \mu_S + \alpha_2 \mbox{Flow}_t
\]  
...
bearraw=read.csv("caphist.csv")
bearraw$count=1
a=which(bearraw$snare == "HS46")
bearraw=bearraw[-a, ]

junk.melt=melt(bearraw,id.var=c("bear.ID", "snare", "year", "session"), measure.var="count")
bearArray=cast(junk.melt, bear.ID~snare~year, sum)

locs=read.csv("locs.csv")
locs2=locs[order(locs[,1]),]
X<-cbind(locs2[,2:3]/1000)

xlim<- c(min(X[,1] - mean(X[,1]))-20, max(X[,1] - mean(X[,1]))+20)
ylim<-c(min(X[,2] - mean(X[,2]))-20, max(X[,2] - mean(X[,2]))+20)
...
T<-3
n<-dim(bearArray)[1]
M<-400 # do constant data augmentation to all years

# flesh out the missing traps and created augmented dataset.
Yarr<- array(0, dim=c(M,ntraps,T))
a=pmatch(row.names(bearArray[1,,]), locs2[,1])
Yarr[1:n,a,] <- bearArray[,,]
Yarr2<-Yarr

K<-4
Sst<-cbind(runif(M,xlim[1],xlim[2]),runif(M,ylim[1],ylim[2]))
Sst<-array(Sst,dim=c(M,2,T))

#for(i in 1:T){
#Stmp<-Sst[,,i]
#Stmp[1:n,1:2]<-spiderplot(Yarr[1:n,,i], as.matrix(X))$avg.s
#Sst[,,i]<-Stmp
#Sst[Sst == "NaN"] <- 0
...
for(i in 1:n){
for(t in 1:T){
Stmp<-colSums(Yarr[i,,t]*X)/sum(Yarr[i,,t])
Sst[i,,t]<-Stmp
}
}
Sst[Sst == "NaN"] <- 0
...
for (i in 1:M){            #loop over M individuals (includes the augmented data)
   ...
   for(j in 1:J) {    #loop over all traps
       for(t in 1:T){
           ...
           D2[i,j,t] <- pow(S[i,1,t]-X[j,1], 2) + pow(S[i,2,t]-X[j,2],2)
            g[i,j,t] <- lam0*exp(-D2[i,j,t]/sigma2)
            pmean[i,j,t] <- 1- exp(-g[i,j,t])
          tmp[i,j,t] <- z[i,t]*pmean[i,j,t]
            y[i,j,t] ~ dbin(tmp[i,j,t], 13)
        }
     }
...
a[i,1]<-(1-z[i,1])
  ...
  for(t in 2:T){
       S[i,1,t] ~ dnorm(S[i,1,t-1], tauX)
       S[i,2,t] ~ dnorm(S[i,2,t-1], tauY)
...
zst<-cbind(zst,zst,zst)

inits <- function(){list (z=zst,sigma=runif(1,1,9), gamma=runif(T,0,1) ,S=Sst ) }

parameters <- c("sigma","N", "phi", "gamma", "R2", "R3", "sigX", "sigY")

data <- list (X=as.matrix(X),y=Yarr2,M=M,T=T,J=ntraps,ylim=ylim,xlim=xlim)


out1 <- jags.model("modelJS.txt", data, inits, n.chains=3, n.adapt=500)
out2JS <- coda.samples(out1,parameters,n.iter=1500)
