...
(Chapt. \ref{chapt.mle}),
and the RSF likelihood contribution is the multinomial telemetry
likelihood having cell probabilities 
Eq. \ref{rsf.eq.rsf}.
The {\bf R} code for this was given in \citet{royle_etal:2012mee}, and
we include a version of this 
in the \mbox{\tt scrbook} package, see \mbox{\tt ? XXXXXX}.
%cript ADD TO REPO XXXXXX... will handle estimated s by sbar
...
using 103 hair snares in June and July,
2011.  Hair snares were baited and scented and checked weekly
...
2 traps, 1 individual in each of 3 and 4 traps. {\bf XXXX this might
  be wrong XXXXX}
Data were available
...
to produce movement outcomes that might be more independent. This produced
195 telemetry locations used in the RSF component of the model.  The
covariate used in the modeling is elevation, a standardized version of
which is shown in 
...
random noise. 
{\bf XXXXXX include trap locations on this figure XXXXXXXXXXXXXXXXXXXX}
}
\label{fig.elevation}
\end{figure}


They fitted a sequence of models based on the Gaussian hazard model
including an ordinary SCR model with no
covariates or telemetry data, the SCR model with elevation affecting
either $\lambda_{0}$ or density $D({\bf x})$ (Chapt. \ref{chapt.state-space}), and models that use
...
\end{itemize}
AIC for the first 4 models can be compared 
because they are nested models. Also the last 2 can be compared with
each other, but not with the first 4, 
because those include telemetry data.
Parameter estimates 
are shown in Table \ref{tab.nyresults} (from \citet{royle_etal:2012mee}).
We provide a full {\bf R} script for fitting  all of these models to
simulated data in Appendix 1.  {\bf XXXX include this XXXXX}

{\bf xxxx this is exactly plagarized from the paper}
...
Royle et al. had a good story for this:
\begin{quote}
...
One point raised in Royle et al. is this......
