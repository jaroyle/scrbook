...
exaactly our Poisson encounter probability model from
...
for trelemetry described above but, rather, to the efficiency of the capture-recapture
encounter device. 
...
the
scale parameter of the Gaussian hazard encounter model, and 
 $\beta$ is the coefficient of elevation on density. Standard errors
 rae in parentheses.
...
$\beta$       & -loglik                                                                         \\ \hline
SCR(elev)      & -2.860    & -1.117        & 0.175       & 4.140        &        & 122.738  \\
             &  (0.390)     & (0.139)       & (0.248)       & (0.366)        &        &           \\
SCR          & -2.729    & -1.122        & ---          & 4.110        &        & 122.990  \\
              & (0.345)     & (0.140)       &              & (0.362)        &        &           \\
SCR+D(elev)      & -2.715    & -1.133        & ---          & 4.114        & 1.247 & 118.007  \\
              & (0.353)     & (0.139)       &              & (0.358)    & (0.408) &           \\
SCR(elev)+D(elev) & -2.484    & -1.157        & -0.384      & 4.255        & 1.571 & 117.075  \\
              & (0.391)     & (0.142)       & (0.276)       & (0.377)   & (0.463) &           \\
SCR(elev)+RSF       & -3.068    & -0.814        & -0.281      & 3.884        &        & 1271.739 \\
              & (0.272)    & (0.036)         & (0.118)       & (0.363)        &        &           \\
SCR(elev)+RSF+D(elev)  & -3.070    & -0.810        & -0.371      & 4.028        & 1.273 & 1266.700 \\
...
model SCR0, and the peformance of the MLEs, under SCR+telemetry
designs having 2, 4, 8, 12, and 16 telemetered individuals (with 20
independent telemetry fixes {\it per} individual). We summarize some
of the results here, but provide an {\bf R} script (see \mbox{\tt
  ?RSFsim}) that can be modified for further analysis and exploration.
They fitted 3 models: (i) the SCR only model, in which the telemetry
...
RSF only model which just used the telemetry data alone (and therefore
$\alpha_{0}$ and $N$ are not estimable parameters).  An abbreviated
version of the results from \citet{royle_etal:2012mee} is summarized
in Table \ref{rsf.tab.sims} below.

One thing we see is a pretty dramatic bias in estimating $N$ if the
normal model SCR0 is fitted (interestingly, there is much less bias in
...
estimator of $N$ when we have at least 2 telemtered individuals, and
as much as a 10\% reduction as in the $N=200$ situations. This makes
...
SCR data. Thus estimating $N$ benefits only slightly from the addition
of telemetry data.  However, there is a large improvement (50-60\%) in
...
is one of the main considerations (Chapt. \ref{chapt.design}) where
spatial recaptures are need to estimate $\sigma$. These results
suggest that, perhaps, spatial recaptures are not needed if some
telemetry data are available.  The resource selection parameter
...
relevance in the study of animal populations and in landscape
...
misclassification, is clearly more difficult to deal with although its
