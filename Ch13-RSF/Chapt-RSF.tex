...
{\bf XXXX  with the re-order of chapters this might go in Richard's
  chapter XXXX }
...
{\bf XXXX Probably need to change z(x) to C(x) XXXXX}

Assume that the landscape is defined in terms of a discrete
...
denote a covariate measured (or defined) for every pixel ${\bf
  x}$. 
 We suppose that individual members of
some population wander around space in some manner related
to the covariate $z({\bf x})$.
%We will define ``use
...
context of SCR models later chapters of this book (Chapt. {\bf XXX and
  XXXX}).
Here we adopt more of a phenomenological formulation of space usage as
follows:
If an individual moves from a pixel ${\bf x}$ to another pixel ${\bf x}'$, this is
defined as a decision to ``use'' pixel ${\bf x}'$. This also induces a
definition of ``truth'' -- that is, over any prescribed time interval,
the animals makes some number, say $R$ of use decisions, and they are,
conceivably, observable by some omnipotent accounting mechanism (e.g.,
continuous telemetry).  {\bf XXX need to check notation consistent
  with chapter 5 XXXX} 
In this case, let $r_{ij}$ be the {\it true}
use frequency of pixel $j$ by individual $i$ -- i.e., the number of
times individual $i$ used pixel $j$.  We assume the vector of use
frequencies ${\bf r}_{i} = (r_{i1},\ldots,r_{iG})$ has a multinomial
distribution:
\[
{\bf r}_{i} \sim \mbox{Multinomial}(R, {\bm \pi}_{i})
\]
where
\[
...
\citep{manly_etal:2002} where all
available pixels are censused, and used pixels are sampled randomly
for each individual. 
\begin{comment}
...
{\bf XXX copy some material from the MEE paper here XXXX}
...
$z({\bf x})$ (and see Chapt. \ref{chapt.poisson-mn}).  In particular,
under this model for space usage or resource selection, if you have no
...
reproduce here in Fig. \ref{rsf.fig.habitat}. {\bf XXXX WRONG FIG!
  XXXXX probs don't need the habitat fig, do we? }
\begin{figure}
\centering
\includegraphics[width=3.5in,height=3.25in]{Ch13-RSF/figs/habitat.png}
...
Space usage patterns for 8 individuals are shown in
Fig. \ref{rsf.fig.homeranges}, simulated with $\alpha_{1} =
1/(2\sigma^2)$ with $\sigma = 2$ and the coefficient on $z({\bf x})$
set to $\alpha_{2} = 1$.
\begin{figure}
\centering
...
in the sense that, if we condition on the total sample size $r_{i.} =
\sum_{j} r_{ij}$, then the vector ${\bf r}_{i}$ has a multinomial
...
create the totals $r_j = \sum_i r_{ij}$, then a standard Poisson
regressino model for the resulting ``quadrat counts'' is
reasonable. This is ``Design I'' in \citet{manly_etal:2002}.

In practice, we never observe ``truth'', i.e., the actual use
frequencies $r_{ij}$.  As formulated in Sec. \ref{scr0.sec.implied},
we assume a binomial (``random'') sampling mode:
\[
...
distribution of $m_{ij}$ is also Poisson but with mean
\[
 \log(\lambda_{ij}) = \log(\phi_{0}) + a_{0} -\alpha_{1} D_{ij}^{2} +  \alpha_{2} z({\bf x}).
\]
Thus, the space-usage model (RSF) for the thinned counts $m_{ij}$ is
the same as the space-usage model for the original variables $r_{ij}$.
This is because if we remove $r_{ij}$ from the conditional model by
summing over its possible values, then the vector of ${\bf m}_{i}$ is
...
in terms of the underlying model of space usage that we just developed.
...
model. The difference is that, for SCR data, we do not have sampling
devices in all locations (pixels) in the landscape, and hence the data
are only recorded at a subsample of them. XXXXX {\bf move the
  following } XXXXX In other words, imagine that we have a sampling
...
instrument.  If it operates intermittently and does not expose the
...
$\lambda_{0} \equiv \exp(\phi_{0})$ represents the sampling
...
$m_{ij}$ for every pixel $j$.

Introducing the latent use frequencies $m_{ij}$, and considering the
Bernoulli SCR model where $y_{ij} = 1$ if the individual $i$ visited
...
Chapt. \ref{chapt.covariates}. While we are only considering the
Bernoulli encounter model here, we 
imagine that any of the standard SCR observation models (Chapt. \ref{chapt.poisson-mn}) could be
...
with the individuals wearing radio-collars; (4) In cases where we {\it
  can} match some individuals between the two samples, regarding them
...
km$^2$ in southwestern New York \citep{sun:2013}. We reproduce their
...
based on the landscape shown in
...
landscape, subjected to encounter 
 over $K=10$ sampling periods, by  a $7 \times 7$ array
of trapping devices located on the the integer coordinates $(u*5,v*5)$
for $u,v = 1,2,3,4,5,6,7$. The encounter model and parameter values were:
\[
...
selection functions \citep{manly_etal:2002} but it has almost never
been studied using capture-recapture models. Conversely, 
essentially all applicatiosn of SCR models 
have focused on density estimation, not understanding space usage.
...
estimation in SCR applications. The development in this cahtper shows
...
Independence of samples... but some individuals might appear in both
data sets.  The argument made by Royle et al. is that 
ignoring that information should entail
only an incremental decrease in precision because a slight bit of
...
This is related to some of
the mark-resign material (chapter 19????). 
...
XXXX Stuff below is wrong XXXXXXXX
