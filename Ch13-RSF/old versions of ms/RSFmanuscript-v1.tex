...
inference about population density from capture-recapture data using auxiliary
information about individual capture locations
\citep{efford:2004,borchers_efford:2008, royle_young:2008}.
SCR models posit that $N$ individuals are located within a region denoted
$\mathcal{S}$. Each individual has a home range or activity area within
which movement occurs during some well-defined time interval, and the
center of the animal's activity has Cartesian
coordinates $\mathbf{s}_i$ ($i=1,\ldots,N$). The population
is sampled using $J$ traps with coordinates
${\bf x}_{j}$ for $j=1,\ldots,J$, and
encounter
probability is expressed as a function of
the distance between trap location (${\bf x}_{j}$), and individual
activity center (${\bf s}_{i}$).
...
probability due to the juxtaposition of individuals with traps. %, and they provide a
...
which are unevently distributed in space. Therefore %, and thus
...
We organize them into an $nG \times 2$ matrix ${\bf X}$.  In SCR
...
Let $n_{ik}$ be the use frequency of pixel ${\bf x}_{k}$ for
$k=1,2,\ldots,nG$, by individual $i$ --
i.e., the number of times individual $i$ used pixel ${\bf x}_{k}$
...
where
\[
 \pi_{ik} = \frac{ \exp( \alpha_{2} z({\bf x}_{k}) ) }{ \sum_{k}
   \exp(\alpha_{2} z({\bf x}_k))}
\]
...
observe $n_{ik}$ for all individuals but, instead, only for a small
...
$D_{ik} = ||{\bf x}_{k} - {\bf s}_{i}||$ be the distance from the home
range center of individual $i$, ${\bf s}_{i}$, to pixel $k$, ${\bf
  x}_{k}$. We modify the space usage model to accommodate that space
use will be concentrated around an individuals home range centroid:
\begin{equation}
 \pi_{ik} = \frac{ \exp( -\alpha_{1} D_{ik}^{2} +\alpha_{2} z({\bf x}_{k}) ) }
{ \sum_{x} \exp(-\alpha_{1} D_{ik}^{2} +\alpha_{2} z({\bf x}_{k}))}
\label{rsf.eq.rsf}
\end{equation}
...
constant $\sum_{x} \exp(-\alpha_{1} D_{ij}^{2} +\alpha_{2} z({\bf
  x}_{k}))$, which ensures that the use distribution is a proper
...
$\pi_{ik}$ are directly proportional to the SCR model for encounter
probability.  For example, setting $\alpha_{2} = 0$, then this implies
probability of use for pixel $k$ is:
\[
p_{ik} \propto  \exp( -\alpha_{1} D_{ik}^{2}).
\]
Therefore,
for whatever model we choose for
$p_{ik}$ in an ordinary SCR model, we can modify the distance
component in the RSF function in Eq. \ref{rsf.eq.rsf} accordingly to
be consistent with that model, by using whatever function $p_{ik}$ we
choose according to
\[
\pi_{ik} \propto \exp( log(p_{ik}) + \alpha_{2} z({\bf x}_{k}) )
\]
...
\[
 n_{ik} \sim \mbox{Poisson}( \lambda_{ik})
\]
where 
\[
 log(\lambda_{ik}) = a_{0} -\alpha_{1} D_{ik}^{2} +  \alpha_{2} z({\bf x})
\]
...
sample size $R = \sum_{k} n_{ik}$, then the vector ${\bf n}_{i}$
has a multinomial distribution with probabilities
\[
 \pi_{ik} = \frac{\lambda_{ij}}{ \sum_{k} \lambda_{ik}}
\]
...
each pixel, i.e.,
create the totals $n_k = \sum_i n_{ik}$, then
...
number of telemetry fixes than actual uses, $n_{ik}$.
We express this ``thinning'' (or sampling) by assuming the observed number of
uses is a binomial random variable based on a sample of size $n_{ik}$:
\[
 m_{ik} \sim \mbox{Bin}(n_{ik}, \phi_{0}).
\]

 Then, the marginal distribution of $m_{ik}$ is also Poisson
but with mean
\[
 log(\lambda_{ik}) = log(\phi_{0}) + a_{0} -\alpha_{1} D_{ik}^{2} +  \alpha_{2} z({\bf x}).
\]
 Thus, the space-usage model (RSF) for the
thinned counts $m_{ik}$ is the same as the space-usage model for the
original variables $n_{ik}$.  This is because if we remove $n_{ik}$
from the conditional
 model by summing over its possible values, then the vector of
${\bf m}_{i}$ is {\it also}  multinomial with cell probabilities
\[
\pi_{ik} = \frac{\lambda_{ik}}{\sum_{k}  \lambda_{ik}}
\]
...
view that these frequencies $m_{ik}$ are {\it latent}. In particular,
