...
models, the ``population membership'' variable $g_{i}$ is a {\it
  ...
  alho:1990, royle:2009}.
Before considering SCR models specifically, in the next
section we talk a
...
populations having unknown sizes $N_{g}$, and we wish to impose model
structure on the population size variables using a Poisson
distribution:
\begin{equation}
...
think about doing data augmentatIon on each population {\it
...
want to pick $M_{g}$ in a way that imposes the correct structure on
...
$z_{i}$.  In other words, as specified in {\bf BUGS} pseudo-code, the
model is:
\begin{verbatim}
psi ~ dunif(0,1)
for(g in 1:G){
