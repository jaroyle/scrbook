...
In the stratified populations just described, an individual is assumed
to be a member of a single strat. However, 
...
might belong to multiple of the strata, but, in the models we discuss
in this chapter, we will not acknowledge that in the model. 
...
the stratum-specific population sizes, $N_{g}$, {\it conditioned} on
their total. This results in a multinomial distribution which we can
analyze in some generality using data augmentation.  As a practical
matter, specification of this multinomial distribution for the $N_{g}$
...
models by data augmentation \citep{converse_royle:2012, royle_coverse:2013}.
...
\[
 N_{s} \sim \mbox{Binomial}(M_{s} , \psi)
\]
...
Two implementations: For each $M_{T}$ individual in the
super-population we introduce a latent variable $g_{i}$ to indicate
which population the individual belongs too, and we introduce a 2nd
...
$z_{i} \sim Bern(\psi)$.  In particular, the multinomial assumption
for the latent variables $G_{s}$ is formulated in terms of ``group
...
\lambda_{s}/(\sum_{s} \lambda_{s})$.  The binomial sampling is
described by the binary variables $z_{1},\ldots,z_{M}$ such that
\[
 z_{i} \sim \mbox{Bern}(\psi)
\]
where $\psi$ is constrained as noted in the previous section.  A
second implementation of the model is suggested by marginalizing
Eq. (\ref{eq.mn.N}) over the prior $N_{T} \sim \mbox{Binom}(M,
\phi)$. In this case, we can define $g_{i}$ to have $S+1$ categories
where the last cell ($S+1$) corresponds to excess zeros.  The cell
probabilities for this multinomial are $\pi_{s}^{+} = \pi_{s} \psi$
for $s=1,2,\ldots,S$ and $\pi_{S+1}^{+} = (1-\psi)$ for the last cell
which corresponds to individuals of the super-population that are not
...
one step using the categorial distribution:
\begin{equation}
...
Any observation model can be used...
...
for a species of {\it Mustelid}, and we establish hair snares
at road/stream intersections within 20 watersheds containing
tributaries to a large river.  
For each watershed we measure a number of covariates, including an
index to habitat being the total km of stream distance plus km of pond
and lake shoreline. So we imagine a simple model like this:
\[
log(lambda_{g}) = log(area_{g}) \beta_{0} + \beta_{1} habitat_{g} 
\]
where $area_{g}$ is the total area of the watershed. Having this
as an offset on the log-scale ensures that the model is producing mink
per ha. 
