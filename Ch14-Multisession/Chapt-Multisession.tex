...
A point to make: Think about doing data augmentaiton on each
...
M_{g} in a way that imposes the correct structure on N_{g}. In
parciular, 
 we want $N_{g}$ to be Poisson so naturally we
choose $M_{g}$ to be Poisson itself and then maginally $N_{g}$ is also
poisson with mean \psi exp(\beta_{0}).  Clearly $\psi$ and $\beta_{0}$
