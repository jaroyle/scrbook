...
(Fig. \ref{searchencounter.fig.snakeline}). This is the type of problem described by
...
Fig. \ref{searchencounter.fig.snakeline}) but the polygon boundaries may or may not be
meaningful in terms of the observation process. That is, if one is
sampling along the path shown in Fig. \ref{searchencounter.fig.snakeline} and
...
Fig. \ref{searchencounter.fig.snakeline}) was used as a mechanism for producing the
...
Using the example from the Fig. \ref{searchencounter.fig.snakeline}, we could imagine that each
...
or collection of line segments (e.g., Fig. \ref{searchencounter.fig.snakeline}).
...
\label{eq.hazard}
\end{equation}
Additional covariates could be included in the
hazard function in the same way as for any model of encounter
probability that we've discussed previously.
