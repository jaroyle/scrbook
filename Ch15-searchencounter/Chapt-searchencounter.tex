...
% XXXX RC: Andy, I like the new paragraph a lot, but I suggest
% reminding people what [s] means. I know -- they should know -- but
% some will need the reminder. Also, explain in words that [u|s] is a
% movement model
...
the observation component, which we denote by $[y|{\bf s}]$, and the
...
write this as follows: The observation model has the form $[y|{\bf
  u}]$, and the process model has two components, $[{\bf u}|{\bf s}]$
and $[{\bf s}]$. Because we can resolve parameters of the $[{\bf
...
recognize 4 basic sampling designs, % XXXX RC: Did you change this to 2?
