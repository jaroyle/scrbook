...
\subsection{Simulation and Analysis in {\bf BUGS}} 
...
Final thing is we do the data augmentation and we make up some
starting values for the location coordinates that are missing.  
\begin{verbatim}
M<-100
nind<-nrow(y)
...
Now show the WinBUGS model right here and fit it .... Figure XXXXX
........ it produces the
following output:
...
The previous development assumed that
...
\]
which resembles Eq. \ref{eq.hardplot} except replacing the encounter
probability function with the constant $p_{0}$. 


In the analysis of Royle and Young 2008 they used the simple bivariate
Gaussian movement model in which 
\[
 {\bf u}_{ik} | {\bf s}_{i} \sim \mbox{Normal}({\bf s}_{i}, \sigma^{2}{\bf I}),
\]
However, clearly more general versions of the model can be fitted.
For example, imnagine a situation where the successive samples of a
...
\begin{eqnarray*}
 u_{1}(i,k) | {\bf s}_{i} &\sim &  \mbox{Normal}( u_{1}(i,k-1),  \sigma_{move}^{2}) \\
 u_{2}(i,k) | {\bf s}_{i} &\sim &  \mbox{Normal}( u_{2}(i,k-1),  \sigma_{move}^{2}) \\
\end{eqnarray*}
...
$\sigma_{move}$ or even an regression parameter $\rho$ pre-multiplying
the lagged value of $u$.  In the repo we provide a script for
simulating and fitting search-encounter data using the iid Gaussian
model and also the random walk model.  The WinBUGS model specification
is shown in Panel \ref{search-encounter.panel.uniform} for the random
walk situation.  Of course we imagine that resouorce seelection
...
It would be useful to do a simulation study to look at this:
