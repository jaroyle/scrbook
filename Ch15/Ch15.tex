...
Implementing a spatial mark-resight model in {\JAGS} is not trivial, so that knowing how to write your own MCMC algorithm comes
...
\]
and estimating $c$ as another model parameter, so that we account for the uncertainty about it. If we choose an uninformative (and conjugate) $Beta(1,1)$ prior for $c$, we can update it directly from its full conditional distribution, which is beta$(1 + \sum y_c, 1 + (\sum y_m-\sum y_c))$, within our MCMC algorithm.
...
and
\[
[{\bf s_i}|{\bf l},\sigma] \propto \left\{\prod_{r=1}^R \frac{1}{2 \pi \sigma^2} exp\left(-1/2 \left[ \frac {(l_{irx}-s_{ix})^2} {\sigma^2} + \frac{(l_{iry}-s_{iy})^2}{\sigma^2} \right]\right)\right\}*[{\bf s}_{i}] 
\]
...
Camera-traps recorded 117 pictures of unmarked raccoons, 33 pictures of 18 marked and identifiable raccoons, and 49 records of marked but not individually identifiable individuals. An average of 16.32 telemetry locations (SD 4.91) were collected for each of the 38 collared individuals. Raccoon abundance on the island was estimated at 305.461 (SE 35.012) individuals, which translated to a density of 13.564 (SE 1.554) individuals per $km^2$. Parameter estimates are listed in Table \ref{partialID.tab.raccoons}. 
