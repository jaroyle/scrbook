...
\begin{itemize} 
\item Tagged individuals are always correctly identified to individual level 
\item Tagged individuals are not always identified to individual level (but whether an individual is tagged or not is always correctly recorded)
\end {itemize}
...
Unobserved $z_{ijk}$ are updated using their full conditional distribution, which is multinomial with sample size $n_{jk} -- \sum z_{[i = known]jk} $.  
...
{\bf WILL GET THE DATA SOON AND HOPE TO GET ANALYSIS RUNNING IN A WEEK OR TWO}
