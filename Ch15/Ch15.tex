...
In general, we have (at least) two sets of data: encounter histories for identifiable individuals, hi, and counts of unidentified records for each sampling device j, yj. Depending on the sampling technique, we can conceive of three slightly different types of partial ID data. 
Imagine tags can always be identified to individual level when a tagged animal is resighted. Under these circumstances, your y will only contain records (sightings, pictures,…) of untagged – unknown – individuals and all records of your tagged – known – individuals will be correctly attributed to the right capture histories in H. You may fail to resight some of the tagged individuals, but since you know how many there are, you can simply assign them all-0 capture histories.
A slight variation of this data type arises when in some instances we can only tell that an individual is tagged, but not who it is. You may be able to see an individual is tagged but the identifying number on the tag may have become unreadable, or may be hidden from view. In this case in addition to y and H, you have some count of records per sampling device of marked but unidentified individuals, x. 
...
(A special case of this kind of data can arise from camera trapping. Even when dealing with a species that has no spots or stripes, some individuals in the study population can have natural marks that make them identifiable on pictures, such as scars or some distinct coloration. Again, in this scenario an individual has to be photographed at least once to be known. In case of camera-trapping, the fact that both the ‘marking’ method and the subsequent resighting method are the same (although marking in this case does not involve any actual physical marking) can be cause for concern: our sample of ‘marked’ individuals may not be a random sample of the population but consist of individuals that are more likely to be photographed.)    
Finally, consider a scat or hair snare survey, where only a part of the samples are analyzed genetically. In this scenario, your y can contain both completely unknown individuals that are not represented at all in H, but it can also contain samples from individuals that we previously identified. The difference is that in the first two scenarios, part of the population of individuals is identifiable, while in the second scenario, part of the ‘population’ of samples is identifiable. This type of data actually violates one of the basic assumptions of mark-resight models, namely, that tags are always reported correctly. For now, we will ignore this kind of data as well as the special case of camera trapping where some individuals can be identified based on natural marks and focus on the two types of typical mark-resight data.  
1)	A - Known number of tagged individuals that are always correctly identified to individual level + records of an unknown number of untagged (unidentifiable) individuals.
B - Known number of tagged individuals that are correctly identified to individual level + records of tagged individuals that are not identifiable to the individual level + records of an unknown number of untagged individuals.
2)	A - Unknown number of tagged individuals that are always correctly identified to individual level + records of an unknown number of untagged individuals.
B - Unknown number of tagged individuals that are correctly identified to individual level + records of tagged individuals that are not identifiable to the individual level + records of an unknown number of untagged individuals.

\section {A brief history of mark-resight models}
Initially, mark-resight methods focused on radio-tagged individuals to estimate population size (White and Shenk 2001). Radio-collars provide a means of determining which of the animals were in the study area and available for sampling, i.e. determining the number of marked individuals in the population. Knowing this number was a prerequisite for earlier mark-resight approaches. The oldest mark-resight model is the good old Lincoln-Petersen estimator, where individuals are marked and a single resight/recapture occasion is carried out (Krebs 1989?). We need not identify individuals, but only tell apart marked from unmarked individuals. Let m1 be the number of marked individuals in the population, m2 the number of marked individuals seen on the resighting occasion, and n2 the total number of marked and unmarked individuals observed during resighting.  Abundance N is then estimated as 
N =m1 * n2 / m2

A suite of more elaborate models using individual capture histories over several resighting occasions were developed in the 1980ies and 90ies and compiled into the program NOREMARK (White 1996). Apart from a model for the basic case with known number of marked individuals and no individual variation in resighting probabilities (joint hypergeometric maximum likelihood estimator; Bartmann et al. 1987, White and Garrott 1990, Neal 1990, Neal et al. 1993), NOREMARK contains models that account for the lack of geographical population closure (Neal et al. 1993), individual heterogenenity in resighting rates and sampling with replacement (i.e. individuals can be seen on any occasion more than once, Minta and Mangel 1989, Bowden, 1993 ). The first mark-resight model for an unknown number of marked individuals was developed by Arnason et al. (1991), but their approach has several limitations. More recently, more reliable and flexible likelihood-based mark-resight models have been developed and incorporated into the program MARK (McClintock & White 2010). These models  account for individual heterogeneity in detection, unknown number of marked individuals and lack of geographical closure, as well as a less than 100\% individual identification rate of tagged individuals, can be applied to sampling with and without replacement and can combine data across several primary sampling occasions in a robust design type of analysis.  , McClintock et al. 2009a,b, McClintock and White 2009).  
...
Under these conditions, the information obtained from the individually recognizable animals can easily be included into the spatial non-capture-recapture model we presented in Chapter X. Recall that without individual identity, the observed counts at trap j and occasion k, yj,k, represent the sum of all latent individual detections at j and k, SZijk. We can model photo counts as
\[
yj,k ~ Poisson (Lamjk)
Lamjk = SZijk
\]

Under this formulation we do not need to update the individual Z’s in our model. However, we can also formulate it as conditional on the –latent – Z’s. This is useful because if we have n individually known animals in our study population, than those n Z’s are no longer latent, but fully observed and can easily be included in the analysis. 
The formulation conditional on Z basically brings us back to the original SCR model, where individual site and occasion specific counts, Zijk, are modeled as
\[
Zijk ~ Poisson(lamij), and
lamij=lam0 * exp(-Dij2/2sigma^2)
\]

Unobserved Zijk are updated using their full conditional distribution, which is multinomial with sample size yjk – SZ[observed]ijk .  
While in the non-spatial mark-resight analysis known individuals provide direct information about individual detection probability (or rate), in the spatial setting they also inform the movement parameter sigma. Including known individuals into the analysis help estimate model parameters more accurately and precisely, especially N, which is usually the objective of inference… some stuff about Richard’s simulation.

\subsection{MCMC for the spatial mark-resight model with known number of marked individuals}
While the partial ID model can be coded relatively easily in WinBUGS, its updates are painstakingly slow. Here, knowing how to write your own MCMC algorithm comes in extremely handy. You will find that for the partial ID model we only have to make relatively simple modifications to the MCMC code for the model without any individual identification presented in Chapter XX, which, in turn, has much in common with the algorithms we developed for regular SCR models in Chapter 6. You can find the full MCMC code in the accompanying R package scrbook (mcmcZdatSim.txt). 
\begin{comment}
...
\subsection  {individual identification rate <100%}
...
McClintock et al. (2009) suggest an intuitive means of correcting for this bias in a non-spatial model framework: When estimating the number of records of unmarked individuals, Tu, as 
E(Tu) = (N-n) { lam , eta/n}
where N is the total population size, n is the known number of marked individuals, lam is the individual encounter rate estimated from the known resighted individuals and eta is the number of records of marked but unidentified individuals. So essentially, because lam is known to be too low, the average number of unidentified pictures per known individual is added as a correction factor. This procedure assumes that the inability to identify a marked individual occurs at random throughout the population.
We can relatively easily translate this concept to our spatial mark-resight models.  In the spatial model framework we are estimating an individual and trap specific encounter rate (the trap specificity is introduced by the relation between trap encounter rate and the distance of the trap to the individual’s activity center). Further, we do not look at the sum of all records of unmarked individuals, but formulate the model conditional on the partially latent individual encounter histories. Thus, instead of using the number of unidentified pictures per marked individual as a correction factor, we need something that applies at the individual and trap level. If we take the sum of all correctly identified records of marked individuals, SZ[known], and divide it by the total number of records of marked individuals, SZ[known] + eta, we get the average rate of correct individual identification for marked individuals, say, c. We can then multiply lam0 times c in the model for the marked individuals, to account for the fact that we observe incomplete individual encounter histories. For example, if on average we are able to assign 80% of the records of marked to the individual, we would multiply lam0 * 0.8 in the part of the model for the marked individuals. The model for unmarked individuals remains as before. 
{bf\Example ?}
