...
\]
XXXX I'm curious why this normal model which is a clear
misspecification ???? Can you say something about this here?  XXXX
...
be written as XXX is this a hypergeometric mean or where does this
come from ? XXXXXX
\begin{equation}
E(n_t) = (N-m) * \theta 
\end{equation}
\label{partialID.eq.E_n}
where $\theta = k \times p$ for a Bernoulli observation model with $k$
replicates and individual detection probability $p$, or $\theta$ =
expected/average individual encounter rate $\lambda$ for a Poisson
observation model. The form of $V(n_t)$ depends on the underlying
observation model, but like $E(n_t)$ it is based on the parameters
...
While these mark-resight models are very flexible and provide robust
XXX not liking word ``robust'' here XXXXX estimates of abundance, they
share the shortcomings of “regular” capture-recapture models
when it comes to estimating population density XXXX reference back to
chapter 1 or chapter 3 or somewhere XXXXX. 
XXX I feel like its a bit late in the chapter to be saying what we're
going to be doing in this chapter. Maybe say.... in what follows or in
the next sections we do xyz XXXXXXX
