...
density.
% XX RS: Maybe add 'and space usage' since the RSF and ecoldist chapters also come before open
...
movement 
% XX RS: Maybe add something like '... on a larger temporal scale' to set it apart from the movement models we have in the closed pop models?
(migration, dispersal, etc...).  The ability to estimate
...
% XX RS: I'd add a little more to this sentence so it's clearer: posterior predictive distributions of projected population sizes? The MCMC algorithm applied to an open population model? 
...
dealing with individuals that do 
% XX RS: do not?
...
% XX RS: ... to account for possible changes in the population over time...
(the spatial capture recapture open models had
...
from immigrants and deaths from emigrants. 
% XX RS: Maybe a sentence here that links this inability to the way we study populations - we only collect information in some area and when an animals disappears, we don't know if it died or emigrated (unless we can recover it dead). Because if we had the data we could separate out these two factors, right?
...
ecology, dispersal can be related with accession
% XX RS: access?
 to reproduction,
...
% XX RS: This first half of the sentence doesn't make sense to me - sampling once between occasions? Maybe a short intro here about how to sample for the JS would help; then you could say that with one sample per primary occasion you can apply the JS, but with several secondary occasions within each primary, you can use pollock's stuff. 
% XX RS: Ok, I see this comes up below, but I think it would be helpful to explain it a little here, because up til now people don't know anything about sampling protocols for the open pop models. Maybe you can move the robust design description up, make it a subsection before starting with the JS part, and say that the number of times you sample within each primary occasion defines what kind of model you can use, or something like that. 
...
% XX RS: Aren't ehy supposed to be closed within each primary and open between?
...
\]
% XX RS: Not a big deal, but why do you index the alive state in parentheses when you use subscripts for other parameters?
...
% XX RS: why not replace the binomials with Bernoullis below; then if the guy has been recruited before, p=0, rather than sample size
% why tau, not t, in the sum in the second formula?
\[
...
% XX RS: I'd mention here that gamma_1 is equivalent to psi in the previous 2-period description of the open model
...
size of 0).  
% XX RS: I'd add aggain that this ensures that an individual cannot be recruited again after initial recruitment.
...
{\it per capita} recruitment.  
% XX RS: It's hard to see for a first time reader where gamma comes in because it's not in the formula for z. Maybe explicitely refer back to what A is. Might be redundant to us but for first time readers this stuff is not trivial.
We carry out this process in \jags~ by
...
\end{panel}
% XX RS: Some of the lines in the panel are too wide; shorten comments or put them in separate lines rather than after code
...
array 
% XX RS: Give dims of array: is it M by J by T? or MxJxKxT?
represents just one individual across all primary periods.
...
shown here.
%%% XXXX Beth: give the name of the script or help file or whatever.
...
% year $t$ could have been alive in year $t-1$.  
Our data
augmentation must be large enough to include individuals alive during
any of the time periods and to account for that, we set $M=200$.  
% XX RS: Mabye give number of birds observed here to give context to M
For this example,
...
are used:
% XX RS: I thought we weren't using the arrows? The lines of code are too wide (inits and out1)
{\small
\begin{verbatim}
...
estimate of density. %%provide a robust estimate of density.
...
{\flushleft \bf Ovenbird Mist-netting study}
% XX RS: either all capital or all lower case - I don't remember which...
...
% XX RS: suggest adding something like: because of the Markovian relationship between abundances the JS is able to make use of the data a little more efficiently; even if nothing added, the sentence reads awkward, needs some restructuring
 and the JS model is able to
...
% XX RS: I have changed N to N_t, because at the very beginning you define N as the superpopulation across all years. Maybe check that notation is consistent; i try to catch it where I can. 
% XX RS: In the panel some of the lines are too wide again; compile to see.
...
% XX RS: here you use the subscript notation for z, not the parentheses as above
 \[
...
detection $p_t$.  
% XX RS: In the JS formulation you just use p, here p_t. Not a big deal but I'd maybe either standardize or say that p can also be non-time-dependent.
If the individual is not alive ($z_{it}=0$), then
...
means that $z_{it}$ will be 1 when $t$ is the primary period individual $i$ is first
captured in.  We denote this $z_{i f_i}$, where $f_i$ indicates the primary
...
entering the model.
% XX RS: Isn't that also the reason why we cannot estimate recruitment? I'd make that distinction from the JS explicit here again.
...
% XX RS: Here you give the example a subsection; above you just give it a non-section heading. I had done that too but I changed all my examples to subsections, I think. Not a biggie but maybe something worth standardizing within a chapter.
...
antenna, it was recorded and summarized weekly for 12 weeks. 
% XX RS: That second half of the sentence doesn't make sense: how's the fish summarized weekly? Is it 'data were summarized'?
The fish
...
% XX RS: Do these guys migrate upstream to spawn? Otherwise I wonder why they don't eat.... might be worth mentioning
...
monitored.  
% XX RS: Ok, they are ocean fish! I'd def. mention that in the very beginning of this section. because I thought they were river fish and was like, what, they don#t eat in rivers? When do they ever eat then?
We demonstrate in Panel \ref{open.panel.nsCJS} how to
...
% XX RS: I find this a little confusing: if it survives to t+1 in A, then how can it move to B just before t+1? Do you mean, it is in A at t, survives from t to t+1 and moves to B just before t+1? I don't know enough about multi-state to know if that's what this means, but since this is the first presentation of multi-state stuff, I'd be very specific here. 
...
% XX RS: Is it right that there is no error in observing states, i.e., if the animal is observed, state is recorded correctly? Might be worth adding if that's true, to distinguish between these two possible observation errors.
Hence, we also have $p^A$ and $p^B$, the probability of detecting an
individual in State A and State B respectively.

% XX RS: I have to say that I cannot follow the next 2 or 3 paragraphs. I find it confusing that s is not time dependent, although individuals can transition between states; I am not sure what psi is in either of the two formulations, and whether p_0 should be state specific, although it is not indexed as such. Is the symbol s, for state, chosen intentionally to be the same as the home range center? if so, why? 
% XX RS - after reading on: Reading on, I think you might even delete this section, or just briefly mention the connection of spatial and multi state models, and go straight to the JAGS formulation. But I think if you keep it it needs some more context.
To relate this formulation of the multi-state model
back to the description 
% XX RS: description of what?
provided in Chapt. \ref{chapt.poisson-mn}, we can define ${\bf s}$
as the index of which state an individual is in 
% X RS: Isnt s sample-dependent also?
and $u_{it}$ as
...
% XX RS: Since psi is a vector, here is a way to make it bold within a math expression: \mbox{\boldmath$\psi$}
\[
...
% XX RS: Ok, this stuff below is helpful, but the context needs to be specified before starting on the states etc a few paragraphs before. 
...
\end{panel}
% XX RS: Maybe add in the figure legend that r denotes state (minus not being alive) here?
...
% XX RS: Isn't that an important observation, that we assume state to remain constant within a primary occasion? Might be worth mentioning somewhere at teh beginning of the multi-state stuff.
...
% XX RS: now you're back to parentheses for z indexing
 \[
...
\subsubsection{Migratory shad study}
% XX RS: Maybe standardize the name for this shad example
...
%BETH - include script name and add it here!
...
% XX RS: I think an opening sentence here would be great. Something along the line of 'To fully specify the dynamics of a population, we also have to think about how the locations of activity centers evolve over time. We know that animal territories can shift and that other types of movement (migration, dispersal) can take place. In the framework of open population SCR models, such dynamics in individual locations need to be reflected in appropriate models for the distribution of the activity centers.' This is just a blurb, but I think people are so trained to think about alive-dead that it is worth pointing out that our process model has anothr component - location - and that we have to think about that part of the process too.
...
% XX RS: 'This might be appropriate for highly territorial species, and if primary sampling periods are relatively close together in time and the voerall time frame of the study is limited.' 
It might seem more likely
that activity centers change over time but are independent from year to year for a
given individual such ${\bf s}_{i} \sim \mbox{Uniform}({\cal S})$.  
% XX RS: 'If there are large time lags between surveys, or if we study populations where individuals redistribute themselves frequently, such a model would be adequate.'
This is also how
...
% XX RS: 'This seems to be the most realistic model for many cases.'
For
example, many migratory passerines, like the ovenbird, return to the
same location, or nearly so each year.


We could use this specification 
% XX RS: Not clear what specification - or do you mean generally, a specification of a model for location changes in s?
to model changes in home range centers
...
% X RS: Suggested change to this last sentence: 'These are just a few simple examples; as
% more information becomes available and data are collected over longer
% time periods, we will be able to use more complex movement models in open SCR models.

{\flushleft \bf Cautionary note}: 
% XX RS: Maybe add 'Using such Markovian models for the change in location of activity centers across primary periods, activity centers are no longer bound by the limits of the state-space. Imagine an individual living at the very edge of S at t -- there is some probability that, under the perturbation model, the location of s at t+1 could be outside S.' 
...
\]
% XX RS: What is mu_S? Maybe make it mu_{\sigma}?
...
% XX RS: Did you run this model? Otherwise I'd say something about that this is just a mental experiment, or something

\subsection{Modeling  dispersal}
% XX RS: Maybe make this a section, since it applies to all, JS, CJS and Pollock?
% Also, I'd put in an intro sentence again, why dispersal is interesting.
...
\citep{ovaskainen_etal:2008}.  
% XX RS: I don't really get the connection of sparse data and different habitat types... does this refer to dispersal habitats often being sub-optimal, so we might spend less effort there?
...
$t$ and then
% XX RS: Shouldn't these S below be lower case, as in activity centers?
\[
...
is the dispersal direction.
% XX RS: Add units for direction... I can't think of the right english term
...
% XX RS: Maybe mention that we have to specify a model for dispersal distance, and then say, that one option is...
One option is
...
% XX RS. Might be worth saying here that pi is not some parameter, but actually the number pi (because we use that symbol for parameters in some other contexts).
If all individuals are expected to
move some distance between periods, then the $pd$ indicator could be
removed.  
% XX RS: I feel like when you told me about Torbjorns model, you said that guys flip a coin if they do long distance dispersal; but otherwise their activity centers could still move... so you could have two levels, close-range shifts in s, and long distance dispersal... just a thought, don#T worry about it if you don't want to add more stuff here.
A number of distributions exists for fitting these
...
% XX RS: Maybe add a sentence with something like ' While we have covered a lot of ground, there are many variations of the basic JS and CJS models, such as dead recovery models, which separate out mortality and true survival (some ref), or models that address transiency (mabye the karanth et al 2006), that we have not explicitely 'converted' to a spatial framework, and that provide a broad field of further model development'. 
