...
%%% XXXX Beth: I don't understand the meaning of this passage
%%% here. The first issue at hand is "basic fact of the data structure"
...
% XXX This line just above, it seems like this was just said 3 lines aog?
...
%% XXX But this is not a spatial model right?
...
\]
%% XXXX Can you link to the precise section?
...
%%% XXXX What are the two steps? No clear here...
...
%% XXXXX BETH: you never used the term "spatial multi-session model"
%% did you? Maybe i read over it.....
...
%%%% XXXX Andy sez: I added the rest of the paragraph here
...
% Andy sez: I added a word avoe to make that clear
...
% XXXX Beth: Can you change this to "Shad study in the New River" ? or similar?
%% never mind -- how about this:
\subsection{Movement and survival in American shad in the New River}
...
%%% BETH: Did you make that change suggested above? Also, below, are
%%% the "AB" supposed to be a joint subscript. They are not....
...
To extend this model, {\bf XXXXX To what??? XXXXX}
we can define ${\bf s}$
...
%% XXXX If this is true, Beth, can you remove one and work them together?
\[
 \Pr(y_{it} = 1| u_{it} )  = p_{0}
\]
...
to reduce the   %%%% XXXX BETH IMPORTANT -- there was a Git conflict
                %%%% living in this document -- see below. You need to
                %%%% figure this out!
<<<<<<< HEAD dataset however to fit this model and
information on spatial location was lost in creating just two states,
downstream and upstream.  Losing information is one potential effect
of using a multi-state model, additionally when states are hidden or
unknown (e.g., when animals are in a region not exposed to sampling),
these models can be difficult to fit (XXX cite Sarah?).  Both of these
issues can be resolved using the fully spatial CJS model.  =======
...
problem to solve and current approaches (\cite{link_etal:2010,
  mcclintock_etal:inpress}).  >>>>>>> final edits


\subsection{Spatial CJS models}

In Chapt. \ref{chapt.poisson-mn}, we suggested that SCR models are 
%%% XXX Double check usages of "essentially" and try to reduce. This
%%% paragraph has 2 uses of that -- and many times I have seen 2 or 3
%%% uses so I removed some of them.
...
we need only make a few changes to the model.  Essentially, we will
...
%%% XXXX Beth: the "Conversely" doesn't make any sense here! There is
%%% no early thing to contrast with.
Conversely, if the
...
%% XXXX BETH: I think you should carve out the model and put it in a
%% panel, and put all the R code in a script, put the script in the
%% repo and IDENTIFY THE NAME OF THE SCRIPT here.
...
%%%%Activity Center Dynamics}
\label{open.sec.ACdyanmics}


%%% XXXXXXX This intro sentence odesn't make any sense to me. Didn't we
%%% already do that?
...
fit. %XXX update references here (see \cite{McClintock:ecological monograph; johnson et al})
