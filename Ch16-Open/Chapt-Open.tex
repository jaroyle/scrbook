...
Burham JS (\citet{burnham:1997}), and the super-population formulation of
...
%\]
%we can use a set of
Determining the number of recruits into the
population is calculated basedon the alive state of the previous time steps ($1, 2 \dots, t-1$) and the current time
step ($t$).
...
in State A from time $t$ to $t+1$ and $\phi^B$ is analogous for State B.  The
...
State B & $\phi^B \psi^{BA}$ & $\ph^B(1-\psi^{BA})$ & 1 - $\phi^B$ \\
Dead & 0 & 0 & 1\\ \hline
\end{tabular}
...
back to the description provided in Chapt. \ref{chapt.poisson-mn}, we can define ${\bf s}$
...
fit (see \citet{jonsen_etal:2005, johnson_etal:2008b, mcclintock_etal:2012})
