...
species, such as tigers or marbled salamanders,
...
sampling and $N$-mixture models. These methods, and others, can work
...
movement complicates the use of fixed-area plots, these methods may not
...
recognition'' assumption of capture-recapture models is not a
...
model) require neither distance data nor fixed area plots. In fact, it
is virtually identical to standard SCR models except that the
encounter histories $\{y_{ijk}\}$ are not directly
observed. Instead, the observed data are the trap- and
occasion-specific counts, which are realized by
