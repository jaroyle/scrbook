...
Simulating data under the SC model is easy. All that is required is to
simulate standard SCR encounter data and then collapse it into count
data. The following \R~code generates the encounter history data from
the model shown in Sec.~\ref{chapt.unmarked.sec.N}, with parameters
$\sigma=0.1$, $\lambda_0=0.3$, and $N=$
\begin{verbatim}
R code
\end{verbatim}
Converting this to the requiste count data is done using a single
\verb+apply+ command. You could also do it using a loop, or any other
way you like, but this is perhaps the easiest:
