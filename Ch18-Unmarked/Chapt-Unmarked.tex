...
for(i in 1:M) {
   ...
   for(j in 1:J) { # Number of point count locations
...
We used the \jags~model in Panel~\ref{chapt.unmarked.panel.jags1} to
fit the model to the simulated data. One challenge faced during the
implenetation is that \jags~cannot auto-generate good initial values
...
\end{verbatim}
\end{small}
To actually fit the model, we use two functions. The first
...
\verb+coda.samples+ is one of several options for generating posterior
samples. We like it because it returns the samples in the format
required by the \texttt{coda} package. Be aware that the following
...
Results of this small set of posterior samples is shown in TableXXXX.
Also shown are results from fitting the model using the function
\verb+scrUN+ in our \R~package \texttt{scrbook} XXXXX.
