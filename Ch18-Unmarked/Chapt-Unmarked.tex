...
data are may be analyzed using %generalized linear models
...
again, suppose that there are $J$ ``traps'' operated on $K$ occasions
during some time period in which no births or deaths occur. We use the term trap very
loosely in this context. A trap is simply some sampling device capable
of recording the count $n_{jk}$; so a trap could
be a hair snare, camera trap, or even a human observer standing at some
...
x_j$. In some instances, additional data might be avialable such as
trap-specific covariates, state-space covariates,
information on the identities of a subset of individuals, or perhaps even distance
data, but %Some of these extensions are covered in
%Chapt.~\ref{chapt.partialID} %and~\ref{chapt.scrds},
%but
for the sake of simplicity we focus on the basic data structure in
this chapter.

\section{Model}


The state model describing density and distribution
% that we consider here
is the same as in the basic SCR setting. We
assume a homogeneous point process
$\{{\bf s}_i, \ldots, {\bf s}_N\} \sim \text{Uniform}(\mathcal{S})$
for the activity centers $\bf s$ in the state-space $\mathcal{S}$,
% of individual $i = 1,\dots,N$, and $\cal{S}$ is the state-space
which is typically a polygon defining
the region where the $N$ animals occur\footnote{We will temporarily assume
that $N$ is known.}.
The observation model is once again
...
Population size, $N$, is never known. Estimating this important
parameter requires that we put a prior on it. The two common priors,
used in the case of a homogeneous point process,
are $N \sim \text{Poisson}(\mu|\mathcal{S}|)$ and
$N \sim \text{Binomial}(M, $. In
...
and should be based primarily on convenience.
