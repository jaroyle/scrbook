...
GRTS) those units and measure some attribute. The resulting inference
...
context of capture-recapture studies, it is tempting to think that the
...
\url{http://en.wikipedia.org/wiki/Horvitz%E2%80%93Thompson_estimator})
  ...
  using a model.  SCR models achive this effect formally, using a
...
political reasons or legal reasons. To intitiate a study, or perhaps
...
XXXx Rahel: re below: When you say $\sigma^{*} = 5$ is the individual's home
range size, what exactly does that mean? Does that mean if you convert
$\sigma^{*}$ to $\sigma$ then trap spacing is small or large relative
to $\sigma$?
XXXXXX
...
results as the average over 100 simulations.
 ...
 \begin{table}[ht]
  \centering
  \caption{
XXXX Rahel: Can you add the value of ``sigma'' to this table too? This
might make it clearer to interpret.....XXXXXX
...
XXXX Rahel: paragraph below -- should be $sigma^*$ in all cases? XXXX
