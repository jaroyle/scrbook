...
% XXXX suggest shortening title to "Spatial Mark-Resight Models". The
% phrase "partially identifiable" might not be familiar to some readers
...
individuals are
identifiable % XXXX Why "identifiable" instead of just "marked"?
upon encounter, and in Chapt. \ref{chapt.scr-unmarked}
we introduced and developed an SCR model for
non-identifiable % XXXX Kind of contradicts first part of sentence
...
situation, where % a XXXX I don't think you need "a part"
part of the population is tagged or otherwise
marked and can thus be identified upon recapture, while the untagged
% XXXX Suggest sticking with "marked" instead of "tagged" or
% "identifiable" where possible
...
Traditionally, % XXXX suggests past tense
capture-recapture studies involve physical capture of
individuals throughout the study; % XXXX or "study area"?
...
which are analogous % XXXX "analogous", or "identical"?
...
new, untagged ones. Then, for the tagged animals you obtain the same
...
provide. We then move on to the formal development of SMR models,
...
encounter histories for
identifiable % XXXX again suggest "marked" for consistency
individuals $i$ at trap $j$ and
occasion $k$, $y_{ijk}$, and counts of
unidentified records % XXXX "unmarked animals"
for each
$j$ and $k$, $n_{jk}$.
% XXXX n_{jk} could also be just the counts of unmarked guys. THis
% makes the coding and analysis (especially in JAGS), much easier.
...
died % XXXX Somehow you might mention that open population models
     % could be used to model this process
...
If we suspect that some of the marks may have been
lost % XXXX Not just lost -- could be many reasons why you don't know m
between
...
{\bf $Y$}, % XXXX Probably should define this symbol. I don't think we
           % use big Y very often
...
sightings of tagged but unidentified individuals, say $r_{jk}$.
...
provide a means of
determining % XXXX Sort-of, they define "available" as "on the grid",
            % but the grid has no clear boundary.
which of the animals were in the study
...
are somewhat
limited: they % XXXX or, "limited in that they do not ..."
...
resightings are collected across a network or array of locations,
however, they come with the same spatial information as recaptures in
a regular SCR study.
In the following sections we will consider mark-resight sampling in
the framework of spatial capture-recapture. We will look at models for
both known and unknown numbers of marked individuals, and for
imperfect individual identification of marks. In the spatial
...
% which we include as variables (or missing data) in our MCMC scheme.
...
$n_{jk}$: XXXX Rahel: What is the vector exactly? Can you write it out
in terms of its scalar quantities, as a defintion, before using the
bold-font vector representation? XXXXXX
\[
\mathbf{y_{ujk}} \sim \mbox{Multinomial} (n_{jk}, \mathbf{\lambda_{uj}})
\]
where \textbf{\emph{u}} is an index vector of the $M-m$ hypothetical
unmarked individuals. % One thing you can do is to let "yu"_{ijk} be
                      % the latent encounter histories and n_{jk} be
                      % the counts of unmarked guys so that you can write:
                      % yu_ijk ~ MN(n_{jk}, \lambda_{ijk}*(1-w_i)) where w_i
                      % indicates if a guy is marked or not. The
                      % latent encounter histories will be 0 for
                      % marked guys
...
% XXXX I didn't comment on this section, because I assume you're going
% to overhaul it
...
% XXXX I don't think many readers will be familiar enough with our
% MCMC functions to be able to understand this. I suggest removing it
% and replacing it with a panel of JAGS code. Interested readers could
% be directed to the source code.
{\small
\begin{verbatim}
...
situation % XXXX suggest replacing "situation" with "model" or "design"
where the same individual could be drawn repeatedly, to a sampling
without replacement situation (an individual drawn once at $j$ and $k$
...
% XXXX Again, I think this code won't make sense to most readers. My
% opinion is that the R code we show should stand-alone so that
% readers can work with it directly
{\small
\begin{verbatim}
 Y <- array(NA, c(M, J, K))
#[...]
...
available for resighting, treating this number as unmarked is probably
more realistic % XXXX Not necessarily. Most people might have
...
array, % XXXX Need to define "resighting array" or use something like
       % "sampled region"
% XXXX It might also be good to list a few sampling methods in the
% introduction of the chapter. Many avian ecologists will think of the
% situation in which re-sighting is done while conducting "area
% searches". THis would require a search-encounter model since the
% actual locations of animals are recorded. Probably should mention that.
for us to know that it is there. When resightings are pooled
across the entire sampling grid, then the total individual counts $\sum_j y_{ij}$ have to be $>$ 0 for all resighted individuals and a zero-truncated distribution can be used to model these counts. However, we are concerned with trap-specific encounters, $y_{ij}$, which can easily be 0 for a resighted individual, as long as a single $y_{ij}$ is $>$ 0. Thus, the zero-truncation does not apply to the individual and trap specific counts we observe, but only to the sum of these counts over all traps.
...
of marked individuals\footnote{For the interested reader,
  \citet{mcclintock_hoeting:2010} implement a non-spatial mark-resight
  model with a binomial observation model in a Bayesian framework
  using data augmentation}. % XXXX Suggest simple citation instead of footnote
...
and tagged with neck and leg bands in Greensboro, North Carolina
(Fig. \ref{partialID.fig.geese}). Geese were resighted at 87 %different
locations % XXXX What were the locations? Habitat patches, quadrats, ..?
...
North Carolina. % XXXX State that you are assuming closure during this period
During this time frame, 57 of the resighting sites vere visited and 654 marked geese were resighted 3994 times at 40 different sites. In addition, 7944 sightings of unmarked geese were recorded at 48 sites.

In the model, we allowed $\sigma$ to vary between males and
females. We augmented the data set with
$5700 - 654$ % XXXX Not sure what the 5700-654 means
all-zero encounter histories, ran 50000 MCMC iterations and removed a burn-in of 5000 iterations. To describe the state-space, we buffered the resighting locations by 3 km. We provide all the data ({\tt data(`geesedata')}) and functions ({\tt geeseSMR}) for you to repeat this analysis but be aware that given the large data set it will take days to do so. The {\bf R} code to set up the data and run 5000 iterations of the goose model is given as an example on the help page for {\tt geeseSMR}. The model results, including the derived parameter density ($D$) in individuals per $km^2$ are shown in Tab. \ref{partialID.tab.geese}.
...
\centering
  ...
  \begin{tabular}{lccccc}
       ...
       &   Mean &   SD &  2.5\% & 50\% & 97.5\% \\
           \hline
$N$      & 4364.05 & 119.74 & 4140 & 4360& 4607 \\
$D$       &  12.40 & 0.34 & 11.76 & 12.39 & 13.09 \\
$\lambda_0$    &  0.29 & 0.01 & 0.28 & 0.29 & 0.30 \\
$\sigma$, females  &  0.92 & 0.02 & 0.89 & 0.92 & 0.96 \\
$\sigma$, males  &  1.01 & 0.02 & 0.97 & 1.01 & 1.04 \\
$\psi$     &  0.77 & 0.02 & 0.72 & 0.76 & 0.81 \\
$\phi$     &  0.40 & 0.02 & 0.35 & 0.40 & 0.45 \\
    \hline
  \end{tabular}
\end{table}

We see that credible intervals of estimates are pretty narrow. Take, for example, $\sigma$ for males and females: Although they differ only by 0.09 km, there is no overlap between the respective credible intervals, surely an effect of the large data set. The parameter $\phi$ in this model is the probability of being a male, a measure of the sex ratio of the population, which is close to 1:1.
...
For further illustration purposes we simulated a data set with $N=80$
individuals randomply distributed across a state space of 10x10
units. Of those, we randomly choose 40 to be marked and identifiable,
and then simulate encounter data for both marked and unmarked
individuals on an 8x8 grid with unit spacing over $K=5$ occasions,
with $\sigma=0.5$ and $\lambda_0=0.5$, adopting a Poisson encounter
process. To do so we use the {\tt scrbook} function {\tt sim.data},
which also allows you to create data sets from a Binomial % XXXX binomial
observation process, known number of marked individuals, and with telemetry locations (sec. \ref{partialID.sec.telemetry}) or individual identification rate $<$ 100 \% (sec. \ref{partialID.sec.IDrate}). We analyzed the simulated data both assuming we do not know the total number of marked animals in our state space, and assuming we do know this number, using the {\tt scrPID} function and running 20000 iterations (note that since we simulate and analyze the data under the same state-space, the model with known number of marked individuals should yield unbiased results). You can repeat the analysis by executing the R code below.
...
Looking at the data, we see that of the 40 marked animals, 26 were
recorded at least once. In terms of data that means that in the second
model, where we know $m$, we have 14 observed all-zero encounter
histories that we cannot use in the model where we assume $m$ is not
known. This reduction in data is reflected in the model results
(Tab. \ref{partialID.tab.unknownsim}). The estimate of $N$ for the
unknown-$m$ model shows some positive bias, although the 95 \% BCI
still includes the true value of 80. Thus, while we can formally
account for the fact that we often do not know the number of marked
individuals in the state space, we clearly loose quite a bit of
accuracy and precision. It would be an interesting
%little
project to quantify this loss in accuracy and precision in a small simulation study.
...
\hline
			&     	   &  Mean &   SD   & 2.5\% &  97.5\% \\
\hline
$m$ unknown	& $N$     & 86.612 & 9.386 & 70  & 107  \\
			& $\lambda_0$  & 0.4679 0.069 & 0.346 & 0.602 \\
			& $\sigma$ & 0.521 & 0.029 & 0.470 & 0.583 \\
			& $\psi$   & 0.541 & 0.070 & 0.411 & 0.684 \\
\hline
$m$ known	& $N$     &  79.525 & 6.149 &  69 & 93   \\
			& $\lambda_0$ & 0.550 & 0.077 & 0.403 & 0.707  \\
			& $\sigma$ & 0.514 & 0.0284 & 0.4638 & 0.5750  \\
...
with % XXXX "with" or "by"?
$c$
to account for the fact that we observe incomplete individual
...
before simply $\lambda_0$ (or in other words, %their
$c$ equals
1). Observe that now, in addition to assuming that failure to identify
...
encounter it in. Incomplete individual identification of marked
individuals is included as an option in the {\tt scrPID} function and
we show an example of using $c$ in an analysis in
sec. \ref{partialID.sec.telemetry}.

{\bf Imperfect individual identification and unknown number of marks.}
The approach
described above works only if the number of marked
individuals is known because, in that case, we can observe the all-zero
encounter histories of marked individuals and know that all augmented
individuals have to be unmarked individuals. If the number of marked
animals is unknown, on the other hand, some of the augmented
individuals may well be marked individuals we never observed. For
those individuals we should multiply $\lambda_0$ with $c$, but we
don't know who (or how many) they are. As of this moment we have not
implemented a model with unknown number of marked individuals and
imperfect identification of marks. It seems like one strategy to
tackle that problem would be to include all-zero encounter histories for all marked individuals that could potentially be in the study population (e.g., the total number of individuals that were marked before the resighting study) but that were never observed, mutliply  $\lambda_0$ for those individuals with $c$, but estimate their $z_i$ just as we do for all other augmented individuals. This way we account for the fact that there are potentially some marked individuals in the unobserved part of the population that require the correction factor on detection without the need to fix $m$. How well this approach works remains to be explores.

As long as individuals are identified based on the same type of tags
...
haven't %ourselves
experimented with it, we believe that the approach
described
above could readily be extended to account for these
...
mark-resight models, the actual locational % XXXX "locational data" or "locations"?
data collected by telemetry tags can provide detailed information on individual location and movement, and being able to incorporate this information directly into the SMR model should improve estimates of these parameters, especially when resighting information is sparse.
...
\]
% XXXX Say something about this independence assumption. People really
% like to model temporal correlation in telemetry data
...
Under the standard mark-resight assumption that marked individuals are
a representative sample of the population, the estimate of $\sigma$
can be applied for the entire population. For the unmarked individuals
...
% XXXX Same comments about previous code fragments
{\small
\begin{verbatim}
...
\begin{table}%[hb]
\centering
\caption{Summary statistics of posterior distributions %parameter
                                %estimates
