...
distribution). We can, however, work around that by separating the marked from the unmarked data. 


Alternatively, we can use the technical skills presented in \ref{chapt.mcmc} XXXX OR CHANGE TO APPENDIX XXX and derive our own MCMC algorithm. You will find that we only have to make relatively
...
contribution to our
knowledge of these parameters from the $m$ marked individuals. You can find the full MCMC code (including the modeling options we'll discuss in the following sections) in the accompanying {\bf R} package {\tt scrbook} by invoking {\tt scrPID}.
...
%   scrPID.tel}. 
Note that not all marked individuals need to be
