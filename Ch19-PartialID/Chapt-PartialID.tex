...
So far, % XXXX RC: suggest "Throughout most of this book".. since the
        % previous chapter is an exception
we have dealt with the situation where all detected
individuals are
identifiable
upon encounter because they carry some form of individual mark,
and in Chapt. \ref{chapt.scr-unmarked} % RC: Suggest new sentence to
                                % highten contrast
...
portion remains unidentified. % XXXX or "unidentifiable"?
...
%new individuals are marked on every % XXXX verb tenses don't agree
%re-capture occasion.
...
%%Above is where I edited a couple sentences, you can make sure it sounds ok
...
% XXXX RC: Rahel, I think a key point to make is that marked
% individuals will not follow a homoegeneous point process because
% their density will decrease with distance from the traps. This makes
% it challenging to fit these models, but we have 2 solutions. (1)
% Either randomly mark guys within some region B \in S (which may not
% be possible in practice), or (2) fit an inhomogeneous model for the
% marked guys. This could be done using the methods in Ch11, or if the
% marginal point process (for marked and unmarked guys) is
% homogeneous, then we can exploit that to fit the model more
% easily. I'm sure you get into this later in the chapter, but I think
% it is a big deal so deserves some space in the intro.
...
{\flushleft \bf (1) Known number of marked individuals:}
...
{\flushleft \bf (3) Unknown marked status:}
...
%do you mean third scenario - ie., this one?
 part of the
...
two model components: one describing the resighting process of
marked individuals and one describing the number of unmarked individuals observed.
The resighting process of marked individuals can use either
a Poisson or a Bernoulli observation model, depending on whether sampling is with or without replacement,
and the resighting probabilities can have both fixed effects to model
...
The process describing the number of unmarked individuals observed (or,
...
sampling occasion, for example, a year or a season) which are
...
\]
For a
single-season study, the $t$ subscript does not need to be included.
...
individual.  This is the mean or expected value of the underlying
observation model:
\begin{equation}
...
In the SMR framework, the random sampling assumption is no longer
required. Instead, the requirement is that the the distriubtion of the
marked individuals' activity centers can be described using a spatial
...
distance from the traps. Developing a suitable point process model to
describe such departure from uniformity is one of the primary
challenges when fitting SMR models. However, we first contemplate two
cases where a homogeneous point process model for the marked
individuals may be reasonable.
...
may be uniform in $\cal S$. In this case we can simply frame the
...
subset % XXXX RC: I think we need to make the point that they don't
       % need to be a random subet in the SMR case.
of the population in %of
...
Then, %in genereral it will be impossible to randomly sample
...
As it turns out this is not a trivial problem, but we provide some ideas to approach this problem in later sections of this chapter. For now, we will go on developing SMR models assuming that the marked animals are, indeed, a random sample of the entire population in ${\cal S}$. % XXXX RC: Rather than proceed with this unrealistic assumption, I suggest stating that we we will proceed by assuming that we have chosen an adequate point process model for the marked guys.
...
population % XXXX RC: suggest adding "within $\matchal{S}$"
...
%%You call the $j$ traps as resighting locations above, would it makes sense to stick with that terminology?
...
$n_{jk}$.
% XXXX RC: I think the "index vector" notation is more of an R thing,
% than something you see in the stats literature. If you order the
% indviduals such that the m guys come first, then the notation can be
% simplified a bit. Plus, we ususally reserve "u" for the location of
% a guy... at least in the search-encounter context
...
\]
%While
Whereas unmarked individuals %in the non-spatial mark-resight analysis known individuals
...
distribution). % XXXX RC: I think this sentence will only make sense
               % after introducing the dsum() distribution. Then, you
               % could just say that the arguments of dsum() must all
               % be random variables (no observed data)
...
You can look at the two sets of output invoking {\tt summary(out)} for the {\bf JAGS} analysis and {\tt summary(window(mcmc(out2),start=801))} for the custom MCMC algorithm, excluding the first 800 iterations (analogous to the adaptive phase in {\bf JAGS}). % XXXX RC: The adaptive phase in JAGS is trying to find optial tuning values. I would change this sentence to say that you are discarding burn-in iterations
We summarized the results in Table \ref{partialID.tab.knownm}.
%We see that there is some positive bias in estimates of $N$,
The posterior mean is slightly higher than the data-generating value of $N=80$, but it falls comfortably within the credible intervals. % overlap the true value (80).
...
You will find that sometimes, {\bf JAGS} produces an error message, upon trying to compile the model, saying that some of the observed $y$ are inconsistent with parent nodes at initialization. We have mentioned before that  {\bf JAGS} cannot always auto-generate acceptable %is very sensitive to
...
% XXXX Okay, but note that your results will depend on your choice of the state-space.
...
\end{figure}
%Goose pic is adorable, just walking across a parking lot!
...
% XXXX RC: Captialize words in section headings
...
%%XXX Is this table directly from Chandler Royle paper?  If so, we should say that in the caption.
\begin{table}[ht]
\centering
...
So how could we combine resighting data and telemetry data in a unified mark-resight model? Recall that the basic SCR model underlying all the SMR models we discuss here uses a Gaussian kernel %(or `half-normal')
...
% XXXX RC: I would add something like "alternatively, movement models could be used that acknowledge the temporal correlation in location data (cite a bunch of the recent movement papers and teh search-encounter chapter, where Andy added a few ideas.
...
% XXXX RC: Should emphasize that the telemetry data need to be independent of the capture data.
...
% XXXX RC: This last sentence doesn't seem to get at the key point, i.e. that the marked guys can almost never be assumed to be uniformly distributed in S.
...
A fundamental assumption of SMR models is that marked animals are a random sample from ${\cal S}$. % XXXX RC: I don't think this is true. I would replace all instances of this in the chapter by saying that we will almost always need an IPP for the marked and unmarked guys.
