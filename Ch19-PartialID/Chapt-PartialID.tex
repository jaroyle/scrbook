...
% delete "and recognizable" here? This has not previously been
% discussed so it is slightly confusing here
detection of marked 
%XXXXX and recognizable XXXXXX individuals 
...
%% good point below about spatial distribution also!
...
% Andy sez:
%%% The next paragraph seems out of place with the 1st one which
%%% started "Initially..... in 2001......" and now below you are
%%% saying "A more sophisticated suite was developed in the
%%% 1980s. Seems like you are referring to 2001, but I know you are
%%% referring to Lincoln-Petersen.....
...
clearly defined.
%%% Andy sez: I added this last sentence here so as to get people's
%%% attention on the point (which was well-made)
...
% XXX Andy sez: ok, they don't need to be -- but they can be right? (I
% mean, from a modeling stand-point, this make life easier?)
...
area. % XXXX RC: I don't think this latter idea would work because the
      % point process for the marked guys is still not homogeneous
%% Andy sez: hmm, I dunno. ;)
...
%%% XXX Andy sez: Point of next paragraph is mark-resighting studies
%%% INDUCE heterogeneity in the point process intensity. And so this
%%% has to be accounted for in the subsequent modeling. Is that right?
%%% I wonder if that could either be the lead-off sentence or the closer?
...
population in ${\cal
  S}$. % XXXX RC: Rather than proceed with this unrealistic assumption, I suggest stating that we we will proceed by assuming that we have chosen an adequate point process model for the marked guys.


\section{Known number of marked individuals}

We begin the model development with the simplest situation. Here, a
%%% XX below do you have to say "representative"? I mean, you really
%%% mean "random" right?
...
%% XXX Andy sez: I moved the mathbf close below to just include y. Ok?
...
distribution).  We can, however, work around that by separating the
...
%%%% andy : I added the phrase below and started a new sentence "In other words
...
underestimating %%detection/trap 
%% XXXXX is it ok to just say "encounter rate" here?
...
%%% XXXX I added "a plausible model" below
(a plausible
model when sampling
...
Tab. \ref{partialID.tab.sim2}.  We see that if we update $\lambda_0$
...
estimates (Tab. \ref{partialID.tab.sim2}). Thus, unmarked individuals
...
%%% XXXXX Andy sez:  I added a {\bm } to the \Sigma stuff to reflect
%%% that it is a matrix. cool?
...
Tab. \ref{partialID.tab.raccoons}.
...
random sample from ${\cal
  S}$. % XXXX RC: I don't think this is true. I would replace all instances of this in the chapter by saying that we will almost always need an IPP for the marked and unmarked guys.
%%% XXX andy: right -- its not necessarily true, but helpful. I agree
%%% with Richard that a modification of this statement is in
%%% order. Its not an assumption that is necessary, but one that is
%%% convenient.  More general formulations seeem called for, which is
