...
A broad class of spatial capture-recapture models can be represented
as a generalized linear model (GLM) with an individual random effect
(Royle et al. 2009; Royle and Gardner 2009)
representing a latent spatial attribute of individuals, interpretable loosely
 as the center of activity or home range center (Efford 2004; Borchers
and Efford 2008; Royle and Young 2008).
This formulation makes analysis of the models readily
...
Here we use these classical methods for constructing optimal designs
to obtain the configuration of traps
(or sampling devices) in some region (the design space, ${\cal X}$), 
that minimizes some appropriate function the variance of the MLEs of the
detection model parameters based on the asymptotic variance-covariance
matrix. Existing theory (Sanathanan 1972) suggests that such designs
...
counts , i.e., just a GLM
 with
\[
g( E[y]  ) =  alpha0  + alpha1*dist(s,x)
\]
Lets think about this in the context of a normal linear model then:    
\[
{\bf y} = {\bf M}'\alpha + error
\]
We could analyze the design problem for the binomial or Poisson case
but I think we can get by just looking at the Normal case which is
what I'm going to do here to clarify a couple of thoughts:
The variance-covariance matrix of ${\bm \alpha}$ is
\[
 Var( {\bm \alpha}) = ({\bold M}(s)'{\bold M}(s))^{-1}
\]
Therefore 
if we know ${\bf s}$ we could now easily find the set of "x"
coordinates that minimize functions of the variance, whatever function
we want. (I didn't make the dependence on the design explicit but I
should!).

If we don't know ${\bf s}$ we might as well minimize the expected variance:
\[
  E[var(alpha)) = sum_{s \in {\cal S}}  solve(M'(s)M(s))  
\]
This is really easy to do for any number of design points x[1], ...,
x[J] using imperfect exchange algorithms which always improve the
...
In fact, it is not sufficient to make a design that is optimal for estimating regression parameters -- we also want to produce a low variance for estimating N.   since $n \sim Bin(N,pbar)$  we want n to be as close to N as possible, generally speaking.  This suggests that we should find a design that maximizes "pbar" -- i.e., generates the highest expected sample size.
...
it looks as if we drive the 2nd term to 0, except that $\par{p}$ {\it
