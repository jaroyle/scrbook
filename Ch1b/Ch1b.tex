...
\end{equation}
Earlier we approximted the expected value of the binomial distribution
...
\begin{table}[t!]
  ...
  \caption{Common probability density functions (pdfs) and probability
    mass functions (pmfs) for modeling random variables.}
  \begin{tabular}[t]{lccccc}
    ...
    Normal & $x \sim N(\mu, \sigma^2)$ & $\frac{1}{\sigma\sqrt{2\pi}}
      \exp(-\frac{(x-\mu)^2}{2\sigma^2}$ & $\mu$ & $\sigma^2$ & \verb+dnorm+ \\
    \hline
  \end{tabular}
