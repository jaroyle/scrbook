...
%processes. \hl{Define these terms}. In the absence of stochasticity, we could develop
...
To clarify the concept of a random variable, let $X$ be the number of American shad (\emph{Alosa
  sapidissima}) caught after $K=20$ casts at the secret shad
hole on Deerfield River in Massachusetts. Suppose that we had a good
day and caught $x=7$ fish. If there were no random variation at play,
we would assume the true probability of catching a fish $p$ is
0.35 and we would
always catch 7 shad after 20 casts. This would be an example of a
deterministic model, but we can be sure that it would not be a good
model because for predicting the number of fish that we might catch the
next time we returned to the hole. For this purpose, we need a model
that acknowledges uncertainty (i.e., stochasticity), and specifically
...
$\text{Pr}(X=x|K,p)$ expressed as $f(x|K,p)$ or $p(x|K,p)$ or
$\pi(x|K,p)$ or $\mathbb{P}(x|K,p)$ or $[x|K,p]$ or even just $[x]$!
Just remember that these expressions all have the same meaning. We
will try to use bracket notation throughout because it is the most
concise.
...
Bayesian methods.
%In \R, we can generate a single quasi-random outcome from this
%model, using the command \verb+rbinom(1, 20, 0.5)+. Similarly, the command
%\verb+dbinom(7, 20, 0.5)+ returns
%the probability of observing our data, $Pr(X=7 | N=20, p=0.5)$.

Now that we have specified our model, we can use it to make
probability statments about future outcomes. Continuing with our fish example, we might want
to know what the probability of catching $x=7$ again on a future
fishing outing assuming that we
know $p=0.35$. The answer is found by evaluating the binomial pmf,
which is shown in Table~\ref{modeling.tab.pdfs}. In \R~, we can simply
use the \verb+dbinom+ function,
\begin{verbatim}
> dbinom(7, 20, 0.35)
[1] 0.1844012
\end{verbatim}
...
\begin{comment}
If we are a frequentist, the only source of
...
number of replicate samples is very large (100000). \hl{rbinom not
  introduced yet...need to bring up material from below}
\begin{verbatim}
N <- 20
p <- 0.5
x <- rbinom(100000, N, p)
...
\subsection{Properties of Probability Distribution Functions}
...
requirements. The first is that the function
must be non-negative for all realized values of the random variable
$X$, $[x] \geq 0$. The second requirement is that $\int_{-\infty}^{\infty} [x] = 1$ for
continuous random variables, and $\sum_x [x] = 1$ for discrete random
variables. The following \R~code demonstrates the latter property
for the normal and binomial distributions:
\begin{verbatim}
