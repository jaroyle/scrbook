...
$\text{Pr}(X=x)$. An approach for acheiving this goal is to develop a
...
the secret shad hole on Deerfield River in Massachusetts. Suppose that
we had a good day and caught $x=7$ fish. If there were no random
variation at play, we would say that probability of catching a
...
meaning---they are all probability density functions. We will try to
use bracket notation (the last two examples)
throughout because it is the most concise.

In cases where $x$ is continuous, we refer to
$[x]$ as a probability density functions (pdf), or when
$x$ is discrete, we call $[x]$ a probability mass functions (pmf). In
parametric inference, we assume a particular distribution for $x$, which in
...
binomial size parameter. The $\sim$ symbol is meant to represent a
stochastic relationship, and can be read ``is distributed as.'' We
will use this notation to complement the bracket notation, which will
...
\text{Poisson}(Kp)$. The other reason for the $\sim$ notation is that
it resembles the syntax of the \bugs~language, which we will use to
fit models using Bayesian methods.

Now that we have specified our model as $x \sim \text{Binomial}(K,p)$,
...
assuming that we know $p=0.35$. The answer is found by evaluating the
binomial pmf, which is shown in Table~\ref{modeling.tab.pdfs}. In \R~,
we can simply use the \verb+dbinom+ function,
\begin{verbatim}
> dbinom(7, 20, 0.35)
[1] 0.1844012
\end{verbatim}
Indeed, by definition, we can evaluate the probability of observing
any $x$ given $K=20$ and $p=.35$. This probability mass distribution
can be visualized using this bit of code:
\begin{verbatim}
...
essential to in process, and thus we need to
...
As previously stated, probability distributions are defined by
probability density (or mass) functions. These functions are like any
othter functions in the sense that they have one or more arguments
whose values determine the
result(s) of the function. However, probability functions have a few
properties that distinguish them from other functions.
% We have introduced random variables and the the key role that
% probability distribution (density or mass) functions
% play in statistical inference. Now, we will
% describe the basic properties of probability distribution functions,
% beginning with the two fundamental
% requirements.
...
a pdf must be unity, $\int_{-\infty}^{\infty} [x] = 1$, for a
continuous random variable, and similarly,
summing over all possible values of pmf results in unity, $\sum_x [x]
= 1$ for discrete random variables. The following \R~code demonstrates
...
A probability distribution can often characterized in terms of its
expected values and variance. The expected value,
which we will denote $\mathbb{E}[X]$, can be thought of as the average
...
thus we don't need to resort to such approximations. For instance, the
expected value of the binomial distribution is $\mathbb{E}[x] = Kp =
20 \times 0.35 = 7$. In this case, it happens to take an integer
value, but such is not necessary, even for discrete random
variables.
...
other distributions, the variance is not an explicity parameter to be
...
variance. This is often viewed as a restriction because in ecology
...
determined by the two parameters $K$ and $p$, $Var(X) = Kp(1-p)$. Thus
...
\begin{table}[t!]
  ...
  \begin{tabular}[t]{lccccc}
    ...
    & \R~function \\
    \hline
    \multicolumn{6}{c}{Discrete random variables} \\
    ...
    & $N\pi_k(1-\pi_k)$& \verb+dmultinom+ \\
    \multicolumn{6}{c}{Continuous random variables} \\
...
%     &  \\
    ...
    (1-x)^{b-1}$ & $a/(a+b)$ & $\frac{ab}{(a+b)^2(a+b+1)}$& \verb+dbeta+ \\
    Gamma & $x \sim \text{Gamma}(a,b)$ &
    $\frac{b^a}{\Gamma(a)}x^{a-1}\exp(-bx)$ & $a/b$ & $a/b^2$ & \verb+dgamma+ \\
    \hline
  \end{tabular}
...
So far we have focused our attention on a single random variable,
...
marginal, and conditional distributions. The
conditional distribution is the distribution of one variable, given a
realized value of the other. In the case of two discrete random
...
write this as $[X|Y]$. Continuing with our
previous example where $X$ was the number of shad caught after 20
casts, now supposed that $X$ depends on $Y$, the number of fisherman
competing at the hole. In such a case, we might expect that the
probability of $x$ being high, decreases as $y$ increases. In other
cases, we might ask questions like what is the expected number of birds likely
to be captured in a mist-net \textit{given} that there are 10
individuals in the population?
%, i.e. ``the probability
%distribution of $X$ given $Y$.'' We might also want to know the
%distribution of $X$ \textit{un}conditional on $Y$, $[X]$, i.e.
%the marginal distribution.
Conditional distributions are defined as follows:
\begin{equation*}
  [X|Y] = \frac{[X,Y]}{[Y]} \qquad [Y|X] = \frac{[X,Y]}{[X]}
\end{equation*}

[Here is an example in R. Suppose the number of fisherman is Poisson
distributed with mean...]

Computing marginal distributions requires . In the case of continuous
random variables we do this:
\begin{equation*}
  [X] = \int_{-\infty}^\infty [X|Y] \, \mathrm{d}Y \qquad
  [Y] = \int_{-\infty}^\infty [Y|X] \, \mathrm{d}X
\end{equation*}
and for discrete random variables we replace the integral with a
summation over all possible values of the
\begin{equation*}
  [X] = \sum_Y [X|Y] \qquad
  [Y] = \sum_X [Y|X]
\end{equation*}

This is important because when we maximize likelihoods with random
effects, we always need to compute the marginal likelihood by
integrating over the prior distribution.

Earlier, we discussed an
example where $X$ was the number of fish caught after 20 casts. But
what if $X$ depends upon $Y$, the number of other fisherman at the
hole on that day?
...
We have already briefly introduced the binomial distribution and
elaborate on it here because it plays a critical role
in ecology---being
...
would recognize that the expected value of $x_j$ is $10 \times 0.5 = 5$. That is,
...
function (pmf) for the binomial distribution. The probability of
...
REFER TO ESTIMATION SECTION HERE
...
If $N$ individuals are uniformly distributed in some area $A$, and $N$ is a
