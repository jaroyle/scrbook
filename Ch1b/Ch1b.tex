...
0.35 and we would always catch 7 shad after 20 casts. This is almost
certainly not true, and thus if we want to know how many fish we might catch the
next time we returned to the hole, we need a model that describes the
probability of catching $x$ fish given $K$ and $p$,
$\text{Pr}(X=x|K,p)$. Sometimes, we simply this expression using bracket
