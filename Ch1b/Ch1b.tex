...
variables and the interactions between them. Earlier, we discussed an
...
hole on that day? When $X$ depends upon $Y$, we need to think about
the \textit{conditional distribution} $[X|Y]$, read ``the probability
distribution of $X$ given $Y$.'' We might also want to know the
distribution of $X$ \textit{un}conditional on $Y$, $[X]$, which is
