...
It might be helpful to suffer through a couple examples
building custom MCMC algorithms. Here, we develop an MCMC algorithm
for 
...
alpha.cand<-rnorm(1,alpha,.25)         # generate candidate
...
\begin{figure}
\begin{center}
\includegraphics[height=3in,width=4in]{figs/poissonmcmc}
\end{center}
\caption{MCMC output for Poisson regression parameters (top trace:
  intercept $\alpha$; bottom trace: slope $\beta$). This is for
  $\delta = 0.25$. 
}
\label{glms.fig.poissonmcmc}
\end{figure}


The first 300 iterations of the MCMC history of each parameter 
is shown in Fig. \ref{glms.fig.poissonmcmc}. The appearance of this is
not very appealing but a couple of things are evident: First, the
Markov chains clearly stabilize - ``burn-in'' --  after about 60 or
70 iterations. They also appear to mix very slowly once convergence is
achieved, although this is not so clear given the scale of the
$y$-axis.
We decreased  the standard deviation
of the candidate generating distribution from
$\delta = 0.25$ to $\delta = 0.05$ and re-ran 
re-ran the MCMC algorithm producing the output shown in
Fig. \ref{glms.fig.poissonmcmc2}. We see 
that the burn-in takes longer but it seems to mix better although it
takes slightly longer to burn-in.
Using this value of $\delta$ we generated 10,000 posterior samples,
discarding the first 500 as burn-in, and the result is shown in
Fig. \ref{glms.fig.poissonmcmc3}, this time seperate panels for each
parameter. 
The ``grassy''
...
\begin{figure}
\begin{center}
\includegraphics[height=3in,width=4in]{figs/poissonmcmc2}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[height=4in,width=5in]{figs/poissonmcmc3}
\end{center}
\caption{nice grassy mcmc output, longer run of previous with $\delta
  = 0.05$.}
\label{glms.fig.poissonmcmc3}
\end{figure}
...
term is important. Small values lead to much
better mixing but it should be noted that values that are too small
will slow down burn-in and also lead to high correlation.
...
contemplate finding that optimal value for this
problem\footnote{effective sample size definition?} 
(3) For the flat normal prior distributions
