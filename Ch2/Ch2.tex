...
%%% 7. FIX parameter notation! I have beta0 beta1 , alpha beta, and a, DONE
%%%     b in the same chapter!   Use alpha beta probably?
%%% 8. spell check this document

%%% NEW STUFF TO DO
%%% Andy, add followind data/functions to REPO: BBS data (add as data object? or as .csv?), pahabdata.csv, R code for mcmc for Poisson regression, mallard data
%%% see question in sec. 3.4 (small sample size)
%%% missing ref: link_barker:2009; could not find it in ref list
%%% around line 1000 - which section in the closed pop chapter do you want to reference?
% XXX Should DIC/selection/FIT material go into Chapter 8? XXXXXXX

%%%NOTATION PROBLEM: IN MCMC CHAPTER WE GENERATE theta(t) from theta(t-1), here we generate theta(*) from theta(t) - which one do you prefer?
...
%%% redo BBS analysis without fit statistic
%%% make mcmc algorithm a panel?
%%% Fig 3.5 and 3.6 - add parameter names
...
%%% in line 2211, mallard example, are t=5 observations? So does p also vary with time? Which geographic region do these data come from?
...
parameters. \begin{comment}Andy, I moved the footnote up bc it looked
  like 'to the 4th power' after the formula'\end{comment}
...
the resulting output. Close the {\bf WinBUGS} GUI and the data will be
...
1966-1998. BBS data can be obtained online at \mbox{\tt http:\//\//www.pwrc.usgs.gov\//bbs\//}, but the particulart chunk of data we will be using here is also included in the {\tt scrbook} package ({\tt pa_bbsdovedata}).
...
{\small
\begin{verbatim}
data(pa_bbsdovedata) #loads data frame 'bbs'
bbs[1:2,1:6]
...
\begin{verbatim}
data(pa_bbsdovedata)
y<-bbs[,29]  # pick out 1990
notna<-!is.na(y)
y<-y[notna]
spatial.plot(bbs[notna,2:3],y)
\end{verbatim}
}
...
\end{center}
\caption{Plot of forest cover in the vicinity of BBS survey routes in Pennsylvania.}
...
\end{center}
\caption{First 300 MCMC iterations for the Poisson GLM model parameters $\beta_0$ and $\beta_1$ using
...
\end{center}
\caption{Nice grassy plots of 10,000 MCMC iterations for the Poisson GLM model parameters $\beta_0$ and $\beta_1$ using
a Metropolis-Hastings tuning parameter of
 $\delta = 0.05$.}
...
For these data, $y_i$ is the number of mallard ({\it Anas platyrhynchos}) bands recovered out
...
(including an interaction term). The data ({\tt mallarddata}) are provided with the {\bf
...
\end{center}
\caption{Predicted recovery rates of mallard bands across XXX. Note the negative east-west gradient.}
