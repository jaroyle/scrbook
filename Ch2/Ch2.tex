...
\citet{royle_dorazio:2008}, and \citet{kery:2010}. %cite the GLM book?
Indeed, likelihood
...
accessible treatments of considerable merit. %Again suggest citing the 
%GLM book, and/or GAM book.  
...
$.25*.8/(.25*.8 + .2) = 0.50$. 
%maybe write this as:  $(1-.5)^2*.8/((1-.5)^2*.8 + (1-.8) = 0.50$. just
%so the reader easily follows from the eqn.
...
referred to as the ``likelihood'').  Suppose $[\theta]$ %remove brackets?
is a parameter of
