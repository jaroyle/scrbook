...
\mbox{Normal}(\alpha + \beta (x-\bar{x}) , \sigma^{2})$\footnote{We center the 
independent variable here so that things look more intuitive in the result}
where lets say $\sigma^{2}$ is known.
The full conditionals are, in symbolic terms,
\[
[\alpha|y,\beta]
\]
 and
...
or 
\mbox{\tt rjags} \citep{plummer:2009} pacages. 
