...
\caption{
These are weasels. Cute, but blood-thirsty weasels.
}
\label{fig.weasels}
\end{figure}
...
spurred on by the advent of new technologies of DNA sampling, camera
trapping, and acoustic sampling
capture-recapture is more relevant and widely used than ever before
and for species that could not be studied efficiently even a few years
ago. And for some species that are just difficult to capture or handle including
...
In currently developing work, we (Brian Reich, NCSU - lead author) propose a model that accounts for
