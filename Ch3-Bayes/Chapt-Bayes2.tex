...
%XXX Andy - I think this is a bit much if the reader does not know
%occupancy modeling well.  We at least need to state something like:
%The probability of not dectecting the species (Pr(\mbox{not detected}))
%is composed of two parts - not observing the species given that it
%is there ($(1-p)^{K} \psi$) or the species is not present ($1-\psi$).
...
%%XXX Andy - what things look more intuitive?  Footnote is confusing.
...
symbolic terms, and using ``bracket notation'',
\[
[\beta_0|y,\beta_1]
\]
 and
...
%% XXX pdf and pmf are not defined in this chapter, do we need to do that?
...
%%XXX h() can be anything or any function?
...
App. 1]{kery_schaub:2011} for more on practical analysis in {\bf
  WinBUGS}.  
%% XXX  Andy - is App. 1 the Appendix?  Write it out maybe.
...
current {\bf R} working directory (note that sometimes you will need to specify the place where you installed {\bf WinBUGS} within the \mbox{\tt bugs()} call, using the \mbox{\tt bugs.directory} argument).
 ...
 %XXX  Andy - I change the data below (y=y is more general)
...
populations. 
% depending on their migration ecology. 
As such, we fit a
