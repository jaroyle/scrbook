...
nights)
 producing encounters of $n$ individuals.  
Because sampling
...
%sampling \citep{efford_etal:2009ecol})
%
...
same currency. 
%Unfortunately, we have found that people take
...
% so that we can directly apply to this situation what we learned
%in Chapt. \ref{chapt.glms}.
...
%produce this analysis for the logistic and half-normal models. 
The
...
% (note, we have a parameter $\sigma$ which we discuss
%shortly)
\begin{comment} \footnote{Final as of 1/11/2012. 
...
One of the key issues in using ordinary capture recapture  models
which we've brought up over and over again is this issue that the
area which is sampled by a trapping array is unknown -- in other
words, the $N$ that is estimated by capture-recapture models does not
have an explicit region of space associated with it.
Classically this has been addressed in the ad hoc way of prescribing
an area that contains the trap array usually by adding a buffer of
some width, which is not estimated as part of the capture-recapture
model.
In SCR models we avoid the problem of not having an explicit linkage
between N and ``area'', by prescribing explicitly the area within
which 
the underlying point process is deffined -- the state-space of the
point process.
This state-space is {\it not} the ``effective sampled area'' -- it is
desireably that it be somewhat arbitrarily larger than in the sense
that individuals at the edge of the state-space have no probability of
being captured, but as part of the SCR model we don't try to estimate
or otherwise characterize the ESA explicitlly.

However, it is possible to provide a characterization of effective
sample area under any SCR model. [have to note shortly that it is
impacted by definition of the state-space].  This is directly
analogous to the calculation of ``effective strip width'' in distance
sampling (REF FOR THIS?)


The conceptual definition of ESA follows from making the following equivalence:
\[
 D = N/A   = n/ESA 
\]
In other words, ratio of $N$ to the area of the state-space should be
equal to the ratio of the observed sample size $n$ to some number
ESA. Both of these should equal density. 
So to compute ESA for a model we 
just plug-in E[n] into the equation to get: 
\[
 ESA = E[n]/D 
\]
Imagine our habitat mask for the wolverine data, or the bins we just
used to produce a density map, then we can write $E[n]$ according to:
\[
E[n] = \sum_{s}  Pr(capture | s) E(  N(s)  ) 
\]
...
$E(N(s))$ is the expected population size at pixel s  which is the
density times the area of the pixel, i.e., $\mathbb{E}(N(s))= D*a$.
Therefore
\[
 \mathbb{E}(n) = D*a* \sum_{s} \Pr(capture | s)
\]
and (plugging this into the expression above for ESA)
\[
 ESA = \frac{D*a*\sum_{s} \Pr(capture | s) }{D}
\]
We see that $D$ cancels and we have
\[
  ESA = a*\sum_{s} \Pr(capture | s) 
\]
So what you have to do here is substitute in $\Pr(capture | s)$ and just sum them up over all pixels.  For the Bernoulli model of hair-snares 
\[
\Pr(capture|s) =    1-(1-p(s))^{K}
\]
with slight modifications when encounter probability depends on
covariates.


For the wolverine data we did this using........
...
%$M_{h}$'' \citep{otis_etal:1978, dorazio_royle:2003}.
...
%bout the individual effect which is completely latent in model
%$M_{h}$. If the state-space of the random effect ${\bf s}$ is discrete,
%the SCR model resembles more closely the finite-mixture 
%eterogeneity models \citep{norris_pollock:1996} which parameterizes
...
%locations where individuals are captured.  
