...
In some problems, there will be interest in distinguishing between 3
distinct estimates of population size or density: (1)
the size of {\it the} specific population under study; (2) the size of
some specific population that we didn't study; (3) Less often we care
about the expected population size for some population.
[some of this is covered by
 \citep{efford_fewster:2012}]. Efford and Fewster called (1) the
 realized population size and they also used the term expected
 population size in an ambiguous manner. 

Imagine 2 populations of tigers in 2 reserves. You do a camera
trapping study on one of the reserves to estimate $N_{1}$ and you
think the reserves are similar and homoegeneous so you're willing to
apply a density estimate based on $N_{1}$ to the 2nd reserve. 


that we're studying, then we probably want to make an inference about
$N$, the realized population size. If, on the other hand, we wish to
make a guess at the size of some population that wasn't sampled, then
we probably want an estimate of $\mathbb{E}(N)$ for that population,
possibly scaled by area. Thus, the same distinction between realized
and expected value applies to {\it density} as well, although it is
not usually thought of as such. Realized density
is $N/\mbox{area}$ whereas the expected density is
$\mathbb{E}(N)/\mbox{area}$. 

In the model we've developed so far, we suppose that our population
size is $N$ and this is meant to be the realized population size --
i.e., of
the population that was sampled. 

Think of it this way: Suppose $N \sim Poisson(\lambda)$ and $\lambda$
is known with certainty.
If we have some patch of landscape with area $A$ then our best guess
at the expected number of guys out there is $A\lambda$ and there is no
variation in that at all. If we have estimated $\lambda$ somehow then
ther eis some estimation error involved.   On the other hand suppose
we really care about some realized population $N$ on a reserve that we
never sampled but we suppose our estimate of $\lambda$ applies to that
reserve. Then we care about predicting a new $N$, say $N_{new}$ just
to be clear, and there is extra variation in that prediction.  In
particular, $Var(N_{new}) = Var(N_{new} | \lambda) + Var(\lambda)$
Var(E Nnew|lambda) + E Var Nnew|lambda = 
Var(\hat{lambda}) + \hat{\lambda}
### need to put an A in there. 

Now suppose we care about a population that we just sampled, then N =
n + n0 and because we observe a component of that population (n) there
is no variation about that, and we only have variation about the n0 part.
The variance of this realized N should be less than the variation of
predicting a new N
 because a portion of the population is observed and,
