...
% XXXX Mention here that in Ch11 we will model inhomogeneous PPs XXXX
...
% XXXX Note that the uniformity assumption is equivalent to what is called
% an homogeneous point process XXXX
...
representation of the state-space as a discrete set of points (sec. % XXXX mention that this is what secr does XXXX
...
consideration, we note that $area({\cal S})$ % XXXX suggest using
                                % words instead of area(S) XXXX
affects data
...
\[
	y_{ij} \sim \mbox{Binomial}(K, z_{i}*p_{ij}) % XXXX Sometimes "*" sometimes not XXX
\]
...
session or year. % XXXX Define session XXXX
