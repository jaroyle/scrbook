...
should be related to a basic model of space usage, this has not been
...
which is the population size that was sampled. In the context of
\citet{efford_fewster:2012} this is the {\it realized} population
size. Conversely, sometimes we see estimates of {\it expected}
population size reported, which is $E(N)$, although the distinction is
not usually made explicitly in applications of SCR models. 
In fact, there is a third type of inference objective that is relevant
in pratice. To elaborate on this, consider a simple situation in which
we are concerned about the tiger population in 2 distinct reserves in
India.
We do a camera trapping
study on one of the reserves to estimate $N_{1}$ and you
think the reserves are similar and homoegeneous so you're willing to
apply a density estimate based on $N_{1}$ to the 2nd reserve. So lets
do this $D_{1} = N_{1}/A_{1}$ and then $N_{2} = D_{1}*A_{2}$ should be
an estimate of the realized population size for reserve 2. If we do
this all as part of the MCMC algorithm then our shit should be tight, bro.

I read the Efford and Fewster paper and I find this to be difficult to
read. Frequentists have a tough time with the idea of predicting
random variables and this is a good illustration of that.  In my mind
there are 3 basic inference problems:
\begin{itemize} 
\item[(1)] What is the value of N for some population that was
  sampled.  In this case, it is necessary that you account for the
  fact that part of N was observed.  This is what Efford and Fewster
  call "realized N"
\item[(2)] You need to estimate N for some population that you didn't
  ...
  stochastic model of N).
    ...
    As near as I can tell, Efford and Fewster did not talk about this problem.
  \item[(3)] In some extremely limited cases (but probably none, in
    reality) you might care about estimating the expected value of N,
    E[N] = lambda or psi*M under data augmentation.  This is only
    useful as a point estimator of N and doesn't have any information
    about uncertainty in N. It is not useful, ever, from a practical
    stand point.  Anyhow, this is what Efford and Fewster call
    "Expected population size".
\end{itemize} 
 
If you do likelihood analysis then distinctions between different
inference problems are often discussed as whether they're
``conditional on $N$'' (realized) or not (expected).  The argument
goes something like this: If you obtian an MLE of $N$, say $\hat{N}$,
then the inference procedure tell us the variance of this {\it
  conditional} on $N$. i.e., $Var(\hat{N}|N)$. This is fine, if we
...
really an important conceptual one.  
	
If you do Bayesian analysis then none of this really matters. You
...
$N/area$ where $N$ is the realized population -- we understand it to
be that, or else we would put an expectation operator around it like
$\mathbb{E}(N)/area$.
In any case,  the formula for this is slightly different depending on
...
E[N] = D*A
\]
so what we think of as ``density'', $D$, is $ D = M \psi /Area$.
