...
{\small
\begin{verbatim}
> hra(pGauss1,parms=c(-2,.3993),plot=FALSE,xlim,ylim,ng=500,target.area=3.00735,tol=.0005)
...
\begin{figure}[ht]
\begin{center}
\includegraphics[width=6in,height=2.14in]{Ch4-SCR0/figs/Mh_buffer}
\end{center}
...
y<-data$Y
traplocs<-data$traplocs
nind<-nrow(y) # in this case nind=N because we're doing the known-N problem
X<-data$traplocs
...
analyses by running the \mbox{\tt simSCR0.fn} script provided in the
...
\[
	y_{ij} \sim \mbox{Bin}(K, z_{i}*p_{ij})
\]
...
This analysis can be run directly once \mbox{\tt scrbook} is loaded,
...
\begin{verbatim}
jinit<- jags.model("SCR0a.txt", data=data, inits=inits, n.chains=nc, n.adapt=nb))
jout <- coda.samples(jinit, parameters, n.iter=ni-nb, thin=nthin)
\end{verbatim}
...
\end{center}
\caption{
Wolverine camera trap locations (black dots) from
\citet{magoun_etal:2011}.
The study took place in SE Alaska (inset).
}
...
an efficient file format which is easily manipulated. The file
...
in the {\bf SPACECAP} \citep{gopalaswamy_etal:2012}, and
\mbox{\tt SCRbayes} \citep{russell_etal:2012} packages, both of which 
have a 3-column format (trapID, indID, sampID).
...
with optional arguments \mbox{\tt ntraps} and \mbox{\tt nsamples}, and
...
{\bf XXXXX 2nd line is wrong b/c format changed XXXXXXXX}
{\small
\begin{verbatim}
...
XXX NOTE: This may differ by 1 from Royle et al. 2011 table. Check
this. XXXX
}
\centering
...
\hline \hline 
   & mean &	sd	&n.eff&	mean&	sd	&n.eff&	mean&	sd
   &n.eff&	mean&	sd&	n.eff \\ \hline
...
effective ``home range area'' is then area of this circle, which is
$\pi 15.66^2 = 770.4$ $\mbox{km}^{2}$
Using our handy function \mbox{\tt hra} we do this:
\begin{verbatim}
 hra(pGauss,parms=c(-2,1/(2*.64*.64)),xlim=c(-1,7),ylim=c(-1,7))
...
$cloglog(p_{ij}) = \alpha_{0} - (1/\sigma^2)*d_{ij}^2$, but the
estimates of $\sigma$ reporetd in their Table 2 are actually based on
the model according to $cloglog(p_{ij}) = \alpha_{0} -
...
if a wolverine is using traps as a way to get yummy chicken, so its
...
patterns. As such, we caution againnst direct biological
...
2 or 3 at the most) can explain really high levels of heterogeneity
...
burn-in, which yields 120000 total posterior samples.  Summary results
...
algorithms that often simply do not work with realistic state spaces.
...
summaries are given and run time. Results obtained  using 
{\bf WinBUGS} run from {\tt R2WinBUGS}. 
}
\begin{tabular}{crrrrrrrc}
grid  & Mean & SD &   NaiveSE & Time-seriesSE &  runtime (sec) \\
...
\caption{Comparison of the effect of pixel size on the estimated
  density surface of wolverine sin SE Alaska 2007. The 3 cases are 
...
(Fig. \ref{scr0.fig.density}) map. We can do
