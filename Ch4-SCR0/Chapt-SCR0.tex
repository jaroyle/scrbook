...
of the form:
% XXXX Again, a real table would look better XXXX
\begin{verbatim}
...
parameter naming across models, we will sometimes parameterize any encounter probability
model so that the coefficient on distance (or distance squared) is
$\alpha_{1}$. So, for the half-normal model,
 $\alpha_{1} = 1/(2\sigma^{2})$.

There are a large number of standard detection
...
The negative exponential model has $p_{ij} = p_{0}\exp(-\alpha_{1}
||{\bf s}_{i}-{\bf x}_{j}||)$
and the Gaussian hazard model has
$p_{ij} = 1-\exp(-p_{ij})$. % XXXX This isn't right is it? XXXX
\begin{figure}[ht] % XXXX capitalize axis labels? XXXX
\begin{center}
...
% XXXX I would include the definition that a home range is "the area
% used by an organism during some time period" XXXX
...
not accounted for by the basic SCR0 model:
...
like that. Our point is that we should separate statistical
