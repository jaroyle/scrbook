...
usage: XXXXXXXXX ANDY IS HERE XXXXXXXXXXXXXXXX
...
hypothetical or unsampled population. The former is usually called the
...
\subsection{Estimating where an individual lives}

A problem related to producing a density map, is producing a map of
where a specific individual's activity center is located. 
For any observed encounter history, we can easily generate a posterior
distribution of ${\bf s}_{i}$ for individual $i$ and produce a
posterior density surface of those individuals. [see Fig. XXXXXX]. 
...
individual might live, say ${\bf s}_{n+1}$ [see Fig. XXXXX].   Obviously, all such
uncaptured individuals (for $i=n+1,\ldots,N$) should have the same posterior distribution.

A few notes:
the posterior distributions of both are affected by
 traps of capture {\it and} traps of
non-capture in fairly intuitive ways. 
