...
captured in any number of traps during each occasion but (iii) xxx$here the (ii) and (iii) are unclear to me$xxxxx
...
sampling literature. Because it is the kernel of a bivariate normal probability
density function we will just refer to it as the ``(bivariate) normal model.''
There are a large number of standard detection models 
commonly used, and we consider these in
Chapt. \ref{chapt.covariates}).
Note that the bivariate normal model is equivalently represent as 
\begin{equation}
\log(p_{ij})  = \log(p_{0}) - \alpha_{1} *||{\bf s}_{i}-{\bf x}_{j}||^2
\label{scr0.eq.norm}
\end{equation}
which we make use of sometimes in specifying the model in \bugs. 
...
model is really useful for anything. xxxxxx $around here need some references$ xxxxxxxx Indeed, the idea of a home range
...
known so that we can directly apply to 
this situation what we learned in
Chapt. \ref{chapt.glms}. To do this, we note that the individual effects ${\bf
...
\]
xxxxxxx$strange sentence$ xxxxxxx And $p({\bf s}) = p(y=1|{\bf s})$ is some function of ${\bf
  s}$. Therefore, for any specific xxxxxxxxx$say what g is$ xxxxxxxxx$g(p)$ and ${\cal S}$ we can work
...
above, under the logistic model for distance xxxxxxxxxxxxxxxx$refer to e.g., 4.2.2.$xxxxxxxxxxxxx. {\bf R} code is
...
to the left as the buffer increases, implying more individuals
 with lower encounter probabilities, as their home range
...
strikingly similar to a classical distance sampling model xxxxx $add reference$xxxxxxx. Both have
...
tigers xxxxxx$repetition, might say when studying carnivores such as bears or large cats$.xxxxxxxx Furthermore, SCR models allow us to relax many of the
...
zero'' observations to that data set. This is because such
...
The column labeled ``MC error'' XXXXX this is only shown in WinBUGS? own log file, but not in the above table produced by R2WinBUGS. Apparently, this is what ?Time-series SE? and ?Naive SE? in the output from ?rjags? by JAGS means, but I never understood this before XXXXXX XXXX is this out of place? Where is first occurrence of WinBUGS output XXXXXXXXX$ is the Monte Carlo error - the error
...
XXXXXX I would stick to either the more impersonal ?the reader? as here or else to the more direct ?you?. I prefer ?you? XXXXXX
...
$this figure is way too small. Also, I would take out some of the legends in the actual figure and put this into the figure legend (e.g., what the dots mean and the grey area)$
...
in the {\bf R} package \mbox{\tt SCRbayes} \citep{russell_etal:2012} XXXXXXX $why did this come out as ?E et al.? in the pdf ?$ XXXXXXXX.
