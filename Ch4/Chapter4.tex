...
XXXX {\bf Andy is just babbling here mostly} XXXX
...
particular, if use outcomes are bivariate normal, then $||{\bf x}-{\bf
  s}||^2$ has a chi-square distribution with 2 d.f. and the constant
...
> hra(pGauss1,c(-2,.396),plot=FALSE,xlim,ylim,ng=300)
[1] 0.9730349
radius to achieve 95% of area:  0.9730349
home range area:  2.97445
[1] 2.97445
> hra(pGauss2,c(-2,.396),plot=FALSE,xlim,ylim,ng=300)
[1] 0.9827653
radius to achieve 95% of area:  0.9827653
home range area:  3.034237
[1] 3.034237
...
What this means is that $B(\alpha) = 0.973$ is the radius that
encloses about 95\% of all movements under the standard bivariate
normal encounter model.  Therefore, the area is about $\pi*.973*.973 =
2.97$ spatial units.  You can change the intercept of the model and
find that it has no affect.\footnote{ note: I don't know if that works
  ...
  perhaps).}  The radius of 0.973 compares well with the actual
(analytic) solution of 0.969 which we could improve upon if we
increase the resolution of our spatial grid (increase the \mbox{\tt
  ng} argument).
...
To demonstrate how to do this, we first define an objective function
that computes the difference between implied home range area under
some model and the desired target value. Then we find the root of that
function.

XXX need to fix this func to accept ``parms'' and pfunc
XXX change x0 to be ``Balpha''
\begin{verbatim}
...
[1] 0.3957799
\end{verbatim}
We see this is equal to 0.396 which is the value we simulated data
from above - so things seem to be working in an orderly manner.
