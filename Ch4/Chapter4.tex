...
grid cells).  But lets think about this in a perfect world, where our
detection devices were absolutely perfect and so $Pr(encounter|use) =
1$. Imagine we record perfectly every time an individual moves into a
pixel so that after a long period of time we observe an enormous
sample size of $x$ values. We tally those up into each pixel, $m(x,s)$
and this forms an estimate of the use distribution of individual ${\bf
  s}$.
So Pr(usage) is just the standardized version of those counts.
In other words, we regard the counts m(x) as a multinomial observation
with probabilities $\pi(x|s)$. What kind of models for $\pi(x|s)$?
Well they should have lower probability as $x$ gets far away from $s$.
...
\label{scr0.eq.ud}
\end{equation}
where $k(s,x)$ is some kernel function that decays monotonically.
...
and, if detection is perfect, 
it is exactly our encounter probability mnodel if we put traps
somewhere. Note that even though detection was assumed perfect,
conditional on use, this is definitely not 1 everywhere because it
decays as a function of distance from ${\bf s}$. 
...
\begin{comment}
\subsection{Hazard formulation of Gaussian model}
