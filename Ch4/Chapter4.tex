...
sampling literature, usually with $\alpha_{1} = 1/(2\sigma^{2})$. 
Because it is the kernel of a bivariate normal or
...
We developed our basic SCR model in terms of a latent variable ${\bf
  s}$ which we call a home range center.
Surely, then, the encounter probability model which relates encounter
of individuals in specific traps to ${\bf s}$ must somehow imply a
certain model for home range geometry. 
Here
we explore the nature of that relationship.  
...
sigma     NEED RESULTS WITH SIGMA HERE XXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
alpha1     3.77  0.43   2.92   3.48   3.79   4.05   4.66 1.04    70
...
XXXXX  need to finish this XXXXXXXXXXXXX

We simulated data from the Gaussian kernel model up in 
sec. \ref{scr0.sec.unknownN} and then we fitted 4 models to it.
Gaussian, cloglog, exponential and logit with the square of distance.
What is the ESA under these ?
...
have no record. Thus we will need to know \mbox{\tt ntraps} 
and \mbox{\tt nperiods} when
...
Buffer	sigma			psi			N			D		
	mean	sd	n.eff	mean	sd	n.eff	mean	sd	n.eff	mean	sd	n.eff
10	0.65	0.06	1800	0.13	0.03	10000	39.63	6.7	7100	5.97	1	7100
15	0.64	0.06	510	0.16	0.04	3800	48.77	9.19	3300	5.78	1.09	3300
20	0.64	0.06	1200	0.2	0.05	16000	59.84	11.89	20000	5.77	1.15	20000
25	0.64	0.05	3600	0.24	0.05	3400	72.4	14.72	2700	5.79	1.18	2700
...
40	0.64	0.05	410	0.39	0.09	480	118.05	26.17	410	5.87	1.3	450
45	0.64	0.05	10000	0.45	0.1	3600	134.43	28.68	3300	5.83	1.24	3300
...
For some models, the parameter 
 $\alpha_{1}$ is related to the home range radius (sec. 
XXX MISSING XYZ). For the Gaussian kernel model we interpret the half-normal scale
parameter $\sigma$, which is related to $\alpha_1$ by $\alpha_1 =
...
which is rather huge for measured home ranges (REF XXXXX). We probably
...
data, the predictions tend to the  XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
expected value under the prior
distribution which, in this case, is constant.
xxxxx$is this correct ? I would have thought, they tend to the mean of what the prior AND the data say it should be ?$ xxxxx The other thing to note about
