...
\begin{equation}
	\mbox{logit}(p_{ij}) = \alpha_{0} + \theta ||{\bf s}_{i}-{\bf x}_{j} ||
\label{scr0.eq.logit}
\end{equation}
...
%quantities.
We probably expect that the parameter $\theta$ in
...
run the MCMC algorithm for an adaptive burn-in period.
Then the Markov chains are updated using
...
\end{verbatim}
We find that {\bf JAGS} seems to be 20-30\% faster for the basic SCR model
which the reader can evaluate using the function \mbox{\tt
...
input file format in 
 {\bf
  SPACECAP}
 \citep{gopalaswamy_etal:2012} and in the {\bf R} package \mbox{\tt SCRbayes} \citep{russell_etal:2012}.
To illustrate this
format, the wolverine data are available in the package \mbox{\tt scrbook} by typing:
\begin{verbatim}
data(wolverine)
\end{verbatim}
which contains a list having elements \mbox{\tt wcaps}.............. 
ANDY STOPPED HERE...............
as an encounter data {\bf R} object
