...
\begin{figure}
\begin{center}
\includegraphics[height=6.5in,width=4.8in]{Ch4/figs/wolverinelocs}
\end{center}
\caption{
...
structure is a simple matrix with 4 columns, those being: (1) sessID: the trap
session or year. For a single-year study (as considered here) this
should be an integer  that is constant for all records; (2) indID: the
individual identity, being an integer from $1$ to $n$ (repeated for
multiple captures of the same individual) indicating which
individual the record (row) of the matrix belongs to; (3) sampID: The integer sample
occasion which generated the record, and (4) trapID: the trap identity, an
integer from 1 to $J$, the number of traps.  The structure of the EDF
is the same as used in the \mbox{\tt secr} package (Efford XXXXXXXX)
...
\footnote{ This may differ by 1 from Royle et al. 2011 table. Check this}
}
\centering
...
function
XXXXXXXXXXXXXXXXXXXX FIX THIS FUNCTION NOW XXXXXXXXXXXXXXXXXXXXXXX
