...
by DK Dawson and MG Efford. The data from this study are available in
the \mbox{\tt secr} package, and have been analyzed previously 
...
``{\it Netting was passive (i.e. song playback was not used to lure birds
...
robust design'' \citep{pollock:XXXXX}.  
...
emigration model'' \citep{kendall:XXXXX}, and this model is used 
 in \mbox{\tt secr} for 
``multi-session'' situations. 
In that case, you can stack up all your data together and do
something.....
look in secr manual here XXXXXXX  
We formalize this type of approach from a Bayesian perspective in
Chapt. \ref{chapt.XXXXXX}.
For now, we do something even simpler -- we 
assume that each session has its own 
$N_{t}$ parameter
...
There is one issue we don't deal with here. There was a single
