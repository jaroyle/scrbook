...
\mbox{\tt secr} documentation of this data set we take this: ``{\it
  ...
  a brood patch (females) or cloacal protuberance (males).}''  The
...
{\it 
Recaptures within a day are not included in this dataset, so each bird occurs at most once per day and the detector type is ‘multi’ rather than ‘proximity’
}
...
for such replication by groups of time or space. 
In the case of the ovenbird data, sampling was carried out
at multiple years, with a number of sample occassions within each year
...
models for recruitment and survival.


A special case of open models arises when we assume $N_{t}$ are
independent from one session to the next -- this is a ``random temporary
emigration model'' \citep{kendall_etal:1997}, and this model is used 
 in \mbox{\tt secr} for 
``multi-session'' situations.  In particular, by assuming that $N_{t}$
is Poisson with mean $\lambda_{t}$ one can model variation in
abundance among sessions based on the Poisson-integrated likelihood in
which parameters of $\lambda_{t}$ appear directly in the
likelihood. We provide some results from \mbox{\tt secr} for the
ovenbird data here shortly, and 
we formalize the multi-session model approach from a Bayesian perspective in
Chapt. \ref{chapt.XXXXXX} based on \citep{converse_royle:2012}. 
...
multi-session models from \mbox{\tt secr} both here and in
Chapt. \ref{chapt.hscr} (Bayesian),
...
Also Efford uses a habitat mask which we don't use.
...
\]
and so on.  In a funny way, this model is a type of local behavioral
response model but where the response is to other individuals being
captured.   {\bf XXXX should use the mlogit for this b/c it affects that
and not directly the pi's XXX }
