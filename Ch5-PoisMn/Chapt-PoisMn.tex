...
according to \ref{eq.cloglog}. % XXXX Do we need "Eq." before number?
...
Chapt. \ref{chapt.scr0} (see the function \mbox{\tt simSCR0}). In particular, we modify the block of code
...
traps hold multiples of animals, % XXXX or "multiple animals"?
and pit-fall traps are commonly used % XXXX new sentence?
...
the same plot but not, in the same sample session, % XXXX session or occasion??
at different plots.
...
$(0,0,0,0,0,0,1)$.  If we sample for 5 periods % XXXX occasions?
in all and the
...
\citep{royle_chandler:2012}, % XXXX update ref
...
\begin{verbatim}
##
## Random activity centers: Define N, Xl, Xu, etc...
##
...
with $K=7$ occasions. % XXXX periods, etc..
...
relevant to any stratified population.
% XXXX Why then aren't these called multi-strata models? I think that
% is the terminology used in program DISTANCE. "session" sounds vague
% to me. Oh well.
...
with birds being caught multiple times in a set. % XXXX What do you
                                % mean by set?
...
the 3 main types of encounter data: % XXXX No mention of Poisson?
binary encounters, multinomial trials from ``multi-catch'' or
``single-catch'' \citep{efford:2004, efford:2011, royle_gardner:2011}
trap systems, and encounter frequency data from devices that can
...
webs'' \citep{anderson_etal:1983, wilson_anderson:1985b,
  jett_nichols:1987, parmenter_etal:1989} % XXXX cite Barker/Link paper
