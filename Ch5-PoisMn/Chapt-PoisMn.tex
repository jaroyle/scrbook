...
likelihood is $\Phi(\gamma(stuff))$ (see Eq. 8 from \citet{efford_etal:2009ecol}):
\[
\Pr(S_{ij}| {\bf u}_{i} ) = \Phi( \gamma( stuff) )^{1-1(S_{ij}>c)}
...
\prod_{j=1}^{J} \Phi( \gamma(stuff, {\bf x}_{j}, {\bf u}) ) 
\]
...
Efford et al. 2009 have this expression below which I think is wrong.
