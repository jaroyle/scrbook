...
make sense of such data \citep{borchers_efford:2008,
  royle_young:2008,efford_etal:2009ecol, gopalaswmay_etal:2012ecol,
...
record data, or humans pick up scat, we could establish an array of
...
referred to audio recorders to be ``signal strength proximity
detectors'' as opposed to cameras, as in camera trapping, which are
{\it visual} proximity detector.  Then, the spatial pattern of the
...
{\bf XXXXX algebra is wrong through here somehow XXXXX}
...
device $j$ is given by the following expression:
\begin{verbatim}
