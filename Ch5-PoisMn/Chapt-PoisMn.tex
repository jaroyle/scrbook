...
${\bf x}$ % XXXX Suggest keeping the subscripts to be consistent with previous equation
for an individual having activity center ${\bf s}$.
...
Even in such cases % XXXX not in the range
...
period of time. 
% XXXX List some of the reasons for doing this intentially. Also,
% discuss the independence assumption of the Poisson model, which
% might often be violated in camera trapping studies, if an animal
% becomes interested in a camera. This might be a case where you would
% want to use the Bern model.
% OOPS. I see that you dicuss this in next section.
But sometimes, 
...
detection of scat by dogs. % XXXX E.g.,
...
cage-traps hold multiple animals, as do 
 pit-fall traps which are commonly used 
...
the same plot but not, in the same sample occasion,
at different plots.
...
referred to audio recorders as ``signal strength proximity
detectors'' to distinguish them from other types of proximity
detections, including camera traps, which are 
{\it visual} proximity detector.  Using audio records, the spatial pattern of the
{\it signal strength} at the different microphones can be used for
inference about density
...
The basic technical formulation of these models comes from
\citet{efford_etal:2009ecol}, and it was applied to field study of
birds  by \citet{dawson_efford:2009}. In that study, recording devices
were organized in groups of 4 (in a cross pattern), 
with an array of $5 \times 15$ such
cross-patterns, seperated by 100 m  (300 total recorder locations). 
This data set is provided with the \mbox{\tt secr} package along with
some sample analyses and help files. See \citet{efford_dawson:2010}, 
a version of the document \mbox{\tt secr-sound.pdf} that 
comes with the \mbox{\tt secr} package, which
you can access directly from the main help file (\mbox{\tt ?secr}).
...
parameters $\alpha_0$, $\alpha_1$ and $\sigma$ are to be estimated.
...
To expand on and formalize this, 
let $y_{ij}$ be the value of $S$ for animal $i$ at detector $j$. 
The encounter probabiliy is $\Pr(y_{ij}>c)$ which is
$\Pr(y_{ij}>c) = 1- \Pr(y_{ij} < c)$, so that, if we standardize the variate 
we have
\[
1-\Pr( \frac{ (y_{ij}- \mathbb{E}(y))}{\sigma}  <  \frac{ 
(c -  \mathbb{E}(y)) }{\sigma})
\]
This probability 
calculation is the CDF  of a standard normal variate, say $\eta$, is
less than the quantity
$\gamma = (c -  \mathbb{E}(y)) }{\sigma})$
which is a function of all the parameters $\alpha_{0}$, $\alpha_{1}$,
$\sigma$ and also the individual location ${\bf u}$ and trap locations
${\bf x}$.  We can compute 
$\Pr(y_{ij}>c) = 1-\Pr(  \eta <
\gamma(\alpha_{0},\alpha_{1},\sigma,{\bf u},{\bf x}))$. We'll label the CDF of a
standard normal by $\Phi(\gamma)$.
...
\[
p_{ij} = p(\alpha_0, \alpha_1, \sigma | {\bf x}_{j}, {\bf u}_{i} ) = 1- \Phi( \gamma(stuff) ) 
\]
where
 ${\bf u}_{i}$ is the instantaneous location of individual $i$ and 
${\bf x}_{j}$ is the location of trap $j$.
...
Ok, so the ``data'' from an acoustic survey are the signal strength
...
device $j$ can be constructed as we did for the Bernoulli model in
Chapt. \ref{chapt.mle}, but see \citet{borchers_etal:2009ecol} for
technical details of this..............
.................
Pr(detect|Signal)Pr(signal)............
....................
...
You have to set the \mbox{\tt cutval}
...
although we imagine that implementation of such models in the {\bf BUGS} engines
should be achievable.  
It seems easy enough to write down a
 general hierarchical model that would accomomdate sampling on repeated
 occassions. Let  ${\bf s}_{i}$ be the home range center, and 
let ${\bf u}_{ik}$ the instantaneous location of individual $i$ during
 ...
 for ${\bf u}_{ik}$ can be specified  conditional on ${\bf s}_{i}$. For example, we
 ...
 according to the signal attenuation model (Eq. \ref{poisson-mn.eq.signal}),
 or perhaps some other
 model. Then we generate the binary encounter data by truncating the
 observed signal at $c$. 

 Instead of demonstrating this model here,  we leave it to the reader
 to explore simulating data and devising a 
 Bayesian implementation of the acoustic model in one of the {\bf
   BUGS} engines.
Note that for a single occasion, you can simulate the data
using the two stage model (having both ${\bf s}$ and ${\bf u}$) or you
can simulate ${\bf u}$ uniformly at the start.
...
\citet{efford_dawson:2010} noted that 
 various other types of acoustic data might arise for which
SCR-like models would be useful.
