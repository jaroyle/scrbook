...
location.''
we will refer to it as the ``(bivariate)
normal'' or ``Gaussian'' model although the distance sampling term
``half-normal'' is widely used.  
...
models. Despite this, making explicit assumptions about the
distribution of
$N$, such as
...
of disjoint regions of the state-space, say $A_{1}, \ldots, A_{k}$, then these counts are
...
useful in practical settings
 since they allow for a range of distribution patterns without violating the assumption of spatial randomness.
  More relevant for SCR models is that we
...
it does not conform to any concept or quantity that is relevant to
