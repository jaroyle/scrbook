...
everyone only cares about the size of actual populations -- and their realized
population size. 

If you do likelihood analysis then the distinction between realized vs
expected is often discussed by whether its ``conditional on $N$''
(realized) or not (expected). The naming arises because in obtaining
the MLE of $N$, its properties are evaluated ``conditional on $N$'' --
in particular, if it is unbiased then $E(\hat{N}|N) = N$ and
$Var(\hat{N}|N) = \tilde{\sigma}_{N}^{2}$ is the sampling variance.
This is bad nomenclature because, if we need to predict a new
population that was not sampled (see below), in which case case we are
just predicting a new random variable ``unconditionally'', then we
have to do some gyrations to cook-up estimates and variances (see
below).  Nor does this nomenclature conform to anything relevant to
Bayesian inference.  If we care about $N$ for the population that we
sampleed then ``realized'' population size is understood to be the
