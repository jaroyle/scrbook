...
to be monitored, and then send everything off to our MCMC engine.
...
inits <- function(){
  list (alpha0=rnorm(1,-2,.4),beta=runif(1,1,2),s=sst,z=z,psi=.5)
}
...
Summarizing the output from WinBUGS we have:
...
I think the point i want to make here is that trap locations are
``states'' that an individual belongs to during some moment in time
and therefore we can formulation SCR models as multi-state models
with spatially structured transition probabilities.........
Can be ``random'' movement or Markovian....
...
from x to s. This is a familiar looking function -- it is ``the MAXENT
