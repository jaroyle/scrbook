...
to the Bernoulli model in the sense that $\Pr(y\in \{0,1\})
\rightarrow 1$ as $\lambda \rightarrow 0$. i.e., $y$ is almost
always 0 or 1. Additionally, $\Pr(y>0) \rightarrow \lambda$.  
...
\section{Analysis of the Fort Drum Bear Data}
...
typically performs much better \citep{converse_royle:2013}. In the
examples here, we use {\bf JAGS}
 exclusively.
...
birds which we analyze here. These data are from
\citet{efford_etal:2004} and see also \citet{borchers_efford:2008}.
Mist netting data from Patuxent Wildlife Research Center,
collected by DK Dawson and MG Efford. 
...
XXX 
IF this chapter goes after the MLE chapter then we can do more
analysis here for comparison.
XXXXX
