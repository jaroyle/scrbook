...
binomial XXXX EFFORD CITE XXXXXX.  Here we focus on using the Poisson
model only. 
...
of the encounter process model is that resulting from movement of individuals
about their home range. What if we had perfect samplers in every pixel
of the landscape that discharge at set time intervals (e.g., every hour), and let $m_{ij}$ be the number of times the
individual was recorded in pixel $j$. In a pure model of space usage,
we might think of this model: 
\[
...
(RSF) models (see XXXXX below XXXXX).  But now suppose our samplers are not
perfect but, rather, record only a fraction of the resulting visits. A
sensible model is
\[
 y_{ij}|m_{ij} \sim \mbox{Bin}(m_{ij}, \lambda_{0})
\]
(in this case, $\lambda_{0} \le 1$)
which, marginally, is the same as
\[
...
another case in sec. \ref{poisson-mn.sec.singlecatch} below.
...
$\Pr(y>0)$ vs. $E[y]$. We see that the models are
...
Therefore, under the Poisson model we have
\begin{eqnarray*}
\Pr(y>0) \approx E[y]  \\
1-exp(-\lambda_{0} g(x,s)) \approx  \lambda_{0} g(x,s)
\end{eqnarray*}
which explains the close correspondance we have found between these
two versions of the Gaussian encounter probability model (XXXXX REF XXXX). 
...
analyze an ideal data set using {\bf WinBUGS}. The new function,
...
\[
 \pi_{ij} = \frac{  exp(\alpha_{1} D_{ij}) }{ 1+ \sum_{j}
   exp(\alpha_{1} D_{ij})}
\]
...
\[
 \pi_{ij} =  \frac{ \lambda_{ij} } { \sum_{j} \lambda_{ij} } 
\]
which is precsely, under the Gaussian kernel model, 
\begin{equation} 
...
as looking similar to the ``maximum entropy distribution'' from MAXENT
species distribution modeling, or resource selection functions (RSFs)
from telemetry studies \citep{lele_keim:XXXX). This suggests therefore how we might model
landscape or habitat covariates using such methods -- i.e., by
including them as explicit covariates in this model, and include
RSF data from telemetry studies, which we pursue in
Chapt. \ref{chapt.rsf}.

The model is not completely specified, however, absent a model for the
probability of encouter or the frequency of captures. If
the total number of encounters $y_{i}$ has a Poisson distribution then, by marginalizing
Eq. \ref{poisson-mn.eq.mn}
over the Poisson distribution for 
$y_{i}$ produces the original set of $iid$ Poisson frequencies.
If 
$y_{i}$ is Binomial with sample size $K$ and probability  $\phi_{i}$,
say with 
\[
\phi_{i} = \lambda_{0} \exp(-\beta \sum_{j} D_{ij}^2)
\]
...
then the $K$ encounter outcomes have precisely the multinomial
probabilities from XXXX above.
Therefore the Poisson and multinomial models are equivalent in how
...
\subsection{Multinomial Model as a Resource Selection Model}

XXX this is just Andy doing some head-scratching XXXXX

Imagine the landscape pixelated and we randomly sample pixels and we
telemeter an individual and record the number of times the guy is in
each pixel $m_{ij}$. The standard RSF model is
\[
 \pi_{ij} = \frac{ exp( -\alpha_{1} D_{ij} )}
 { \sum_{j} exp(-\alpha_{1} D_{ij}) }
\]
but, remember, we like to have $D_{ij}^{2}$ to express the bivariate
normal model of space usage.  Anyhow, suppose instead of having a guy
telemetered we use camera traps within each pixel of relatively small
size. In that case we don't have perfect observation but some
detection rate $<1$. But this constant cancels from the numerator and
denominator and so it doesn't affect the apparent distribution of
space usage. 

Finally, suppose we don't sample every pixel but just a few
pixels. Let $o_{j}$ be a binary indicator of whether we sample a pixel
or not.  The apparent distribution of encounters in the sampled pixels is
\[
 \pi_{ij} = \frac{ o_{j} exp( -\alpha_{1} D_{ij} )}
 { \sum_{j} o_{j}  exp(-\alpha_{1} D_{ij}) }
\]
which is exactly the same as the multinomial probabilities from
above. That is, the relative distribution is unaffected by the
incomplete sampling within the cells that are actuall sampled, and the
unsampled ones are all 0. 

thus the multinomial cell probabilities for the independent multinomial
observation model are just ``thinned'' RSF probabilities. 
That is the multinomial model probability of encounter in pixel j
``given that pixel j is sampled''  is entirely, i.e., only, the result
of animal USE of pixel j. 



\section{Mist-netting example}

Was going to use a MAPS example but that has too many issues. But
Efford has a data set in his \secr package -- 

This is a mist-netting data set on ovenbirds
\begin{verbatim}
library("secr")
data(ovenbird)
\end{verbatim}
...
perimeter of a 600-m x 100-m rectangle were operated for approximately
9 hours on each of 9 or 10 non-consecutive days during late May and
June in each year. 
Netting was passive (i.e. song playback was not used to lure birds into the nets). Birds received individually numbered bands, and both newly banded and previously banded birds were released at the net where captured. Sex was determined in the hand from the presence of a brood patch (females) or cloacal protuberance (males). A small amount of extra netting was done by other researchers after the main session in some years. 
...
Fit a model to one of the years of data. A basic model.
...
XXX IF this chapter goes after the MLE chapter then we can do more
analysis here 
XXXXX
