...
Therefore, the model is not completely specified, however, absent a model for the
...
In particular, if we suppose that $y_{i}$ is Poisson with mean
$\sum_{j} \exp(\alpha_{0} -\alpha_{1} d({\bf x},{\bf s})^2)$ then the marginal
distribution of $y_{ij}$ is Poisson with mean 
$\exp( \alpha_{0} - \alpha_{1}  d({\bf x},{\bf s})^2 )$, equivalent to
...
from telemetry studies \citep{manly_etal:2002, lele_keim:2006}. 
This suggests therefore how we might model
