...
state-space (possible outcomes) $(1,2,3,4,5,6,7)$ where outcome $y=7$ corresponds to
...
$1:(J+1)$) where $J+1$ is the event not captured.  Therefore, for our
illustration in the previous table, 
...
discard the uncaptured individuals.  \mbox{\tt sim.mnSCR} will also
...
with $K=7$ periods, etc.. We'll run the model using {\bf JAGS} which
...
The interpretation of this model merits some discussion. That is, {\it
...
where he does some analysis -- see the helpf file ?possum for secr,
look at chapter XXX for info on secr, and so on.
To load the data we have to load secr first
\begin{verbatim}
library("secr")
data(possum)
\end{verbatim}
From the help file Efford says this:
...
Originally the data were analyzed in Efford et al. (2005) and the
study area encompases approximately 300 ha.  

An interesting feature -- 5 trapping grids -- look at plot....... what
do we do about this?   

Efford et al. (2005) reported a live-trapping study of possums in
Pinus radiata plantation on coastal sand dunes. The 300-ha site at
Waitarere in the North Island of New Zealand was a peninsula, bounded
on one side by the sea and on two other sides by the Manawatu
river. Cage traps were set in groups of 36 at 20-m spacing around the
perimeter of five squares, each 180 m on a side. The squares (‘hollow
grids’) were centred at random points within the 300-ha area. Animals
were tagged and released daily for 5 days in April 2002. Subsequently,
...
another polygon, the nominal removal area of Efford et al. (2005
Fig. 1) (from secr 2.3).
...
NEED A MAKE.STATESPACE   function
