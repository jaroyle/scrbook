...
formulation of the models analyzed in BUGS engines including {\bf
  WinBUGS} and {\bf JAGS}.  Despite our focus on
...
methods and frequentist inference. 
In fact, simple SCR models can be analyzed fairly easily
using such methods. This has been the approach taken
by \citet{borchers_efford:2008, dawson_efford:2009} and related
papers.
...
purpose wrapper named \mbox{\tt scr} which has other capabilities. 
...
\mbox{\tt SCR23darray.fn} which we defined in section XYZ.XYZ.
...
In section xxxx back in chapt. 4 we used a discrete representation of
...
into the likelihood evaluation above. The R function \mbox{\tt scr}
...
has also released an {\bf R} package named secr, that contains many
...
and run models in secr, you must download the package and load itin
{\bf R}.
\begin{verbatim}
