...
traps<-as.matrix(cbind(1:37,wolverine$wtraps)) 
...
standard scaling for the wolverine data. Note that we add a 'trapID' column to
...
function called \mbox{\tt secr_wolverine}. Printing the output object
...
estimate is 8.35 individuals/1000 $km^2$.  $\sigma$ is given in units of
...
= 6.289$ $km$. 
These differences between Tab. \ref{mle.tab.wolverine} and those
produced by \mbox{\tt secr} are likely due to subtle differences in  internal
tuning of optimization algorithms, starting values or other numerical settings.
