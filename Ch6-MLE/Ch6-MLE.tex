...
\[
[y|{\bm \alpha}]  = \frac{A({\cal S})}{nG} \sum_{u=1}^{nG} [y|{\bf s}_{u},{\bm \alpha}] [{\bf s}_{u}]
\]
...
\mbox{\tt X} which were created above. However,
...
function beyond the boundary of the parameter space (e.g., $n_{0} < 0$).
...
a standard errors using the delta approximation: {\bf XXXX Check these
  commands and compare with whats in the table XXXXXX}
\begin{verbatim}
area<- nrow(G)*4
...
secr has a function called \mbox{\tt region.N} for estimating the
realized value of $N$ and not its expectation.
...
baseline encounter probability $p_{0}$ (named $g_{0}$  in \mbox{\tt
  secr}) by just doing this: {\bf XXXXX need centered tildes here XXXXX}
\begin{Verbatim}[commandchars=\\\{\}] 
out<-secr.fit(wolvcapt,model=list(D \mytilde session, g0 \mytilde session, sigma \mytilde 1), buffer=20000)
\end{Verbatim}
