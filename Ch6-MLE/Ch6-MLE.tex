...
To execute this function for the data that we created with \mbox{\tt simSCR0},
...
\end{verbatim}
While usually produces one or more {\bf R} warnings due to numerical
...
shape. This makes it easy to do things like 
 clip out non-habitat, or create a {\it habitat mask} which defines
 suitable habitat.
Clearly that
...
For each point in the state-space grid (object \mbox{\tt G} in the
code below which is assumed to exist), we deterimine whether it is
inside the polygon\footnote{We perform this check using the {\tt over} function. This function takes as its second argument (among others) an object of the class ``SpatialPolygons'' or ``SpatialPolygonsDataFrame'', which can hold additional information for each polygon, and the output value of the function differs slightly for these two classes: if using a ``SpatialPolygons'' object, the function returns a vector of length equal to the number of points (e.g., in the example above), but if using a ‘’SpatialPolygonsDataFrame’’ it returns a data frame (e.g., see sec. \ref{mcmc.sec.state-space} in Chapt. \ref{chapt.mcmc}). If you use the {\tt over} function, make sure you know the class of your second argument so that when processing the function output you index it correctly.}, identifying such points with a value of \mbox{\tt
  mask=1} and \mbox{\tt mask=0} for points that are {\it not} in the polygon.
We load the shapefile which originates by an application of the
\mbox{\tt readShapeSpatial} function. We have saved the result into an
{\bf R} data object called \mbox{\tt SSp} which is in the \mbox{\tt scrbook} package.
Here
are the {\bf R} commands for doing this (see the helpfile \mbox{\tt ?intlik4}): 
{\small
\begin{verbatim}
library(maptools)
library(sp)
...
\begin{Verbatim}[commandchars=\\\{\}] 
 secr.fit(capturedata, model = list(D \mytilde 1, g0 \mytilde 1, sigma \mytilde 1), buffer = 20000)
\end{Verbatim}
...
\citep{royle_young:2008} which we discuss further in Chapt. \ref{chapt.search-encounter},
...
functions as of version 2.3.1 (see Tab. \ref{covariates.tab.detmodels} in Chapt. \ref{chapt.covariates}), but
...
$\sigma$.
The units of this parameter default to meters in the \mbox{\tt secr} output.
We caution that the meaning of this parameter
...
noted in sec. \ref{scr0.sec.implied} every encounter probability model
implies a 
...
occasion. 
In addition to providing the encounter data file (EDF),  we must tell \mbox{\tt secr}
...
#
wolv.secr2<-secr.fit(wolvcapt2,model=list(D \mytilde 1, g0 \mytilde 1, sigma \mytilde 1), buffer=20000,mask=gr2)
\end{Verbatim}
}
We are using the 
...
Model           :  D \mytilde 1 g0 \mytilde 1 sigma \mytilde 1 
Fixed (real)    :  none 
...
obtained in our likelihood analysis summarized in Tab. \ref{mle.tab.wolv}
...
= 6.289$ $km$. 
The difference in the MLE between Tab. \ref{mle.tab.wolv} and those
...
augmentation and Poisson-integrated likelihood) are 
approximately the same as $M$ gets large. However, doing a Bayesian
analysis by MCMC,  we obtain an
estimate of both $N$, the {\it realized population size}, and the parameter controlling its expected value
...
\begin{Verbatim}[commandchars=\\\{\}] 
out<-secr.fit(capdata,model=list(D \mytilde session, g0 \mytilde session, sigma \mytilde 1), buffer=20000)
\end{Verbatim}
...
\citep{converse_royle:2012,royle_converse:2012} which we discuss, with
examples,  in Chapt. \ref{chapt.hscr}.



\subsection{Other stuff secr does}

secr does a ton of other things that are useful:

(1) it gives you some summary stats about observed animal movement
(e.g. MMDM)\footnote{note: we need a SCRsmy function to do this}

(2) it has a population closure test 

(3) has a likelihood ratio test to compare fitted models

(4) can give you model averaged parameter estimates (See section XXXXX)

(5) can calculate the effective sampled area (see Section XXXX)

(6) can suggest a buffer width (and I am pretty sure it gives an error message or warning if the buffer/S is obviously chosen too small)


(1) It estimated density directly, and regards N as a derived quantity. There is a function called region.N() for this.

(2) There are functions for computing creating maps of detection contours for individuals traps, or for the entire trap array. See pdot.contour()

(3) The function fxi.contour() computes the 2D pdf of the locations of one or more individual activity centers.


We can also model covariates on density, allowing that the activity
centers ${\bf s}$ are a realization of an inhomogeneous point
process. We cover such models in Chapt. \ref{chapt.state-space}.
