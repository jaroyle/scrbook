...
shapefiles \index{shapefile} which we demonstrate here. Our approach
...
packages \mbox{\tt sp} \index{R package!sp} and \mbox{\tt maptools}
\index{R package!maptools} \index{maptools}.  For each point in the
...
\subsection{Other \mbox{\tt secr} capabilities}
...
these other capabilities that you need to know about as you use
\mbox{\tt secr}.

\subsubsection{Summary statistics}

\mbox{\tt secr} provides a useful summary of the data including
summary statistics about animal movement including mean-maximum
distance moved. XXXXXXXXXXXX how to do thix? XXXXXXXXXXXXX

\mbox{\tt secr} 
will produce a warning if the state-space buffer is chosen too small,
and it will suggest an appropriate value.  XXXX how does it do this ? XXXXXXXXXX

\subsubsection{Model selection,  averaging and statistical tests}
...
\mbox{\tt secr} Does a likelihood ratio test to compare nested models
using the function \mbox{\tt LR.test}.

\mbox{\tt secr} will produce model selection tables based on AIC or
AICc, and will produce model-averaged parameter estimates (See
Chapt. \ref{chapt.gof} for examples.)


\subsubsection{Density mapping and effective sample area}

\mbox{\tt secr} reports estimates of density directly, but you can use
...
traps, or for the entire trap array. 
 $\bullet$ The function \mbox{\tt pdot.contour} does XXXXXXXXXX

 $\bullet$  The function \mbox{\tt fxi.contour} computes the 2D pdf of
 the locations of one or more individual activity centers (as in
 Sec. XXXXXX). In the context of likelihood analysis, estimation of a
 random effect ${\bf s}$ is based on kind of a plug-in application of
 Bayes Rule. 
\[
 [{\bf s}| {\bf y},\theta] = \frac{ [{\bf y} | {\bf s},\theta] [{\bf     s}] }
\]
 $\bullet$ Calculate the effective sampled area using XXXX FUNC XXXX (see Section XXXX)
...
It has some built in response surface models , fitting easting and
northing, linear or quadratic, response surfaces.  These are done by
modifying the density model in secr.fit. e.g., D ~ 1 is a constant
density  surface, D ~ x+ y is a linear response surface, etc...
See the manual on \mbox{\tt secr-densitysurfaces.pdf} for a lot of
details.

To make your own covariates, there seems to be many ways to do
this. One is to use the addCovariates function and supply it a mask or
traps object along with some ``spatialdata'' and what happens depends
...
\mbox{\tt predictDsurface}

Can allow covariates on encounter probability........ we covered a lot
of default types of covariates in Chapt. \ref{chapt.covariates} and also see
...
sex or whatever (see Chapt. \ref{chapt.covariates}......).
