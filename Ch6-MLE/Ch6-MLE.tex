...
the multinomial encounter models described in Chapt. \ref{chapt.poisson-mn}. For a full review of the detector types,
...
functions (see {\bf Table XXXX} in Chapt. \ref{chapt.covariates}), but
...
trap session, individual identity, sample occassion, and trap identity.
...
information about the traps which is formated as a matrix with columns
``trap ID'', and the (x,y) coordinates of each trap, and also the type
