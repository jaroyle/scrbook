...
traps<-cbind(1:nrow(traps),traps)  # pad an arbitrary trap ID variable
...
location,  similar in information content to our encounter data file (EDF).
\mbox{\tt secr} requires that you specify the trap type. For 
...
which allows for a trap to be a sampled polygon (e.g., see Chapt. \ref{chapt.searchencounter}), e.g., scat surveys,
and 'signal' detector which allows for traps that have a strength
indicator, e.g., acoustic arrays.  
...
The other detector types allowed are ‘proximity’, ‘count’, and
‘polygon’. To understand these, consult
the \mbox{\tt secr} help manual, which can be accessed in {\bf R} after
...
each trap during each occasion (1=operational, 0=not). We must also
communicate the type 
of trapping device being used 
so that \mbox{\secr} uses the correct likelihood for the data. Instead
of 
describing models by their statistical distribution, 
 \mbox{\tt secr} 
accepts the type of 
detector used ('proximity', 'multi', etc.) when reading in the
...
dimnames(traps)<-list(NULL,c("trapID","x","y",paste("day",1:164,sep="")))
...
secr 2.3.1, 09:41:03 29 Aug 2012
...
D     -9.749576 0.2302783 -10.200913 -9.298238
g0    -4.275735 0.1584610  -4.586313 -3.965158
sigma  8.699202 0.0786894   8.544973  8.853430

Variance-covariance matrix of beta parameters 
                  D            g0        sigma
D      0.0530281019  0.0005468597 -0.005226898
g0     0.0005468597  0.0251098852 -0.005885204
sigma -0.0052268977 -0.0058852041  0.006192022

Fitted (real) parameters evaluated at base levels of covariates 
       link     estimate  SE.estimate          lcl          ucl
D       log 5.831940e-05 1.360971e-05 3.713640e-05 9.158542e-05
g0    logit 1.371121e-02 2.142902e-03 1.008756e-02 1.861207e-02
sigma   log 5.998123e+03 4.727203e+02 5.140849e+03 6.998354e+03



XXXX OLD RESULTS BELOW THIS LINE
...
In particular, we reported a density of $D=5.77$ (posterior mean) and
$\sigma = 0.64$ (posterior mean) which was for a state-space where a
standard unit was 10 km (is this true?) 

\begin{verbatim}
Was going to compare secr estimates to the WinBUGS estimates....
...
(Eq. 
\ref{mle.eq.binomialform}
above) over a Poisson prior for $N$ -- what we will call a
