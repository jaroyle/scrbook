...
involves the unknown $n_{0}$ and thus it must be included in the
likelihood. 
...
XXXX This next par should ref ahead to Chapt. 9 where the table is  XXXXX
...
\subsection{Likelihood Analysis in the \secr Package}
...
Specifically, we compuate the marginal
probability of each ${\bf y}_{i}$ as before,
\[
  [{\bf y}_{i}|{\bm \alpha}] = 
\int_{{\cal S}} \mbox{Binomial}({\bf y}_{i} |{\bf s}_{i}, {\bm \alpha})g({\bf s}_{i}) d{\bf s}_{i}
...
individuals we compute the probability of an all-0 encounter history:
\[
\pi_{0} = [{\bf y} = {\bf 0} | {\bm \alpha}] = 
\int_{{\cal S}} \mbox{Binomial}({\bf 0} |{\bf s}_{i}, {\bm \alpha})g({\bf s}_{i}) d{\bf s}_{i}
\]
{\bf NOTE: PUT THIS FORMULA IN CH 5!!! (also formula
for pi_{0})}
Then, the marginal likelihood has this form:
\[
 {\cal L} = \frac{N!}{n! n_{0}!} 
 \left\{ \prod_{i}  [{\bf y}_{i}|{\bm \alpha}] \right\}
 (\pi_{0})^{n_{0})
