...
component of model fit has a huge amount of precedence in the
...
-2*\mbox{\tt logL}(\theta|{\bf y})$.  The DIC is defined as the
...
##
  p[i,j,1] <-  p0[i,1]*exp( - alpha1.vec[i,1]*dist2[i,j] )
  p[i,j,2] <-  1-exp(-p0[i,2]*exp( - alpha1.vec[i,2]*dist2[i,j] ) )
  logit(p[i,j,3]) <- p0[i,3] - alpha1.vec[i,3]*dist2[i,j]
...
\begin{verbatim}
 p[i,j,1]       <-  p0[i,1]*exp( - alpha1[1]*dist2[i,j] )
 p[i,j,2]       <-  1-exp(-p0[i,2]*exp( - alpha1[2]*dist2[i,j] ) )
 logit(p[i,j,3])<- p0[i,3] - alpha1[3]*dist2[i,j]
\end{verbatim}
where
\begin{verbatim}
...
fit in dynamic occupancy models similar to that which we suggested above.
...
checking, should be viewed as experimental, based on mostly {\it ad hoc}
