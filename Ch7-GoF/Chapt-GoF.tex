...
\citet{burnham_etal:1987}, \citet{lebreton_eta:1992}, and Chapt. 5 of
...
\sum_{g : N_{g} = 1}^{G-N} (N/G)^2
 = N(1 + (G-N)/G)
\]
(we use $:$ here to mean ``such that'').  If $G$ is huge
...
using {\bf WinBUGS}. $T()$ are the posterior summaries of fit
statistics used in the Bayesian p-value analysis (See text for
...
\citet{kery_etal:inreview} used a similar strategy for assessing model
