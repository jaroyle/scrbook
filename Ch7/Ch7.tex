...
This is it! You can go ahead and simulate some data, $y \sim \mbox{Normal}(5, 0.5)$ and then use the function \mbox{\tt NormGibbs.R} in the {\bf
...
XXXX Maybe we make this a subsubsection... since it's still part of the 'Types of MCMC samplers' section...
...
$y_{ij}$, from $i=1,2,\ldots,n$ plots \begin{comment} Rahel: I put
  ...
  this. lets talk later. \end{comment} in $j$ different study sites, and we believe that the counts are influenced by some plot-specific covariate, ${\bf x}$, but that there is also a random site effect. So our model is:
\[
...
\end{eqnarray*}
Note that for simplicity we assume that $\beta$ is constant across the $j$ study sites, and for analysis we would set $\mu_{\beta}$ and $\sigma_{\beta}$.
...
First, we set the initial values $\alpha^{(0)}$ and $\beta^{(0)}$. Then, starting with $\alpha_1$, we draw $\alpha_1^{(1)}$ from $\mbox{Norm}(\alpha_1^{(0)}, \delta_{\alpha})$, calculate the conditional posterior density of $\alpha_1^{(0)}$ and $\alpha_1^{(1)}$  and compare their ratios,
\[
...
r = \frac{\mbox{Poisson}({\bf y}|exp(\alpha + \beta_1^{(1)}{\bf x}))
  *\mbox{Norm}(\beta^{(1)}|\mu_{\beta}, \sigma_{\beta})} { \mbox{Poisson}({\bf
    y}|exp(a + \beta^{(0)}{\bf x})) *\mbox{Norm}(\beta^{(0)}|\mu_{\beta}, \sigma_{\beta})},
\]
...
\begin{comment}
 Actually should I remove part of the code from the text -- maybe just show the updating of alpha -- and put the full code into the R package?
\end{comment}
{\small
\begin{verbatim}
...
#this lets you check the acceptance rate of a at every 100th iteration
if(iter %% 100 == 0) {  
