...
has the following form (for some of the algebra behind this, see Chapt. 2 in \citep{gelman_etal:2004}):
\begin{equation}
...
This is it! You can go ahead and simulate some data, $y \sim \mbox{Normal}(5, 0.5)$ and then use the code \mbox{\tt NormGibbs.R} in the {\bf
  R} package \mbox{\tt scrbook}
to and run your first
Gibbs sampler. 
...
\[
y \sim \mbox{Bern}(\frac{\exp(\theta)}{1+ exp(\theta)})
\]
and
\[
...
XXXX Andy, is this notation ok, or should it be 
\[
r = \frac{\mbox{Bern}(y;\theta^{(t)}) * \mbox{Normal}(\theta^{(t)};\mu, \sigma)} {\mbox{Bern}(y;\theta^{(t-1)}) * \mbox{Normal}(\theta^{(t-1)};\mu, \sigma)}
\]
XXXXXX
...
vector of all counts for site $i$. The other full conditionals for
...
First, we set the initial values $\alpha^{(0)}$ and $\beta^{(0)}$. Then, starting with $\alpha_1$, we draw $\alpha_1^{(1)}$ from $\mbox{Norm}(\alpha_1^{(0)}, \delta_{a})$, calculate the conditional posterior density of $\alpha_1^{(0)}$ and $\alpha_1^{(1)}$  and compare their ratios,
\[
...
For $\beta$, we draw $\beta_1^{(1)}$ from $\mbox{Norm} (\beta^{(0)}, \delta_{b})$, compare the posterior densities of $\beta^{(0)}$ and $\beta^{(1)}$,
\[
r = \frac{\mbox{Poisson}({\bf y}|exp(\alpha + \beta_1^{(1)}{\bf x}))
  *\mbox{Norm}(\beta_1^{(1)}|\mu_{\beta}, \sigma_{\beta})} { \mbox{Poisson}({\bf
    y}|exp(a + \beta_1^{(0)}{\bf x})) *\mbox{Norm}(\beta_1^{(0)}|\mu_{\beta}, \sigma_{\beta})},
\]
...
\[
[p|y] \propto \mbox{Beta}(a + \sum y_i, b + n-\sum y_i)
\]
\begin{comment} Rahel: above is why i don't like p for distribution! Agree. Taken care of\end{comment}
