...
% XXX Maybe remove state space example... don't want to ask the panther guys. Any other ideas?
...
Toinclude this new limit in our WinBUGS model, we need to change the following:
\begin{verbatim}
...
ymax[i]<-4+0.4*SX[i]
# use step function to see if location [SX, SY] is below the Y limit (pIn = 1) or not (pIn = 0)
...
use the actual state-space polygon to constrain our ${\bf s}_i$. 
...
In other circumstances you may still want to create your state space as before, by adding some buffer to your trapping grid, but you may find that the resulting rectangle includes water bodies, paved parking lots or any other kind of habitat you know is never used by the species you study. 
