...
%%% BETH: meaning of the rest of this sentence is unclear? XXXXX
%% of which the
...
%%% BETH: Later in the chapter you give commands for connecting the
%% the bear data, through R, with the BUGS model specification for
%% a SEX model.  I wonder if you should include HERE a similar
%% set of R instructions for connecting the bear data with the BUGS
%% model shown in THIS PANEL? (below)???
...
%%% BETH: nind should be the size of the augmented data set M right?
%%% I think you need a few sentences somewhere where you  bring that 
%% up. It is worth saying that the observed DATA are the nind x ntraps
%% x nreps BUT THAT, for the bayesian analysis, you have to do data
%% augmentation and bring your data set up to size M
...
%%% BETH: Next 2 lines are contractory. Estimates were similar. But the
%%% estimate of N was smaller? Which is it?
...
model. 
%%% BETH: In the example analysis is the variable SEX a binary 0/1
%%% variable or is it a CATEGORICAL Variable having values 1/2? This
%%% makes a difference -- if the former, then the following is
%%% correct. If the latter then the following is NOT CORRECT and it
%%% should be categorical and the cell probabilities need to be a
%%% vector of length 2. I think in this case below it is 0/1 so we're
%%% good right?
...
%%% BETH: I changed SEX to be Sex -- in trying to be consistent with
%%% some other covariate models in other chapters
%% BETH: Is beardata$sex a cateogircal variable? So you're convertin
%% it to binary? Please add a sentence or two to the above paragraph
%% to clarify that point in the paragraph (I see you have a comment
%% below but I think saying it in the text will set the reader up to
%% be understand whats going on)
{\small
\begin{verbatim}
...
%%% ALSO: I think you should say something in the text about the fact
%%% that you're converting freely between categorical and Bernoulli
%%% versions of the random variable Sex. This is confusing to some readers. 
\begin{panel}[htp]
\centering
\rule[0.1in]{\textwidth}{.03in}
{\small
...
finite-mixture models using \secr~ in
...
parameter of the encounter  probability model $\sigma^{2}$ or its inverse
$\alpha_1 = 1/\sigma^{2}$. We might thus assume a distribution for
either $\sigma^{2}$ or its inverse, $\alpha_1$.  We call this 
...
\begin{figure}[ht]
\begin{center}
\includegraphics[height=3.5in,width=3.5in]{Ch8-Covariates/figs/fig1.ps}
\end{center}
\caption{
...
%% BETH: Point of this seemed unclear so I commented it out. 
