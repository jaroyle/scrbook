...
\[
 AIC = -2 \mbox{\tt logLikelihood}  + 2 \mbox{\# parameters}
\]
...
small-sample adjustment and so forth but we cannot provide strict guidelines
...
for producing a model selection table and also doing model-averaging
which we recommend for obtaining estimates of density from multiple models.
As an example of using \mbox{\tt secro}, we consider the wolverine camera trapping data
and consider 4 distinct models: (1) model SCR0 with constant parameter
values for both male and female wolverines but with a parameter
$\psi_{sex}$ the population proportion of males; (2) sex-specific intercept
$p_{0}$ but constant $\sigma$; (3) sex-specific $\sigma$ but constant
$p_{0}$ and (4) sex-specific $p_{0}$ {\it and} $\sigma$. 
To fit these models we use the multi-session formulation provided by
\mbox{\tt secr} (introduced in sec. \ref{mle.sec.multisession}). This suggests one additional model in
our set, which is that with no sex effect on density, equivalent to
having $\psi_{sex} = 0.5$ instead of estimating it. We will label this
last model ``Model 0'' and models 1-4 in our list as Model 1, 2, 3,
and 4, respectively. The specific
formulation of sex effect models is discussed in sec. XXXXX. 
...
these commands and resulting output look like this:
{\small 
\begin{verbatim}
...
$\bullet$ gives estimates in individuals per ha.

$\bullet$ so we have to scale the density estimates and for $\sigma$
to be in the desired scale, which is usually per $km$ or per $100$
$km$ for large carnivores.

$\bullet$ We don't agree with the use of AICc here and think its better to use
...
Anyhow, by looking at the best model by AIC, we find that the model
with estimated sex ratio and sex specific $g_{0}$ is preferred. This
is just slightly better than the model with a fixed sex ratio of
$\psi_{sex} = 0.50$ which is not a reasonable model at all. 



We do the same things with the mask which excludes ocean. We should do
this right? So we package all of the results in Table. XXX along with
the model-averaged estimates of density for the two sets of models.
...
Xobs       92.18  5.61  82.01  88.23  91.89  95.77 104.00    1 60000
Xnew       65.19  6.12  53.66  61.02  64.99  69.17  77.66    1 43000
X1obs      12.80  3.35   7.26  10.41  12.44  14.83  20.39    1 38000
X1new       9.66  2.75   5.15   7.70   9.37  11.29  15.85    1 60000
X3obs      21.70  2.13  18.19  20.17  21.46  22.96  26.52    1 11000
X3new      13.19  3.29   7.62  10.85  12.91  15.23  20.41    1 60000
...
Xobs       91.06  5.54  81.03  87.20  90.76  94.60 102.70    1 18000
Xnew       64.50  6.11  53.13  60.30  64.30  68.53  76.95    1 60000
X1obs      12.26  3.33   6.77   9.87  11.90  14.29  19.68    1 19000
X1new       9.79  2.76   5.19   7.82   9.51  11.48  15.97    1 33000
X3obs      21.56  2.03  18.15  20.12  21.34  22.77  26.10    1  2800
X3new      13.18  3.28   7.63  10.84  12.89  15.20  20.36    1 60000
...
Xobs       92.13  5.60  82.06  88.20  91.81  95.71 103.90    1 18000
Xnew       65.10  6.10  53.63  60.89  64.88  69.12  77.56    1  7800
X1obs      12.83  3.34   7.33  10.42  12.48  14.87  20.28    1  6500
X1new       9.66  2.72   5.20   7.72   9.37  11.29  15.81    1 14000
X3obs      21.69  2.14  18.16  20.17  21.46  22.97  26.50    1 20000
X3new      13.17  3.28   7.63  10.84  12.88  15.17  20.42    1 60000
...
Xobs       90.97  5.55  80.97  87.09  90.65  94.53 102.70    1 60000
Xnew       64.40  6.07  53.08  60.21  64.20  68.35  76.96    1 53000
X1obs      12.24  3.31   6.77   9.87  11.91  14.22  19.71    1  5100
X1new       9.81  2.77   5.21   7.83   9.53  11.49  16.01    1 47000
X3obs      21.50  2.02  18.11  20.07  21.30  22.73  25.99    1  7900
X3new      13.18  3.26   7.67  10.85  12.90  15.20  20.35    1 60000
...
DIC is an estimate of expected predictive error (lower deviance is better).
> print(toad4,digits=2)
Error in print(toad4, digits = 2) : object 'toad4' not found
