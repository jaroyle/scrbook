...
be more likely to 
return subsequently -- the response of
trap happiness.
...
SCRgof(bleen,5,5,traplocs=traplocs,buffer=.4)
...
provides an adequate fit to the data.  A similar result is obtained if
we use the fully sex-specific model. 

Next, we did a Bayesian p-value analysis of the observation component
of the model, using the 3 fit statistics
described previously. These statistics can be calculated as part of
...
calculations. For example, to compute the fit-statistic 1... we have
to do this:
\begin{verbatim}
SAME CODE AS ABOVE -- OFF DESKTOP DIRECTORY
\end{verbatim}
...
heterogeneous than suggested by the model. We
...
behavioral response which can be fitted using the function \mbox{\tt
...
[1] 0.4990667
\end{verbatim}
Posterior summaries are given in Tab. XXXXXX. (below).
