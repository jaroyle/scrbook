...
this in the literature.
 We break the problem up into 2 components which we attack
separately (Royle et al. 2011): Does the observation model fit?  Does
the uniformity assumption appear adequate? The latter has a huge
...
al. (2011) applied to SCR models. Really this is a test only of a form
of ``spatial randomness'' because CSR implies the use of a Poisson
point process which, as we discussed, is not universally adopated in
our implementation of SCR models.
...
which we call the
{\it model selectors paradox}: we carryout inference using parametric inference paradigms
...
falsibiable, but actually false.''\footnote{I like that quote}. 

We apply these ideas of model selection and assessment to the
wolverine data --- sex specificity of parameters and behavioral
response -- and black bear data?
...
We talk here of variable selection methods.  i.e., fixed effects. AIC
is the standard for likelihood analysis of models and it is versatile
and flexible for fixed effects.
For Bayesian analysis we rely on different methods.
...
Mallick indicator variable selection approach
 ...
 BUGS packages can be tempramental, it produces direct statemtns of
...
\citet[][Chapt. XXXX]{link_barker:2010}
...
effort: (1) Choosing among models that represent distinct, meaningful biological hypotheses; and, (2) choosing among
different parametric encounter probability models.  
...
\subsection{AIC and \mbox{\tt secr}}
...
for producing a model selection table and doing model-averaging.
As an example of that, we consider the black-bear models : WHICH
MODELS? and here is the code for generating the AIC Table:
\begin{verbatim}

\end{verbatim}
We provide an AIC table for the wolverine camera trapping data in
sec. XXXX below. 
...
the "variable weights" idea from \citep{kuo_mallick:1998}.
...
beta.sex   -0.01  1.73  -2.85  -1.50  -0.01   1.49   2.85    1 60000
alpha.sex   0.00  1.73  -2.85  -1.50   0.00   1.50   2.85    1 60000
psi         0.30  0.07   0.19   0.25   0.30   0.34   0.45    1 25000
psi.sex     0.52  0.10   0.32   0.45   0.52   0.59   0.72    1 60000
sigma       0.64  0.06   0.54   0.60   0.64   0.67   0.76    1  3100
logitp0    -2.82  0.18  -3.17  -2.94  -2.82  -2.70  -2.48    1 12000
N          60.03 11.89  40.00  52.00  59.00  67.00  86.00    1 17000
D           5.79  1.15   3.86   5.01   5.69   6.46   8.29    1 17000
beta        1.25  0.21   0.87   1.10   1.24   1.38   1.70    1  3100
deviance  441.95 12.56 419.70 433.10 441.20 449.90 468.50    1  6100
DIC info (using the rule, pD = var(deviance)/2)
pD = 78.8 and DIC = 520.8

Model 1:
            mean    sd   2.5%    25%    50%    75%  97.5% Rhat n.eff
beta.sex    0.00  1.74  -2.85  -1.51  -0.01   1.52   2.86    1 35000
alpha.sex  -0.74  0.34  -1.43  -0.97  -0.74  -0.51  -0.08    1 60000
psi         0.30  0.07   0.18   0.25   0.29   0.34   0.44    1 15000
psi.sex     0.56  0.10   0.36   0.49   0.56   0.63   0.75    1  1500
sigma       0.66  0.06   0.56   0.62   0.65   0.69   0.79    1  6500
logitp0    -2.45  0.25  -2.92  -2.62  -2.45  -2.28  -1.95    1 34000
N          58.91 11.71  39.00  51.00  58.00  66.00  85.00    1 17000
D           5.68  1.13   3.76   4.92   5.59   6.36   8.19    1 17000
beta        1.18  0.21   0.80   1.03   1.17   1.31   1.61    1  6500
deviance  440.17 12.51 417.80 431.40 439.50 448.10 466.70    1 10000
DIC info (using the rule, pD = var(deviance)/2)
pD = 78.2 and DIC = 518.4

Model 2:
            mean    sd   2.5%    25%    50%    75%  97.5% Rhat n.eff
beta.sex    1.36  1.39  -2.33   0.71   1.73   2.44   2.95 1.05    58
alpha.sex   0.01  1.73  -2.84  -1.48   0.01   1.51   2.86 1.00 60000
psi         0.30  0.07   0.19   0.25   0.30   0.34   0.44 1.00  1000
psi.sex     0.52  0.10   0.32   0.45   0.52   0.59   0.72 1.00 52000
sigma       1.52  0.77   0.20   0.90   1.50   2.14   2.91 1.05    57
logitp0    -2.82  0.18  -3.16  -2.94  -2.82  -2.70  -2.47 1.00 29000
N          59.76 11.79  40.00  51.00  59.00  67.00  86.00 1.00   850
D           5.76  1.14   3.86   4.92   5.69   6.46   8.29 1.00   850
beta        1.23  3.20   0.06   0.11   0.22   0.61  12.87 1.05    57
deviance  441.64 12.35 419.70 432.90 440.90 449.50 468.10 1.00  4100
DIC info (using the rule, pD = var(deviance)/2)
pD = 76.3 and DIC = 517.9

Model 3:
            mean    sd   2.5%    25%    50%    75%  97.5% Rhat n.eff
beta.sex    1.50  1.33  -2.11   0.88   1.90   2.50   2.95 1.08    63
alpha.sex  -0.74  0.35  -1.44  -0.97  -0.74  -0.50  -0.06 1.00  3400
psi         0.30  0.07   0.18   0.25   0.29   0.34   0.44 1.00  9700
psi.sex     0.56  0.10   0.35   0.49   0.56   0.63   0.75 1.00  1800
sigma       1.65  0.79   0.22   1.02   1.68   2.27   3.02 1.08    65
logitp0    -2.45  0.25  -2.93  -2.62  -2.45  -2.28  -1.94 1.00  1900
N          59.16 11.95  39.00  51.00  58.00  66.00  86.00 1.00  7000
D           5.70  1.15   3.76   4.92   5.59   6.36   8.29 1.00  7000
beta        1.02  2.96   0.05   0.10   0.18   0.48   9.94 1.08    65
deviance  440.35 12.64 417.90 431.50 439.50 448.30 467.30 1.00  3700
DIC info (using the rule, pD = var(deviance)/2)
pD = 79.8 and DIC = 520.2

Model 5: 9/4/2012 -- NEED TO RERUN THIS HAD WRONG MODEL SPECIFICATION
            mean    sd   2.5%    25%    50%    75%  97.5% Rhat n.eff
mod[1]      0.61  0.49   0.00   0.00   1.00   1.00   1.00 1.00  1900
mod[2]      0.61  0.49   0.00   0.00   1.00   1.00   1.00 1.04    64
beta.sex    0.83  1.65  -2.64  -0.46   1.19   2.28   2.95 1.05    45
alpha.sex  -0.46  1.17  -2.61  -1.04  -0.68  -0.24   2.62 1.00  3300
psi         0.30  0.07   0.18   0.25   0.29   0.34   0.44 1.00 15000
psi.sex     0.55  0.10   0.34   0.47   0.55   0.62   0.74 1.00  2300
sigma       1.20  0.78   0.33   0.63   0.75   1.79   2.91 1.09    27
logitp0    -2.58  0.29  -3.08  -2.80  -2.60  -2.38  -2.00 1.01   440
N          59.47 11.81  40.00  51.00  58.00  67.00  86.00 1.00 21000
D           5.73  1.14   3.86   4.92   5.59   6.46   8.29 1.00 21000
beta        1.03  1.56   0.06   0.16   0.89   1.28   4.67 1.09    27
deviance  440.58 12.54 418.20 431.80 439.80 448.50 467.20 1.00  4000
DIC info (using the rule, pD = var(deviance)/2)
pD = 78.6 and DIC = 519.1
\end{verbatim}
...
model.
For a model set in which all submodels are nested subsets of the one larger
model, such as for models containing explicit covariates,
we expand the model to include the latent
variables say $I_{m}$ for each variable in the model that pre-multiply
covariates. 
...
\[
logit(p_{ij}) = \alpha_{0} + \alpha_{1}I_{1} x_{1,ij} + \alpha_{2}I_{2} x_{2,ij} 
\]
In this case, model identity is the sequence of $I$ variables
defining unique models.

Model selection by this method was covered in
various places -- e.g., Royle 2009, sec. 3.xxx in \citep{royle_dorazio:2008} and
maybe in the Kery and Schaub book, and in the context of SCR by 
 Tobler et al. (in
review).
...
We did this for the wolverine data with sex as a covariate on both
sigma and the intercept. here are the results. rerun.
