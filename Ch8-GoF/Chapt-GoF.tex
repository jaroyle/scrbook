...
For most SCR models,
there are at least two distinct components to model fit, and we
propose to evlauate these two distinct components of fit individually.
First, we can ask, does the model explain the {\it observation} process,
conditional on the underlying point process? We can evaluate this based on the encounter frequencies of
individuals {\it conditional} on the underlying point process ${\bf s}_{1}, \ldots, {\bf s}_{N}$.
We discuss some potential fit statistics in the next section.
Second, we can
evaluate whether the data appear consistent with the  ``uniformity'' assumption about the point process.
 For the simple model of independence and uniformity,
this is similar to the assumption of {\it complete spatial randomness} (CSR)
which we consider in sec. XXX.YYY below. Actually,
this is not strictly the CSR assumption  because of the binomial
assumption on $N$ under data augmentation, but we refer to it as CSR because it is practically
...
things is Cressie (1996; ch. 8) and PPBOOK XYZ.XYZ\footnote{We also
  like Tony Smith's lecture notes (Univ. of Penn. ESE 502), which can
  be found at
...
Standard deviance
based GoF statistic for binary regression are not useful.  
...
Then we typically recommend the XXXX statistic:
\[
 \sum_{g}  (\sqrt(n_{g}) - \sqrt(E(n_{g})))^{2}
\]
...
\[
T = \sum_{g \st n_{g} = 1}^{N}  (1 - \sqrt(N/G))^2 +
\sum_{g \st n_{g} = 1}^{G-N} (N/G)^2
 = N(1 + (G-N)/G)
\]
...
Richard said someting about this.................

 
 
Yeah, they talk about it starting on page 86, Sec 2.7.2. Table 2.5 (page 88) even shows how the chi-squared test is sensitive to the grid size. 

After this, they discuss a generalization of the chi-squared test in which counts in the neighboring grid cells are taken into account. Then they move on to "distance methods".
