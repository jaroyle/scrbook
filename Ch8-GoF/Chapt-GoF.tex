...
\[
 p_{V} - Var(D(\theta)|{\bf y})/2.
\]
We think this is what JAGS and BUGS is doing but we're really not
sure. Its not just us -- no one is.  There are 20 ways to implement or
compute AIC depending on which packages you use. If you run jags with
R2jags or use WinBUGS then it computes the cheap version by default. 
If you run  jags through the rjags package then you have to compute
DIC differently somehow. 


We give the model here which we used to fit a bunch of models -- the 4
mentioned above, but not ``Model 0'' which assumed $\psi_{sex} =
0.5$. We have an R function in the scrbook repo to show how we can fit
each model. It goes like this:
...
> toad4<-wolvSCR0.fn(y3d,traps,wsex=wsex,nb=1000,ni=21000,delta=2,M=200,model=4)
\end{verbatim}
