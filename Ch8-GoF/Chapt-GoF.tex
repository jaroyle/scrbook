...
\centering
\caption{
XXX replace these results with those above XXXXX
...
Using this indicator variable formulation of the model selection problem we can characterize
...
{\bf XXXXX CHECKING RESULTS NOW (12/17/2012) XXXXXXXXXX}
...
things is \citet[][Ch. 8]{cressie:1992} and \citet{illian_etal:2008}\footnote{We
...
(Chapts. \ref{chapt.state-space}, \ref{chapt.ecoldist}, \ref{chapt.rsf}), the
...
in the function \mbox{\tt wolvSCR0gof}.
In addition, we have an {\bf R} function
\mbox{\tt SCRgof} which processes the output from a {\bf BUGS} run to
compute the test of spatial randomness based on quadrat counts and the
Bayesian p-value calculation. The function \mbox{\tt SCRgof} requires
 a few things: (1) the output from a
...
toad4<-wolvSCR0gof(nb=1000,ni=6000,buffer=2,M=200,model=4)
...
> 
> # crop tight to trap grid. Remember the buffer was 2 units.
> a<-SCRgof(bleen,5,5,traplocs=traplocs,buffer=.4)
Error in sample(G[, 1], sum(z[i, ]), replace = TRUE) : 
  object 'G' not found
> rm(SCRgof)
Warning message:
In rm(SCRgof) : object 'SCRgof' not found
> fix(SCRgof)
> fix(SCRgof)
> a<-SCRgof(bleen,5,5,traplocs=traplocs,buffer=.4)
Cluster index observed:  16.49733
Cluster index simulated:  15.10144
P-value  index of dispersion:  0.4130667
P-value2 freeman-tukey:  0.6868
> 
> bleen<- toad4$BUGSoutput$sims.list
> 
> traplocs<-wolverine$wtraps[,2:3]
> traplocs[,1]<-traplocs[,1] -min(traplocs[,1])
> traplocs[,2]<-traplocs[,2]- min(traplocs[,2])
...
> # crop tight to trap grid. Remember the buffer was 2 units.
> a<-SCRgof(bleen,5,5,traplocs=traplocs,buffer=.4)
Cluster index observed:  15.92149
Cluster index simulated:  15.00488
P-value  index of dispersion:  0.4404
P-value2 freeman-tukey:  0.7132667
\end{verbatim}
...
1. behavioral response model.... does it fit?
RAN THIS WITH BUFFER = 1 NEED TO CHANGE
\begin{verbatim}
...
XXXX How does D estimate compare with chapter SCR0 result? XXXXXX

\begin{verbatim}
> print(toad4b,digits=2)
Inference for Bugs model at "modelfile.txt", fit using WinBUGS,
 3 chains, each with 6000 iterations (first 1000 discarded)
 n.sims = 15000 iterations saved
            mean    sd    2.5%     25%     50%     75%   97.5% Rhat n.eff
psi         0.24  0.06    0.14    0.20    0.24    0.28    0.38    1  2900
sigma       0.87  0.11    0.68    0.80    0.87    0.94    1.12    1  1300
p0          0.01  0.00    0.01    0.01    0.01    0.01    0.02    1   720
N          47.97 10.82   31.00   40.00   47.00   54.00   73.00    1  1700
D           7.16  1.62    4.63    5.97    7.02    8.06   10.90    1  1700
beta        0.69  0.17    0.40    0.56    0.67    0.79    1.08    1  1300
alpha2      2.49  0.26    2.00    2.31    2.49    2.66    3.03    1   650
X1obs      54.46  6.07   43.54   50.24   54.15   58.34   67.16    1  2100
X1new      64.28  7.51   50.51   59.12   63.89   69.12   79.89    1  5100
X2obs      13.30  4.00    6.63   10.41   12.89   15.72   22.05    1 15000
X2new      11.74  3.26    6.28    9.40   11.44   13.72   18.97    1  5800
X3obs      12.92  1.78    9.91   11.65   12.75   13.98   16.91    1  2300
X3new      12.92  3.05    7.79   10.77   12.63   14.80   19.63    1  6200
deviance 1126.23 14.49 1100.00 1116.00 1125.00 1135.00 1157.00    1   920

For each parameter, n.eff is a crude measure of effective sample size,
and Rhat is the potential scale reduction factor (at convergence, Rhat=1).

DIC info (using the rule, pD = var(deviance)/2)
pD = 104.7 and DIC = 1231.0
DIC is an estimate of expected predictive error (lower deviance is better).
\end{verbatim}


It might be useful to try these fit assessment exercises using the
clipped state-space as described in sec. XXXXXX. That takes a long
...
\citep[e.g.,]{royle_etal:2009ecol, royle_etal:2011mee}.  We offered up
...
general strategy is a good one, there is essentially nothing known at
all about power to detect various departures and we know of no studies
of power.  There is not a lot of guidance in the literature on model
assessment or goodness-of-fit evaluation for SCR models using
likelihood methods. Although we haven't discussed this explicitly, we
...
Conversely, inferences should be somewhat more sensitive to
bad-fitting models for the observation process. Inference about total
population size is known to be highly sensitive to the observation
model \citep{dorazio_royle:2003, link:2003}.
