...
(i.e., ``goodness-of-fit''), we rely exclusively on Bayesian p-values
\citep{gelman_etal:2006}.  For checking adequacy of SCR models, part
of the challenge is coming up with good summaries of model fit, and
there does not appear to be any guidance on this in the literature.
Following \citet{royle_etal:2011mee}, we break the problem up into 2
components which we attack separately: (1) Does the encounter model
fit? (2) Does the uniformity assumption appear adequate? The latter
...
over-stating the relevance of any model.
...
there seems to be many variations of DIC, and a consistent version is
not always
reported across computing platforms. 
%Our own experience with
%calibration has indicated highly variable effectiveness of DIC. 
Even
 academic
statisticians don't have general agreement on practical issues related
to the use of DIC \citep{millar:2009}. Despite this, it is still
widely reported. We think DIC is probably
reasonable for certain classes of models that contain only fixed
effects, or for which the latent variable structure is the same across
models so that only the fixed effects vare varied (this covers 
many SCR model selection problems).

Model deviance is defined as negative 2 times the loglikelihood; i.e.,
...
\citep{gelman_etal:2004} suggest a different version of $p_{D}$ based on
...
run
from \mbox{\tt R2WinBUGS} or \mbox{\tt R2jags}, respectively.
...
We repeated the analysis of models 1-4 for the wolverine data, but
this time doing a Bayesian analysis paralleling the likelihood analysis we
above in \mbox{\tt secr}.  We analyzed models 1-4
 but not ``model 0'' which implies that  $\psi_{sex} = 0.5$, which we
 do not feel is a biologically meaningful hypothesis.
Following our analysis in \mbox{\tt secr} above, we used the
$\mbox{logit}(p_{0}), \mbox{log}(\sigma)$ parameterization of the models:
\[
 \mbox{logit}(p_{0,i}) = \alpha_{0} + \alpha_{sex} \mbox{\tt sex}_{i}
\]
and
...
the ovenbird data in sec. XXXXXXX [ch.mn-pois.ovenbird] and also the
discussion of sex effects models in Chapt. \ref{chapt.covariates}.
...
kernel of the BUGS model specification is the model in Panel 
\ref{gof.panel.sexmodel}.
...
{\small
\begin{verbatim}
 toad1<-wolvSCR0ms(y3d,traps,wsex=wsex,nb=1000,ni=21000,delta=2,M=200,model=1)
 toad2<-wolvSCR0ms(y3d,traps,wsex=wsex,nb=1000,ni=21000,delta=2,M=200,model=2)
 toad3<-wolvSCR0ms(y3d,traps,wsex=wsex,nb=1000,ni=21000,delta=2,M=200,model=3)
 toad4<-wolvSCR0ms(y3d,traps,wsex=wsex,nb=1000,ni=21000,delta=2,M=200,model=4)
\end{verbatim}
}
{\bf XXXX We should change delta=2 to buffer=2 in all of this code,
  and on the repo. This was dumb. XXXXXXX}
The {\bf R} script in \mbox{\tt wolvSCR0ms}
...
parameters.   {\bf XXXXX probably requires some notation changes
  XXXXXXX}
{\bf I changed ncaps to y. Check repo code. }
}
...
Our {\bf R} script \mbox{\tt wolvSCR0ms} in \mbox{\tt scrbook} also
provides the model indicator variable implementation for the fully
sex-specific SCR0 model.  It is run by setting \mbox{\tt model=5}. We
...
pure prior and being informed by the data. On the other hand, the
...
averaging is done as part of the MCMC.
 The variable 'mod'
...
adequate description of our data? Put another way, we might ask, are
the data we have consistent with realizations from the model which we
just fitted and upon which our inferences are based?  Formal
...
the averaged model have to fit? If none of our models fit, should we
...
model, as if the model was a cherished family heirloom of great
...
Conceptually, we can think of model selection as follows: if we
simulate data under that model, do the simulated realizations resemble
...
provides what is essentially a basis for falsification of the model,
...
that model. This is not really true in a rigorous sense. Rather, it
could mean we're missing something, or many things. The basic model
...
$\hat{\theta}$ and compare realizations with what we've observed.  In
...
for posterior samples of $\theta$ (we briefly introduced the Bayesian
...
actual data.  Whether Bayesian or frequentist, the main idea for
...
increases to some extent (but perhaps not as much as having inherent
...
REF HERE ??? XXXX. The {\bf R} package \mbox{\tt unmarked}
...
\citep[e.g., see][for an application]{sillett_etal:2012}.  Similarly,
for Bayesian analysis of SCR models, there has not been a definitive
or general proposal for a fit statistic or even a class of fit
statistics, although a few specialized implementations of Bayesian
p-values have been provided \citep{royle_etal:2009,royle_etal:2011mee,
...
For most SCR models, there are at least two distinct components to
model fit, and we propose to evlauate these two distinct components of
fit individually.  First, we can ask, does the model explain the {\it
...
$N$ under data augmentation, but we refer to it as CSR because it is
...
things is Cressie (1996; ch. 8) and Illian et al. XXXXXX\footnote{We
...
CSR hypothesis is manifestly false, purely on biological
...
heterogeneous habitat wil.  generate the appearance of clustering even
...
\[
   I =  (\# cells -1)*s^2/\bar{n}
\]
which has approximately a Chi-square distribution on $(ncells - 1)$ degrees-of-freedom.  If $s^2/nbar
...
XXXXXXXXX slightly out of order here.... XXXXXXXXXXXXXXX
As an alternative to the index-of-dispersion, we 
use a 
...
we tabulated $n(g)$ the number of activity centers in bin $g$.
 A standard goodness-of-fit statistic for CSR is based on the
...
value of $I$ obtained by simulating ${\bf s}$ under CSR.  XXXXX
redundant XXXXXXX
...
Then we typically recommend the Freeman-Tukey kind of statistic:
\[
...
the binned counts and their expected values. We have an R function
XXXXX in the R package to do this....... 
...
We could also have the summaries individual x replicate , aggregated
over traps. 
We imagine that all of these are probably too disaggregated in
practical settings. 
...
Bayesian model selection can be tediously slow even for small data sets.
...
There are technicazl issues that are features of Bayesian analysis
which we can't do anything about, sensitivity to priors being the
important one. That is life and you should just be aware of it and
don't put too much faith in model selection, in general. 

Goodness-of-fit assessment is pretty experimental in all of SCR
models. There is not a lot of 
guidance in the literature on GoF in SCR models although we have
offered up a general framework based on independent testing of the
spatial model of independence and uniformity, and testing fit of the
observation model conditional on the underlying point process.  Well
we think this general strategy is a good one, 
...
WE used a classical quadrat binning approach to testing model fit,
although other approaches from spatial point process modeling should
be pursued including 
nearest-neighbor methods or distance-based metrics. 

A question of great practical relevance is what do we do if our model
