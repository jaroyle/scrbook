...
Idea here is to expand the model to include every model in some set as
specific rejections. We can do this by introducing a set of latent
variables that pre-multiply covariates, or we can also introduce a
categorical model variable that allows the Markov chain to switch from
one model to the next.   This kind of stuff has been covered in
various places -- e.g., sec. 3.xxx in Royle and Dorazio XXXXX and
maybe in the Kery and Schaub book?  Other historical material..... 
...
identify a test
statistic, with known asymptotic distribution, to evaluate
goodness-of-fit using a chi-square statistic. Examples from the closed
capture-recapture setting includes XXXXXX XXXXXXXX.
When this is not the case, 
goodness-of-fit is ususally assessed using 
bootstrap methods. Using a bootstrap method we identify a fit
statistic and we compute the value for  which is more or less the standard
approach, in general.


The basic idea is to come up with a fit statistic that depends on the
parameters and the data set, 
$T({\bf y}, \theta)$ and then
 we compute this for the data set and then for simulated data sets
 using the MLES.
We provide an example here. 
A problem with SCR models is  coming up with good fit statistics. 
Bayesian model assessment is similar -- we come up with a fit
statistic and compare it to values for simulated data. The thing is
that this happens using posterior simulated values so we ask is the
value of the fit statistic, given a particular value of $\theta$ from
the posterior, similar to values of the fit statistic for that value
of $\theta$, but for perfect data sets?
So both Bayesian and classical methods of assessing goodness-of-fit
require that we identify fit statistics and then do some kind of a
