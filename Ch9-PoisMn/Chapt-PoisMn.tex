...
In this chapter, we focus on developing additional observation
models.
The observation model could be thought of as
being determined by the type of device -- or the type of ``detector''
using the terminology of \mbox{\tt secr} \citep{efford:2011}.
We consider models that apply when observations
are not binary and, in some cases, that do not require
independence of the observations.  We present models 
when the data are  encounter {\it frequencies}, based on the Poisson
...
such as the negative exponential or the bivariate Gaussian kernel, and
...
model.  
%We ran that model with \mbox{\tt buffer=300}. For values of
%200 or less,  \mbox{\tt
...
\[
 \Pr(y_{i,t} = 1| {\bf u}_{it} )  = p_{0}I({\bf u}_{it} \in {\cal X}).
\]
