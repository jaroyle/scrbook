...
while(count <= 100) {
  ...
  pr <- exp(beta*elev.fn(s.cand)) / int2d(beta)
  Q <- max(c(exp(beta*elev.min) / int2d(beta),
             exp(beta*elev.max) / int2d(beta)))
  if(runif(1) < pr/Q) {
    s[count,] <- s.cand
    count <- count+1
    }
...
obtained an estimate of $\hat{\beta}=2.47$. We could plug in
...
\hline
 secr & $N$ & 51.6743 & 6.1397 & 42.6942 & 67.5503 \\
      & $\beta$ &  2.0443 & 0.6033 &  0.8618 &  3.2269 \\
      & $\lambda_0$ &  0.7063 & 0.0679 &  0.5853 &  0.8525 \\
      & $\sigma$ &  0.0979 & 0.0048 &  0.0891 &  0.1077 \\
 JAGS & $N$ & 48.5695 & 5.5291 & 39.0000 & 61.0000 \\
      & $\beta$ &  2.1778 & 0.5695 &  1.0455 &  3.3087 \\
      & $\lambda_0$ &  0.6841 & 0.0640 &  0.5665 &  0.8161 \\
      & $\sigma$ &  0.1020 & 0.0048 &  0.0935 &  0.1123 \\
\hline
\end{tabular}
