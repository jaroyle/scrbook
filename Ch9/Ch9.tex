...
model used in Chapts. \ref{chapt.scr0} and 5 and the half-normal detection function,
...
study introduced in Chapter 3, using the software
\jags.  We also
...
We assume a standard sampling design in which an
...
In some studies it will make sense to model encounter probability as a
...
such as whether a trap is baited or not, or how many traps were set,
...
resource selection models yield precisely an additive model of the
form here, where the covariate $z({\bf x})$ is the same as that
contained in the resourc eselection model. 
