...
In Chapter 4, we developed the basic capture recapture model using  a
...
\]
We argued (section XXXX) that this model 
...
For example, the most commonly used detection
...
realistic models in Chapter XXXXX.
...
0 & halfnormal &$g_0$, $\sigma$                 &  $g(d) = g0 * exp\{-d^2 / (2  \sigma^2) \}$  \\
1 &hazard rate & $g_0$, $\sigma$, z              &  $g(d) = g0 * (1 - exp(- (d / \sigma) ^(-z) ))$ \\
2 &exponential              &$g_0$, $\sigma$    &  $g(d) = g0 * exp(- d / \sigma)$ \\
3 &compound halfnormal      & $g_0$, $\sigma$, z & $g(d) = g0 * [1 - \{1 - exp(-d^2 / (2 \sigma^2))]^z\}$ \\
4 &uniform                  & $g_0$, $\sigma$     & 
...
6 &annular normal           & $g_0$, $\sigma$, w & $g(d) = g0 * exp(-(d-w)^2 / (2 \sigma^2))$ \\
7 &cumulative lognormal     & g0, $\sigma$, z & $g(d) = g0 [1 -F{(d-\mu)/s)}]$  \\
8 &cumulative gamma         &g0, $\sigma$, z  & $g(d) = g0 \{ 1 - G (d; k,  \theta) \}$  \\
...
on the baseline parameter $p_{0}$ or the scale parameter
$\sigma$......................
...
Individual covariates we observe on the captured guys......

Sex is a special kind of covariate because we can observe it for those
individuals we encounter, but sometimes it might be missing because
...
probability ``alpha'' and also it might affect the typical home range
size, which should cause ``alpha1'' to vary by sex.  The model
structure could generally look like this:
\[
...
Sex is often missing ..... we can easily deal with this in a Bayesian
estimation framework. See Ch. 7 and Gardner et al. 2010JWM and
elsewhere....
...
Ft. Drum bear study.  This data set was first introduced in Chapter 3,
...
``captured'' at least once.  The data are provided in the {\bf R}
