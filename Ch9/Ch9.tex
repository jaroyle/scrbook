...
(e.g., \Verb+y \mytilde x+).
Thus, in \secr we
might have g0 ~ behavior or sigma ~ time; when left unspecified or set
to 1 (e.g., g0 ~ 1), this will default to a model with no covariates
that is constant.  .  Additionally, we can specify covariates in secr
on density, which are set for example as D ~ habitat.  To demonstrate
...
shown in Chapter *5*, we then call the command secr.fit to run the
model.  All of the following models should take somewhere between 30
...
your memory here how to read in and format the data and run the basic
model with no covariates and a half normal distance function in secr
for the Ft. Drum bear study: {\small
