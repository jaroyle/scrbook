...
However, in some cases, we might sample over longer time frames and expect the date to be important for a
...
in the \scrbook package, a function called secr.bear that will 
format the data and then fit 8 models (details shown in panel 
...
Running the function secr.bear, which calls all of the models we
have described, will return
an AIC table with all of the summarized results including the AIC values,
delta AIC, and model weights. The resulting table is shown in Table XX ref XX. 
...
with a local trap response on detection, ``bk'', has a model weight of 1.
The 2-part finite mixture model for $g0$ and sigma has the second lowest
AIC.  Using the AIC provides a fast and convenient mechanism for
