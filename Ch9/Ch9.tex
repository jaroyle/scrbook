...
Discrete space versions of these models can be fitted using
\verb+BUGS+. As discussed in Chapter 5, we can define $s_i$ as the
pixel id, and use the categorical distribution as a prior. A good
...
Fig.~\ref{ch9:fig:hetero}. The \textbf{BUGS} code to fit the model is shown in
the following panel.
...
A complete {\bf R} script to conduct the analysis is presented in the
online supplement.
