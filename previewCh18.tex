\documentclass{book}

\usepackage{float}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{graphicx}
\usepackage{lineno}
\usepackage{natbib}
%\usepackage[pdftex,hidelinks,pdfstartview=FitH,pdfpagemode='UseNone']{hyperref}
\usepackage[pdftex,linkcolor=black,urlcolor=white,pdfstartview=FitH,pdfpagemode='UseNone']{hyperref}
\usepackage{verbatim}

\bibliographystyle{asa}

\floatstyle{plain}
\floatname{panel}{Panel}
\newfloat{algorithm}{h}{txt}[chapter]
\newfloat{panel}{h}{txt}[chapter]


\newcommand{\R}{\textbf{R}}
\newcommand{\bugs}{\textbf{BUGS}}
\newcommand{\jags}{\textbf{JAGS}}
\newcommand{\secr}{\mbox{\tt secr}}
\newcommand{\scrbook}{\mbox{\tt scrbook}}


\linenumbers

\begin{document}




\input{Ch18-Unmarked/Chapt-Unmarked.tex}





\bibliography{AndyRefs_alphabetized}


\end{document}